...
class Cotizaciones_model extends Model{
    ...
    public function listar_presupuestos($number_items='',$offset='')
    {
        ...
        $where = array("COMPP_Codigo"=>$compania,"CPC_TipoOperacion"=>"S");
    ...
    {   $compania = $this->somevar['compania'];
        ...
        $sql = "SELECT p.PRESUC_Fecha,
                ...
                WHERE p.COMPP_Codigo =".$compania." ".$where." AND CPC_TipoOperacion='S'
    ...
    public function insertar_presupuesto($filter=null)
    {
        ...
        $filter->CLIP_Codigo=null;//agregado por error a la hora de insertar
		$filter->MONED_Codigo=$filter->MONED_Codigo ;//agregado por error a la hora de insertar
    ...
    public function insertar_presupuesto_varios($filter=null,$pro_codigo){
        $compania = $this->somevar['compania'];
        $user = $this->somevar['user'];
        $filter->COMPP_Codigo = $compania;
        $filter->USUA_Codigo = $user;
        $filter->CPC_TipoOperacion='S';
        $filter->MONED_Codigo=$filter->MONED_Codigo;
        $filter->CLIP_Codigo=1;
        $filter->PROVP_Codigo= $pro_codigo;
        $this->db->insert('cji_presupuesto',(array)$filter);
        $presupuesto = $this->db->insert_id();
        return $presupuesto; 
    }
    ...
    {
        $where = array("COMPP_Codigo"=>$this->somevar['compania'],"PRESUC_FlagEstado"=>"1","CPC_TipoOperacion"=>'S');
        $query = $this->db->order_by('PRESUP_Codigo','desc')
                           ->where_not_in('PRESUP_Codigo','0')
                           ->where($where)
                           ->get('cji_presupuesto');
    ...
    {
        $where = array("COMPP_Codigo"=>$this->somevar['compania'], 'PRESUC_TipoDocumento'=>$tipo_docu,'CPC_TipoOperacion'=>'S');
        $query = $this->db->order_by('PRESUP_Codigo','desc')
                           ->where_not_in('PRESUP_Codigo','0')
                           ->where($where)
                           ->get('cji_presupuesto');
