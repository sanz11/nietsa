...
public function countFilas($tipo_oper, $filter = NULL, $number_items = '', $offset = ''){
    ...
    public function buscarGuiaremComprobante($guiarem,$estadoAsociacion){
     ...
     public function listar_guiarem_pdf($flagbs ,$fech1, $fech2,$serie,$numero, $cliente, $producto)
    {
        $compania = $this->somevar['compania'];
        $data_confi = $this->companiaconfiguracion_model->obtener($compania);
        $data_confi_docu = $this->companiaconfidocumento_model->obtener($data_confi[0]->COMPCONFIP_Codigo, 10);
        ...
        $where = '';
        if ($fech1 != '--' && $fech2 != '--')
            $where = ' and g.GUIAREMC_Fecha BETWEEN "' . human_to_mysql($fech1) . '" AND "' . human_to_mysql($fech2) . '"';
        switch ($data_confi_docu[0]->COMPCONFIDOCP_Tipo) {
            case '1':
                if ($numero != '--')
                    $where .= ' and g.GUIAREMC_Numero="' . $numero . '"';
                break;
            case '2':
                if ($serie!= '--' &&$numero != '--')
                    $where .= ' and g.GUIAREMC_Numero="' . $serie . '" and g.GUIAREMC_Numero="' . $numero . '"';
                break;
            case '3':
                if ($cliente != '--')
                    $where .= ' and g.GUIAREMC_Serie="' . $cliente . '"';
                break;
        }
        if ($flagbs != 'C') {
            if ($cliente != '--')
                $where .= ' and g.CLIP_Codigo=' . $cliente;
        } else {
            if ($cliente != '--')
                $where .= ' and g.PROVP_Codigo=' . $cliente;
        }

        if ($serie != '--' && $numero!= '--')
            $where .= ' and g.GUIAREMC_Serie="' . $serie. '" and g.GUIAREMC_Numero="' . $numero. '"';
        if ($producto && $producto!= '--')
            $where .= ' and gd.PRODCTOP_Codigo=' . $producto;
        $limit = "";
        if ($cliente != '--')
            $where .= ' and e.EMPRC_Ruc=' . $cliente;
        ...
        $sql = "SELECT g.GUIAREMC_Fecha,
                       ...
                       g.GUIAREMC_Numero,
                       g.GUIAREMC_NumeroRef,
                       (CASE " . ($flagbs != 'C' ? "c.CLIC_TipoPersona" : "c.PROVC_TipoPersona") . "  WHEN '1'THEN e.EMPRC_Ruc ELSE pe.PERSC_NumeroDocIdentidad end) numdoc,
                       (CASE " . ($flagbs != 'C' ? "c.CLIC_TipoPersona" : "c.PROVC_TipoPersona") . "  WHEN '1'THEN e.EMPRC_RazonSocial ELSE CONCAT(pe.PERSC_Nombre , ' ', pe.PERSC_ApellidoPaterno, ' ', pe.PERSC_ApellidoMaterno) end) nombre,
                       m.MONED_Simbolo,
                       g.GUIAREMC_total,
                       g.GUIAREMC_FlagEstado,
                       g.GUIAREMC_TipoGuia
                ...
                LEFT JOIN cji_almacen al ON al.ALMAP_Codigo=g.ALMAP_Codigo
                LEFT JOIN cji_guiaremdetalle gd ON gd.GUIAREMP_Codigo=g.GUIAREMP_Codigo
                " . ($flagbs != 'C' ? "LEFT JOIN cji_cliente c ON c.CLIP_Codigo=g.CLIP_Codigo" : "LEFT JOIN cji_proveedor c ON c.PROVP_Codigo=g.PROVP_Codigo") . "
                LEFT JOIN cji_persona pe ON pe.PERSP_Codigo=c.PERSP_Codigo AND " . ($flagbs != 'C' ? "c.CLIC_TipoPersona" : "c.PROVC_TipoPersona") . " ='0'
                LEFT JOIN cji_empresa e ON e.EMPRP_Codigo=c.EMPRP_Codigo AND " . ($flagbs != 'C' ? "c.CLIC_TipoPersona" : "c.PROVC_TipoPersona") . "='1'
                WHERE g.GUIAREMC_TipoOperacion='" . $flagbs . "'
                    AND g.GUIAREMC_FlagEstado != 9
                      AND g.COMPP_Codigo =" . $compania . " " . $where . "
                    
                      GROUP BY g.GUIAREMP_Codigo
                ORDER BY " . ($flagbs == 'V' ? 'g.GUIAREMC_FechaRegistro DESC' : 'g.GUIAREMC_FechaRegistro DESC') . " " . $limit;
        $query = $this->db->query($sql);
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
