...
function calcula_importe(n){
    ...
    precio = money_format(pu*cantidad);//pu*
    //precio = money_format(pu_conigv*cantidad);//pu*
    total_dscto = money_format((precio*descuento100)/100);
    calcularDescuento=money_format(((pu_conigv*cantidad)*descuento100)/100);
    ...
    importe = money_format(precio-parseFloat(calcularDescuento)+parseFloat(total_igv));
...
function listar_contactos(empresa){
    ...
    if(tipo_oper=='V')
        ...
        cargo='4';
    url = base_url+"index.php/maestros/empresa/JSON_listar_contactos/"+empresa+"/"+cargo;
    $.getJSON(url,function(data){
        console.log(data);
        $.each(data, function(i,item){
            console.log(item);
