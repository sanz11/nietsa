...
function comprobante_ver_pdf_conmenbrete(fondo, comprobante) {
...
function seleccionar_comprobante(guia, serie, numero_comprobante, tipo) {
    ...
    if (tipo == 1) {
        view_document = 'DOCUMENTO FACTURA <br> Serie: 00' + serie + ' Numero: ' + numero_comprobante;
    } else if (tipo == 2) {
        view_document = 'DOCUMENTO BOLETA <br> Serie: 00' + serie + ' Numero: ' + numero_comprobante;
    } else if (tipo == 3) {
        view_document = 'DOCUMENTO COMPROBANTE <br> Serie: 00' + serie + ' Numero: ' + numero_comprobante;
    } else {
        view_document = 'DOCUMENTO INDEFINIDO <br> Serie: 00' + serie + ' Numero: ' + numero_comprobante;
    }

    $('#idSerie').val(serie);
    $('#idNumero').val(numero_comprobante);

    $.ajax({
        url: base_url + 'index.php/ventas/notacredito/detalle_comprobante',
        type: "POST",
        data: {
            comprobante: guia
        },
        ...
        beforeSend: function (data) {
            $('#serieguiaver').html(view_document);
            $('#serieguiaver').show('slow');
            $('#serieguiaverFlecha').show('fast');
            $('#origenDocumento').val(tipo);
            $('#guiaReferente').val(guia);
        },
        success: function (data) {

            $('#tblDetalleComprobante').empty();

            $.each(data, function (i, item) {

                j = n + 1;
                if (j % 2 == 0) clase = "itemParTabla";
                else clase = "itemImparTabla";
                flagBS = $("#flagBS").val();
                producto = item.PROD_Codigo;
                codproducto = item.PROD_CodigoInterno;
                unidad_medida = item.UNDMED_Codigo;
                ...
                igv = item.CPDEC_Igv;
                igv100 = item.CPDEC_Igv100;
                descuento100 = item.CPDEC_Descuento100;
                pu_sinigv = money_format(pu * 100 / (igv100 + 100));
                total = item.CPDEC_Total;
                subTotal = money_format(subTotal + Number(total));
                importe = money_format(pu * cantidad);
                ...
                fila += '<span style="border:1px solid red;background: #ffffff;">&nbsp;X&nbsp;</span>';
                fila += '</a></strong></font></div></td>';
                fila += '<td width="4%"><div align="center">' + j + '</div></td>'; // ITEM
                fila += '<td width="10%"><div align="center">' + codproducto; // CODIGO INTERNO
                fila += '<input type="hidden" name="flagBS[' + n + ']" id="flagBS[' + n + ']" value="' + flagBS + '">';
                fila += '<input type="hidden" class="cajaMinima" name="produnidad[' + n + ']" id="produnidad[' + n + ']" value="' + unidad_medida + '">';
                fila += '</div></td>';
                fila += '<td><div align="left">';// DESCRIPCION
                fila += '<input type="text" class="cajaGeneral" style="width:395px;" maxlength="250" name="proddescri[' + n + ']" id="proddescri[' + n + ']" value="' + nombre_producto + '">';
                fila += '<input type="hidden" class="cajaMinima" name="prodcodigo[' + n + ']" id="prodcodigo[' + n + ']" value="' + producto + '">'; // Codigo del producto
                fila += '</div></td>';
                fila += '<td width="10%" ><div align="left" >'; // CANTIDAD
                fila += '<input type="text" size="5" maxlength="10" class="cajaGeneral" name="prodcantidad[' + n + ']" id="prodcantidad[' + n + ']" value="' + cantidad + '" onblur="calcula_importe(' + n + ');" onkeypress="return numbersonly(this,event,\'.\');">';
                fila += '</div></td>';
                fila += '<td width="6%"><div align="center">'; // PRECIO CON IGV
                fila += '<input type="text" size="5" maxlength="10" class="cajaGeneral" value="' + pu_conigv + '" name="prodpu_conigv[' + n + ']" id="prodpu_conigv[' + n + ']" name="prodpu_conigv[' + n + ']" onblur="modifica_pu_conigv(' + n + ')" onkeypress="return numbersonly(this,event,\'.\');">';
                fila += '</div></td>';
                fila += '<td width="6%"><div align="center">'; // PRECIO UNITARIO SIN IGV
                fila += '<input type="text" size="5" maxlength="10" class="cajaGeneral" value="' + pu + '" name="prodpu[' + n + ']" id="prodpu[' + n + ']" onblur="modifica_pu(' + n + ')" onkeypress="return numbersonly(this,event,\'.\');">';
                fila += '</div></td>';
                fila += '<td width="6%"><div align="center">'; // PRECIO
                fila += '<input type="text" size="5" maxlength="10" class="cajaGeneral" value="' + total + '" name="prodprecio[' + n + ']" id="prodprecio[' + n + ']" >';
                fila += '<input type="hidden" size="5" class="cajaGeneral cajaSoloLectura" name="prodsubtotal[' + n + ']" id="prodsubtotal[' + n + ']" value="' + subtotal + '" readonly="readonly">';
                fila += '<input type="hidden" value="n" name="detaccion[' + n + ']" id="detaccion[' + n + ']">';
                fila += '<input type="hidden" name="prodigv[' + n + ']" id="prodigv[' + n + ']" value="' + igv + '">';
                fila += '<input type="hidden" name="proddescuento[' + n + ']" id="proddescuento[' + n + ']" value="' + descuento + '">';
                fila += '<input type="hidden" name="prodcosto[' + n + ']" id="prodcosto[' + n + ']" value="' + costo + '">';
                fila += '<input type="hidden" name="prodimporte[' + n + ']" id="prodimporte[' + n + ']" value="' + importe + '">';
                fila += '<input type="hidden" name="prodigv100[' + n + ']" id="prodigv100[' + n + ']" value="' + igv100 + '">';
                fila += '<input type="hidden" name="proddescuento100[' + n + ']" id="proddescuento100[' + n + ']" value="' + descuento100 + '">';
                fila += '<input type="hidden" class="cajaMinima" name="flagGenIndDet[' + n + ']" id="flagGenIndDet[' + n + ']" value="' + flagGenInd + '">'; // Generico o Individual
                fila += '</div></td>';
                fila += '</tr>';
                n++;
            });
        ...
        error: function (HXR, error) {
            console.log('errrrprrrr');
            $('#serieguiaver').hide('slow');
            $('#serieguiaverFlecha').hide('fast');
        }
    });
