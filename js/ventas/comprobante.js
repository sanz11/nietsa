...
function buscarComprobanteonclick(){
    activarBusqueda();
}*/
...
function agregar_producto_comprobante() {
    flagBS = $("#flagBS").val();
//alert($("#marcaNombreIdentif").val());
      var cajaMarca =$("#marcaNombreIdentif").val();
   ...
   if ($("#serie").val() == '') {
    ...
    fila += '<td width="36%"><div align="left"><input type="text" class="cajaGeneral" size="60" maxlength="250" name="proddescri[' + n + ']" id="proddescri[' + n + ']" value="' + nombre_producto + '" /> &nbsp;'+cajaMarca+'</div></td>';
...
function obtener_clienteV() {
    var numdoc = $("#ruc_clienteV").val();
    $('#clienteV,#nombre_clienteV').val('');
    ...
    $.getJSON(url, function (data) {
        $.each(data, function (i, item) {
            if (item.EMPRC_RazonSocial != '') {
                $('#nombre_clienteV').val(item.EMPRC_RazonSocial);
                $('#clienteV').val(item.CLIP_Codigo);
                $('#codproductoV').focus();
                

            }
            else {
                $('#nombre_clienteV').val('No se encontró ningún registro');
                $('#linkVerClienteV').focus();
            }
        });
    });
    return true;
...
function obtener_cliente1() {
    var numdoc = $("#ruc_proveedor").val();
    $("#cliente, #nombre_proveedor").val('');
...
function obtener_cliente2() {
    var numdoc = $("#ruc_proveedorV").val();
    $("#clienteV, #nombre_proveedorV").val('');
    ...
    $.getJSON(url, function (data) {
        $.each(data, function (i, item) {
            ...
            if (item.EMPRC_RazonSocial != '') {
                $('#nombre_proveedorV').val(item.EMPRC_RazonSocial);
               $('#clienteV').val(item.CLIP_Codigo);
                $('#codproductoV').focus();
            }
            else {
                $('#nombre_proveedorV').val('No se encontró ningún registro');
                $('#linkVerProveedorV').focus();
            }
        });
    });
    return true;
...
function agregar_todo(guia) {
    ...
    $.getJSON(url, function (data) {
        ...
        $.each(data, function (i, item) {
            ...
            serie = item.GUIAREMC_Serie;
            numero = item.GUIAREMC_Numero;
            serie_nuem=item.Ruc;//item.serie_nuemro;
            codigo_usuario = item.GUIAREMC_CodigoUsuario;
            SerieNumeroOrde=item.OCOMP_Codigo;
            if (item.PRESDEP_Codigo != '') {
                j = n + 1
                producto = item.PROD_Codigo;
                codproducto = item.PROD_CodigoInterno;
            ...
            $('#forma_pago').val(formapago);
            $('#moneda').val(moneda);
            //$("#num_orden_venta").val(serie_nuem);
            $("#num_orden_venta").val(SerieNumeroOrde);
           if (codigo_usuario)
                $("#docurefe_codigo").val(codigo_usuario);
            else if (serie)
                $("#docurefe_codigo").val(serie + '-' + numero);
            else
                $("#docurefe_codigo").val(serie + '-' + numero);
...
function modifcarDependi(){

    var codigo=$("#codigo").val();
    var identif=$("#forma_pago option:selected").text();
    var fecha=$("#fecha").val();
    ...
    if(identif=="CONTADO"){
        valord="C"
       // alert(valord+"el Codigo no ara nada");
    }else{
        valord="V"
       // alert(valord);
    }
    var url_data="index.php/ventas/comprobante/JSON_ModificarCuenta/";
    var url= base_url+url_data+codigo+"/"+valord;
    dataString = "fecha="+fecha+"&importetotal="+importetotal;
    $.post(url,dataString,function(data){
...
function activarBusqueda(){
       ...
       var urlJs=base_url+"js/ventas/comprobante.js";
