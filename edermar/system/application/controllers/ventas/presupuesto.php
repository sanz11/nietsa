...
class Presupuesto extends Controller {
    ...
    public function presupuestos($j = '0', $limpia = '') {
        ...
        $data['titulo_tabla'] = "RELACIÃƒâ€œN DE PRE-VENTA";
...
<tr>
<td style="padding:0px;">CuÃƒÂ¡ndo:&nbsp;' . date("Y-m-d") . '</td>
</tr>
<tr>
...
<div style="padding-bottom:14px;">' . utf8_encode($xls) . '
    ...
    public function presupuesto_ver_xls_correo($codigo) {
      ...
      <td><b>$subtotal_text</b></td><td><b>" . number_format($subtotal, 2) . "</b></td></tr>
        ...
        if (FORMATO_IMPRESION != 3) {
            $xls .= "<tr><td>" . utf8_decode_seguro('Tipo de Cambio del DÃƒÂ­a :') . "</td><td>" . ($tipo_cambio > 0 ? round($tipo_cambio, 2) : '') . "</td></tr>
        ...
        if (FORMATO_IMPRESION == 3) {
            $xls .= "<tr><td>" . utf8_decode_seguro('Banco de CrÃƒÂ©dito Soles') . "</td><td>" . utf8_decode_seguro('NÃ‚Â°  191-1435467-0-65') . "</td></tr>
        <tr><td>" . utf8_decode_seguro('Banco de CrÃƒÂ©dito DÃƒÂ³lares') . "</td><td>" . utf8_decode_seguro('NÃ‚Â° 191-1466829-1-62') . "</td></tr>";
        } else {
            $xls .= "<tr><td>" . utf8_decode_seguro('Cta. Cte. en Soles') . "</td><td>" . utf8_decode_seguro('NÃ‚Â°  191-1435467-0-65') . "</td></tr>
        <tr><td>" . utf8_decode_seguro('Cta. Cte. en DÃƒÂ³lares') . "</td><td>" . utf8_decode_seguro('NÃ‚Â° 191-1466829-1-62') . "</td></tr>";
        }
    ...
    public function presupuesto_ver_pdf_correo($codigo, $img) {
    	...
    	if (count($data_tipocambio) > 0)
    		...
    		$this->cezpdf = new backgroundPDF('a4', 'portrait', 'image', array('img' => 'images/documentos/presupuestoformato.jpg'));
    		$datacreator = array(
    				'Title' => 'Estadillo de ',
    				'Name' => 'Estadillo de ',
    				'Author' => 'gian carlos',
    		...
    		///////////
    		$this->cezpdf->ezText('', 8);
    		$col_namesCuenta = array(
    				'cols1' => 'CTA. CTE. SOLES <b>BCP</b>'
    	
    		);
    		$db_dataCuenta[] = array(
    				'cols1' => '191-2058142-0-12'
    		);
    	
    		$this->cezpdf->ezTable($db_dataCuenta, $col_namesCuenta, '', array(
    				'width' => 500,
    				'showLines' => 1,
    				'shaded' => 0,
    				'showHeadings' => 1,
    				...
    				'fontSize' => 8,
    				'cols' => array(
    						'cols1' => array('width' => 220, 'justification' => 'LEFT'),
    				)
    		));
...
//         $xls .= "</table>";
    ...
    public function presupuesto_ver_pdf_formato1($codigo) {
        ...
        /* Condiciones de venta */
        $db_data = array(array('cols0' => '<b>CONDICIONES DE VENTA:</b>', 'cols1' => ''),
            array('cols0' => utf8_decode_seguro('Tipo de Cambio del DÃƒÂ­a'), 'cols1' => ': ' . ($tipo_cambio > 0 ? round($tipo_cambio, 2) : '')),
            array('cols0' => 'Moneda', 'cols1' => ': ' . $moneda_nombre),
            array('cols0' => 'Forma de Pago', 'cols1' => ': ' . utf8_decode_seguro($forma_pago)),
            array('cols0' => 'Los Precios de los Productos ', 'cols1' => ': ' . ($modo_impresion == '1' ? 'CONTIENEN IGV' : 'NO CONTIENEN IGV')),
            array('cols0' => utf8_decode_seguro('Cta. Cte. en Soles'), 'cols1' => ': ' . utf8_decode_seguro('NÃ‚Â°  191-1435467-0-65')),
            array('cols0' => utf8_decode_seguro('Cta. Cte. en DÃƒÂ³lares'), 'cols1' => ': ' . utf8_decode_seguro('NÃ‚Â° 191-1466829-1-62')),
            array('cols0' => 'Tiempo de Entrega', 'cols1' => ': ' . $tiempo_entrega),
            array('cols0' => 'Lugar de Entrega', 'cols1' => ': ' . utf8_decode_seguro($lugar_entrega)),
            array('cols0' => 'Validez de la Oferta', 'cols1' => ': ' . utf8_decode_seguro($validez)),
            array('cols0' => 'Contacto', 'cols1' => ': ' . utf8_decode_seguro($vendedor_nombre . ($vendedor_nombre_area != '' ? ' - AREA: ' . $vendedor_nombre_area : '')))
    ...
    public function presupuesto_ver_pdf_conmenbrete_formato3($codigo, $img) {
        ...
        $this->cezpdf_horizontal->ezText(utf8_decode_seguro('Lima: Av. Chorrillos NÃ‚Âº 200 Chorrillos - Lima  Telf.: 251 6727  Fax: 252 7547'), 8, array("left" => 15));
        ...
        foreach ($detalle_presupuesto as $indice => $valor) {
            ...
            if ($total_items > 11) {
                ...
                if ($contador == 11) {
                    $col_names = array(
                        'cols1' => 'Item',
                        'cols2' => utf8_decode_seguro('CÃƒÂ³digo'),
                        'cols3' => 'Marca',
                        'cols4' => 'Modelo',
        ...
        $db_data = array(array('cols0' => '<b>Condiciones de venta:</b>', 'cols1' => '', 'cols2' => ''),
            ...
            array('cols0' => utf8_decode_seguro('Banco de CrÃƒÂ©dito Soles'), 'cols1' => ':', 'cols2' => 'Cta. Cte. No. 285-1178292-0-25'),
            array('cols0' => utf8_decode_seguro('Banco de CrÃƒÂ©dito DÃƒÂ³lares'), 'cols1' => ':', 'cols2' => 'Cta. Cte. No. 285-1202278-1-18'),
           ...
           // array('cols0' => utf8_decode_seguro('GarantÃƒÂ­a'), 'cols1' => ':', 'cols2' => utf8_decode_seguro($garantia)),
