...
//                      echo "<script>alert('count(prodcodigo)  : ".count($prodcodigo) ."')</script>";
        ...
        public function ver_reporte_pdf_productos($anio ,$mes ,$fech1 ,$fech2, $tipodocumento,$Prodcod)
        {
        	...
        	if($anio!="--" && $mes =="--" ){// SOLO AÑO
        		$this->cezpdf->ezText("AÑO: ".$anio, 8, $fontdata);
        	}
        	if($anio!="--" && $mes !="--" ){// MES Y  AÑO
        		$this->cezpdf->ezText("AÑO : ".$anio. "     MES : ".$this->get_mesletra($mes), 8, $fontdata);
        	}
        	if($anio=="--" && $mes !="--"){//MES CON AÑO ACTUAL
        		$this->cezpdf->ezText("AÑO : ".date("Y"). "    MES : ".$this->get_mesletra($mes), 8, $fontdata);
        	}
