...
class Cliente extends Controller
{
    ...
    public function clientes($j = 0)
    {
        ...
        $data['tipo'] = "";
        $data['calificaciones'] = "";
        $data['titulo_tabla'] = "RELACIÃ“N DE CLIENTES";
        ...
        if (count($listado_clientes) > 0) {
            foreach ($listado_clientes as $indice => $valor) {
                ...
                $eliminar = "<a href='#' onclick='eliminar_cliente(" . $codigo . ")'><img src='" . base_url() . "images/eliminar.png' width='16' height='16' border='0' title='Eliminar (Solo puedes eliminar los clientes de tu compaÃ±ia )'></a>";
    ...
    public function insertar_cliente()
    {
        ...
        if ($this->input->post('tipo_persona') == 0) {
            if ($this->input->post('tipo_documento') == '1' && $this->input->post('numero_documento') != '' && strlen($this->input->post('numero_documento')) != 8)
                exit ('{"result":"error", "campo":"numero_documento", "msg": "Valor invÃ¡lido"}');
            ...
            if ($this->input->post('cboTipoCodigo') == '1' && $this->input->post('ruc') != '' && strlen($this->input->post('ruc')) != 11)
                exit ('{"result":"error", "campo":"ruc", "msg": "Valor invÃ¡lido"}');
    ...
    public function editar_cliente($id)
    {
        ...
        $this->load->view("ventas/cliente_nuevo", $data);
//         $this->layout->view("ventas/cliente_nuevo", $data);
    ...
    public function modificarclienteeditar()
    {
    	...
    	if ($tipo_persona == 1) {    	
    		//Modificar contactos empresa    		
    		$this->empresa_model->modificar_datosEmpresaCodificado($empresa, $tipocodigo, $ruc, $razon_social, $telefono, $movil, $web, $email, $direccion);
    		$this->empresa_model->modificar_sucursalEmpresaPrincipal($empresa, '1', 'PRINCIPAL', $direccion);
    	}else{
//     		$this->persona_model->modificar_datosPersonaCodificado($persona, $nombres, $paterno, $materno, $ruc_persona, $tipo_documento, $numero_documento, $direccion, $telefono, $movil, $email, $web );
    		$this->persona_model->modificar_datosPersonaCodificado($persona,$direccion, $telefono, $movil, $email, $web );
    		
    	}
    	
    	$this->cliente_model->modificar_datosCliente($id, $categoria, $calificaciones='');

    	$data['redireccionarcloce']= "1";
    	$this->load->view("ventas/cliente_editar_index" ,$data);
    }
    ...
    public function buscar_clientes($j = '0')
    {
        ...
        $data['titulo_tabla'] = "RESULTADO DE BÃšSQUEDA DE CLIENTES";
    ...
    public function ventana_busqueda_cliente($j = 0, $limpia = '')
    {
        ...
        if (count($listado_clientes) > 0) {
            foreach ($listado_clientes as $indice => $valor) {
                ...
                $editar = "<a href='#' onclick='editar_clientepopup(" . $codigo . ")'><img src='" . base_url() . "images/modificar.png'  border='0' title='Modificar'></a>";
                $lista[] = array($item, $ruc_c, $dni_c, $razon_social_c, $tipo_cliente, $seleccionar,$editar, $codigo);
                $item++;
            }
        }
...
//     public function obtener_cliente($codigo){
...
//     }
    ...
    public function ventana_editar_cliente($codigo){
    	
    	$datos = $this->cliente_model->obtener_datosCliente($codigo);
    	$tipo_persona = $datos[0]->CLIC_TipoPersona;
    	$persona = $datos[0]->PERSP_Codigo;
    	$empresa = $datos[0]->EMPRP_Codigo;
    	$data['display'] = "style='display: none'";
    	...
    	$data['tipo_persona'] = $tipo_persona;
    	$data['id'] = $codigo;
    	$data['redireccionarcloce']='';
    	if ($tipo_persona == 0) {//Persona
    		$data['display_datosEmpresa'] = "display:none;";
    		$data['display_datosPersona'] = "";
    		$data['tituloclienteeditar'] = "EDITAR CLIENTE NATURAL ";
    		
    		$datos_persona = $this->persona_model->obtener_datosPersona($persona);
    		$tipocodigo = $datos_persona[0]->PERSC_TipoDocIdentidad;
    		$data['direccion'] = $datos_persona[0]->PERSC_Direccion;
    		$data['telefono'] = $datos_persona[0]->PERSC_Telefono;
    		$data['movil'] = $datos_persona[0]->PERSC_Movil;
    		$data['correo'] = $datos_persona[0]->PERSC_Email;
    		$data['web'] = $datos_persona[0]->PERSC_Web;
    		
    		$data['numero_documento'] = $datos_persona[0]->PERSC_NumeroDocIdentidad;
    		$data['ruc_persona'] = $datos_persona[0]->PERSC_Ruc;
    		$data['nombres'] = $datos_persona[0]->PERSC_Nombre;
    		$data['paterno'] = $datos_persona[0]->PERSC_ApellidoPaterno;
    		$data['materno'] = $datos_persona[0]->PERSC_ApellidoMaterno;
    	...
    	} else if ($tipo_persona == 1) { //EMPRESA
    		$data['tituloclienteeditar'] = "EDITAR CLIENTE JURIDICO";
    		$data['display_datosEmpresa'] = "";
    		$data['display_datosPersona'] = "display:none;";
    		$datos_empresa = $this->empresa_model->obtener_datosEmpresa($empresa);
    		$razon_social = $datos_empresa[0]->EMPRC_RazonSocial;
    		...
    		$data['razon_social'] = $datos[0]->EMPRC_RazonSocial;
    		$data['ruc'] = $datos[0]->EMPRC_Ruc;
    		$data['direccion'] = $datos[0]->EMPRC_Direccion;
    		$data['telefono'] = $datos[0]->EMPRC_Telefono;
    		$data['movil'] = $datos[0]->EMPRC_Movil;
    		$data['correo'] = $datos[0]->EMPRC_Email;
    		$data['web'] = $datos[0]->EMPRC_Web;
			$data['tipocodigo'] = $this->seleccionar_tipocodigo($tipocodigo);
