        ...
        if (count($listado_presupuestos) > 0) {
            foreach ($listado_presupuestos as $indice => $valor) {
                ...
                if ($valor->CLIP_Codigo == 144 ||
                    ...
                    $empreTelefono = $valor->EMPRC_Telefono;
                    $email = $valor->EMPRC_Email;
                    $fechafinsegui = $valor->SEGUIPRE_FechaSistema;
                ...
                $tp = $this->presupuesto_model->obtener_presupuesto($codigo);
                foreach ($tp as $inicio => $val){
                	$codigotipopresupuesto = $val->PRESUP_TipoPresupuesto;
                }
                ...
                $ver2 = "<a href='javascript:;' onclick='ver_presupuesto_ver_pdf_conmenbrete(" . $codigo . ",0,".$codigotipopresupuesto.")' target='_parent'><img src='" . base_url() . "images/pdf.png' width='16' height='16' border='0' title='Ver PDF'></a>";
                ...
                $segimientoCotizacion="<a href='" . base_url() . "index.php/ventas/presupuesto/seguimiento_presupuesto_ventana/" . $codigo . "' class='seguimientodecoti'><img src='" . base_url() . "images/seguicoti.png' width='16' height='16' border='0' title='seguimiento de cotización'></a>";
                
                $listaseguicoti = $this->presupuesto_model->listaseguipresu();
                foreach ($listaseguicoti as $indise1 => $valor1){
                	$codigopresupuesto = $valor1->PRESUP_Codigo;
                
                
                }
                
                //                 echo "<script>alert('codigopresupuesto : ".$codigopresupuesto." and ".$codigo."')</script>";
                if($codigopresupuesto == $codigo){
                	//                 	echo "<script>alert('codigopresupuesto : ".$codigopresupuesto."')</script>";
                	$buscarseguipresu = $this->presupuesto_model->buscarseguimiento_presupuesto($codigopresupuesto);
                	foreach ($buscarseguipresu as $indicando => $valores){
                		$codigoestado = $valores->STATE_Codigo;
                		 
                	}
                
                	$codigocolorstate = $this->state_model->buscarcolor($codigoestado);
                	$color = $codigocolorstate[0]->STATE_Color;
                
                	$estadoseguimiento =  $color;
                
                }else{
                	$estadoseguimiento = "vacio";
                }
                
                $lista[] = array($item++, $fecha, $serie, $this->getOrderNumeroSerie($numero), $codigo_usuario, $nombre_cliente,$empreTelefono,$email,$fechafinsegui, $nom_tipodocu, $total, $img_estado, $editar, $ver, $ver2, $ver3, $enviarcorreo, $vermensaje, $eliminar,$segimientoCotizacion);
                }
                }
...
//********************************************************
        ...
        $filter->PRESUP_TipoPresupuesto = $this->input->post('tipopresupuesto');
        //} else {
    ...
    public function presupuesto_editar($codigo) {
        ...
        $tipopresupuesto = $datos_presupuesto[0]->PRESUP_TipoPresupuesto;
        ...
        $data['tipopresupuesto'] = $tipopresupuesto;
...
//PRESUPUESTO EXCEL VENTA COTIZACION
    ...
    public function presupuesto_ver_pdf_conmenbrete($codigo, $img,$codigotipopresu='') {
        switch (FORMATO_IMPRESION) {
            case 1:  //Formato para jimmyplat
                 $this->presupuesto_ver_pdf_conmenbrete_formato1($codigo, $img,$codigotipopresu);
                break;
            case 2:  //Formato para jimmyplat
    ...
    public function presupuesto_ver_pdf_conmenbrete_formato1($codigo, $img,$codigotipopresu) {
        ...
        if($codigotipopresu == 2){ //TIPO DE PRESUPUESTO 2:SERVICIO  ; 1:PRODUCTO
        	
        	$this->cezpdf = new backgroundPDF('a4', 'portrait', 'image', array('img' => 'images/cabeceras/presupuesto_Servicios_Plantilla.jpg'));
        	
        	$this->cezpdf->ezText(utf8_decode_seguro('<b>COTIZACIÓN N° ') . $codificacion . '</b>', 12, array("leading" => 80, 'left' => 185));        	
        	
        	
          	$this->cezpdf->ezText('', 10);
          	
        	/* Datos del cliente */
        	$db_data = array(array('cols1' => utf8_decode_seguro('Nombres/Razon Social'), 'cols2' => ': ' . utf8_decode_seguro($nombre_cliente), 'cols3' => 'R.U.C.', 'cols4' => ': ' . $ruc . '       Fecha: ' . $fecha),
        			array('cols1' => utf8_decode_seguro('Dirección'), 'cols2' => ': ' . utf8_decode_seguro($direccion), 'cols3' => utf8_decode_seguro('Teléfono'), 'cols4' => ': ' . $telefono), //NOMBRE DEL VENDEDOR
        			array('cols1' => utf8_decode_seguro('Correo Electrónico'), 'cols2' => ': ' . $email, 'cols3' => utf8_decode_seguro('Contácto'), 'cols4' => ': ' .utf8_decode_seguro($vendedor_nombre . ($nombre_area != '' ? ' - AREA: ' . $nombre_area : '')) )
        	);
        	...
        	$this->cezpdf->ezTable($db_data, "", "", array(
        			...
        			'cols' => array(
        					...
        					'cols2' => array('width' => 200, 'justification' => 'left'),
        					'cols3' => array('width' => 50, 'justification' => 'left'),
        					'cols4' => array('width' => 200, 'justification' => 'left')
        			)
        	));
	        ...
	        foreach ($detalle_presupuesto as $indice => $valor) {
	            ...
	            $db_data[] = array(
	                'cols1' => $indice + 1,
	            	'cols2' => $valor->PRESDEC_Cantidad,
	            	'cols3' => $valor->PROD_CodigoUsuario,
	                'cols4' => utf8_decode_seguro($valor->PROD_Nombre . ($valor->PROD_Modelo != '' ? ' - ' . $valor->PROD_Modelo : '')),
	                'cols5' => number_format(($modo_impresion == '1' || $tipo_docu == 'B' ? $valor->PRESDEC_Pu_ConIgv : $valor->PRESDEC_Pu), 2),
	                'cols6' => number_format($valor->PRESDEC_Cantidad * ($modo_impresion == '1' || $tipo_docu == 'B' ? $valor->PRESDEC_Pu_ConIgv : $valor->PRESDEC_Pu), 2)
	            );
	        }
	        $col_names = array(
	            'cols1' => 'ITEM',
	        ...
	        $this->cezpdf->ezTable($db_data, $col_names, '', array(
	            ...
	            'cols' => array(
	                ...
	                'cols3' => array('width' => 60,'justification' => 'left'),
	                'cols4' => array('width' => 280, 'justification' => 'left'),
	        ...
	        /* Condiciones de venta */
	        $db_data = array(array('cols0' => '<b>CONDICIONES DEL SERVICIO:</b>', 'cols1' => ''),
	        		array('cols0' => 'Forma de Pago', 'cols1' => ': ' . utf8_decode_seguro($forma_pago)),
	        		array('cols0' => 'Tiempo de Entrega', 'cols1' => ': ' . $tiempo_entrega),
	        		array('cols0' => 'Lugar de Entrega', 'cols1' => ': ' . utf8_decode_seguro($lugar_entrega)),
	        		array('cols0' => utf8_decode_seguro('Garantía'), 'cols1' => ': ' . utf8_decode_seguro($garantia)),
	        		array('cols0' => utf8_decode_seguro('Lugar para garantía'), 'cols1' => ': ' . utf8_decode_seguro("Of. Lima, salvo acuerdo en contrato de mantenimiento")),
	        		array('cols0' => 'Validez de la Oferta', 'cols1' => ': ' . utf8_decode_seguro($validez)),
	        		array('cols0' => 'Cuentas recaudadoras', 'cols1' => ': ' . utf8_decode_seguro("Banco de Crédito del Perú Soles 193-2226442-0-12")),
	        		array('cols0' => ' ', 'cols1' => ': ' . utf8_decode_seguro("Banco de Crédito del Perú Dólares 193-2206492-1-07")),
	        		array('cols0' => ' ', 'cols1' => ': ' . utf8_decode_seguro("BBVA Continental Soles 0011-0180-010002796875")),
	        		array('cols0' => '', 'cols1' => ': ' . utf8_decode_seguro("BBVA Continental Dólares 0011-0180-0100042940"))
// 	            array('cols0' => utf8_decode_seguro('Tipo de Cambio del DÃ­a'), 'cols1' => ': ' . ($tipo_cambio > 0 ? round($tipo_cambio, 2) : '')),
// 	            array('cols0' => 'Moneda', 'cols1' => ': ' . $moneda_nombre)
        	...
        	/**empiesa el reporte con imagen de producto producto**/
        	...
        	//relaci? utilizada para ajustar la conversi? de los p?eles
        	...
        	//Si tienes que imprimir car?teres ASCII est?dar, puede utilizar las fuentes b?icas como
        	// Helvetica para reducir el tama? del archivo.
        	$pdf->SetFont('helvetica', '', 10 , '', true);
        	
        	// A?dir una p?ina
        	// Este m?odo tiene varias opciones, consulta la documentaci? para m? informaci?.
        	$pdf->AddPage();
        	...
        	$pdf->Image(base_url().'images/cabeceras/logocaf.JPG',$x+0,$y+0, 0, 0, '', '', '', true, 100, '', false, false, 0, false, false, false);
        	
        	
        	
        	$html = '<br><br><h1 align="center">COTIZACIÓN N° '.$codificacion.'</h1>';
        	$pdf->writeHTML($html, true, false, true, false, '');
        	
        		
        	$tbl_header1='<table border="0" width="100%" >';
        	$tabla1.='<tr>';
        	$tabla1.='<td style="width:28%"><b>Nombres/Razon Social</b></td>';
        	$tabla1.='<td style="width:2%">:</td>';
        	$tabla1.='<td style="width:28%;font-family: Arial, Helvetica, sans-serif;font-size: 100%;">'.utf8_decode_seguro($nombre_cliente).'</td>';
        	$tabla1.='<td><b>Ruc</b></td>';
        	$tabla1.='<td style="width:2%">:</td>';
        	$tabla1.='<td style="width:30% ;" >'.$ruc.'</td>';
        	$tabla1.='</tr>';
        	$tabla1.='<tr>';
        	$tabla1.='<td><b>Dirección</b></td>';
        	$tabla1.='<td>:</td>';
        	$tabla1.='<td style="font-family: Arial;font-size: 100%;" >'.utf8_decode_seguro($direccion).'</td>';
        	$tabla1.='<td><b>Teléfono/Celular</b></td>';
        	$tabla1.='<td>:</td>';
        	$tabla1.='<td  style="font-family: Arial, Helvetica, sans-serif;font-size: 100%;">'.$telefono.'</td>';
        	$tabla1.='</tr>';
        	$tabla1.='<tr>';
        	$tabla1.='<td><b>Correo Electrónico</b></td>';
        	$tabla1.='<td>:</td>';
        	$tabla1.='<td style="font-family: Arial, Helvetica, sans-serif;font-size: 100%;">'.$email.'</td>';
        	$tabla1.='<td><b>Contácto</b></td>';
        	$tabla1.='<td>:</td>';
        	$tabla1.='<td style="font-family: Arial, Helvetica, sans-serif;font-size: 100%;">'.utf8_decode_seguro($nombre_contacto).'</td>';
        	$tabla1.='</tr>';
        	$tbl_footer1='</table>';
         	$pdf->writeHTMLCell(0,0,$x+10,$y+55,$tbl_header1 . $tabla1 . $tbl_footer1,0,1,false,true,'L',true);
        	
         	/************ Segunda Tabla ***********/
         	
           	$tbl_header2='<table style="border-collapse: collapse;   width: 100%;">';
         	$tabla2.='<tr style="background-color:#f5f5f5">';
         	$tabla2.='<th style="padding: 8px;text-align: left;border-bottom: 1px solid #ddd;width:17%"></th>';
         	$tabla2.='<th style="padding: 8px;text-align: left;border-bottom: 1px solid #ddd;width:6%">ITEM</th>';
         	$tabla2.='<th style="padding: 8px;text-align: left;border-bottom: 1px solid #ddd;width:10%">CANT</th>';
         	$tabla2.='<th style="padding: 8px;text-align: left;border-bottom: 1px solid #ddd;width:40%">DETALLE</th>';
         	$tabla2.='<th style="padding: 8px;text-align: left;border-bottom: 1px solid #ddd;">UNITARIO</th>';
         	$tabla2.='<th style="padding: 8px;text-align: left;border-bottom: 1px solid #ddd;">TOTAL</th>';
         	$tabla2.='</tr>';
         	foreach ($detalle_presupuesto as $indice => $valor) {
         		$producto = $valor->PROD_Codigo;
         		$marca_prod = $this->producto_model->obtener_marca_modelo_por_producto($producto);
         		$obtenerimgprod = $this->producto_model->obtener_img($producto);
         		foreach ($obtenerimgprod as $indicando => $proce){
         			$img = $proce->PROD_Imagen;
         		}
		         	$tabla2.='<tr>';
		         	$tabla2.='<td style=" padding: 8px;text-align: left;border-bottom: 1px solid #ddd;padding: 15px;"><img align="center" border="1" style="width:100px; height:100px" src="images/img_db/'.$img.'"></td>';
					$tabla2.='<td style=" padding: 8px;text-align: left;border-bottom: 1px solid #ddd;padding: 15px;">'.($indice + 1).'</td>';
					$tabla2.='<td style=" padding: 8px;text-align: left;border-bottom: 1px solid #ddd;padding: 15px;">'.$valor->PRESDEC_Cantidad.'</td>';
		         	$tabla2.='<td style=" padding: 8px;text-align: left;border-bottom: 1px solid #ddd;padding: 15px;">'.utf8_decode_seguro($valor->PROD_Nombre . ($valor->PROD_Modelo != '' ? ' - ' . $valor->PROD_Modelo : '')).'</td>';
		         	$tabla2.='<td style=" padding: 8px;text-align: left;border-bottom: 1px solid #ddd;padding: 15px;">'. number_format(($modo_impresion == '1' || $tipo_docu == 'B' ? $valor->PRESDEC_Pu_ConIgv : $valor->PRESDEC_Pu), 2).'</td>';
		         	$tabla2.='<td style=" padding: 8px;text-align: left;border-bottom: 1px solid #ddd;padding: 15px;">'.number_format($valor->PRESDEC_Cantidad * ($modo_impresion == '1' || $tipo_docu == 'B' ? $valor->PRESDEC_Pu_ConIgv : $valor->PRESDEC_Pu), 2).'</td>';
		         	$tabla2.='</tr>';
         	}
         	$tbl_footer2='</table>';
         	//style="width:21%"
	         	$pdf->writeHTMLCell(0,0,$x+10,$y+80,$tbl_header2 . $tabla2 . $tbl_footer2,0,1,false,true,'L',true);
         	
	         	$html = '<b>SON : '. strtoupper(num2letras(round($total, 2))) . ' ' . $moneda_nombre . '</b>';
	         	$pdf->writeHTML($html, true, false, true, false, '');
	         	
	         	
	         	/************ Tercera Tabla INICIO***********/
	         	$tbl_header3='<table border="0" >';
	         	$tabla3.='<tr>';
	         	$tabla3.='<td style="width:10%" >Subtotal</td>';
	         	$tabla3.='<td style="width:20%">'. number_format($subtotal, 2) .'</td>';
	         	$tabla3.='</tr>';
	         	$tabla3.='<tr>';
	         	$tabla3.='<td style="width:10%" >IGV</td>';
	         	$tabla3.='<td style="width:20%">'. number_format($igv, 2) .'</td>';
	         	$tabla3.='</tr>';
	         	$tabla3.='<tr>';
	         	$tabla3.='<td style="width:10%" >Total</td>';
	         	$tabla3.='<td style="width:20%">'. $moneda_simbolo . '	' .number_format($total, 2). '</td>';
	         	$tabla3.='</tr>';
	         	$tbl_footer3='</table>';
	         	//style="width:21%"
	         	$pdf->writeHTMLCell(180,0,$x+150,$y+200,$tbl_header3 . $tabla3 . $tbl_footer3,0,1,false,true,'L',true);
                ...
                $pdf->writeHTML($html, true, false, true, false, '');
	         	...
	         	$tbl_header4='<table border="0">';

	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:100%" >Condiciones De Venta</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Forma de Pago</td>';
	         	$tabla4.='<td style="width:50%">: '. utf8_decode_seguro($forma_pago) .'</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Monto mínimo de compra </td>';
	         	$tabla4.='<td style="width:50%">: US$ 50.00</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Monto mínimo para reparto </td>';
	         	$tabla4.='<td style="width:50%">: US$ 500.00</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Tiempo de Entrega</td>';
	         	$tabla4.='<td style="width:50%">: '. $tiempo_entrega .'</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Lugar de Entrega</td>';
	         	$tabla4.='<td style="width:80%">: '. utf8_decode_seguro($lugar_entrega) .'</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Garantía</td>';
	         	$tabla4.='<td style="width:50%">: '. $garantia .'</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Lugar para gárantia</td>';
	         	$tabla4.='<td style="width:50%">: Of. Lima, salvo acuerdo en contrato de mantenimiento </td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Validez de Pte</td>';
	         	$tabla4.='<td style="width:50%">: '. utf8_decode_seguro($validez) .'</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Tipo de Canbio del Día</td>';
	         	$tabla4.='<td style="width:50%">: '. ($tipo_cambio > 0 ? round($tipo_cambio, 2) : '').'</td>';
	         	$tabla4.='</tr>';     
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Cuentas recaudadoras</td>';
	         	$tabla4.='<td style="width:50%">: Banco de Crédito del Perú Soles 193-2226442-0-12</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" ></td>';
	         	$tabla4.='<td style="width:50%">: Banco de Crédito del Perú Dolares 193-2206492-1-07</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%"></td>';
	         	$tabla4.='<td style="width:50%">: BBVA Continental Soles 0011-0180-010002796875</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%"></td>';
	         	$tabla4.='<td style="width:50%">: BBVA Continental Dólares 0011-0180-0100042940</td>';
	         	$tabla4.='</tr>';
	         	$tbl_footer4='</table>';
	         	//style="width:21%"
	         	$pdf->writeHTMLCell(0,0,$x+10,$y+205,$tbl_header4 . $tabla4 . $tbl_footer3,0,1,false,true,'L',true);
        	...
        	$pdf->lastPage();
        	$nomDescarga='Presupuesto'; //date("Y");
        	ob_end_clean();
        	$pdf->Output($nomDescarga.'.pdf');
        }
    ...
    public function presupuesto_ver_pdf_conmenbrete_formato3($codigo, $img) {
        ...
        $db_data = array(array('cols1' => utf8_decode_seguro('Señor(es)'), 'cols2' => ': ' . utf8_decode_seguro($nombre_cliente), 'cols3' => 'R.U.C.', 'cols4' => ': ' . $ruc, 'cols5' => 'Fecha', 'cols6' => ': ' . $fecha, 'cols7' => ''),
            ...
            array('cols1' => utf8_decode_seguro('Contácto'), 'cols2' => ': ' . utf8_decode_seguro($nombre_contacto . ($nombre_area != '' ? ' - AREA: ' . $nombre_area : '')), 'cols3' => '', 'cols4' => '', 'cols5' => '', 'cols6' => '', 'cols7' => ''),
    ...
    public function presupuesto_ver_pdf_conmenbrete_formato4($codigo) {
        ...
        $db_data = array(array('cols0' => '<b>CONDICIONES DE VENTA:</b>', 'cols1' => ''),
            ...
            array('cols0' => utf8_decode_seguro('Garantía'), 'cols1' => ': ' . utf8_decode_seguro($garantia)),
            array('cols0' => 'Validez de la Oferta', 'cols1' => ': ' . utf8_decode_seguro($validez)),
            array('cols0' => 'Contácto', 'cols1' => ': ' . utf8_decode_seguro($vendedor_nombre . ($vendedor_nombre_area != '' ? ' - AREA: ' . $vendedor_nombre_area : '')))
        );
        $this->cezpdf->ezTable($db_data, "", "", array(
            'width' => 525,
            'showLines' => 0,
...
//                    
    ...
    public function seguimiento_presupuesto_ventana($codigo){
    	$nombre_persona1 = $this->session->userdata('nombre_persona');
    	$persona1 = $this->session->userdata('persona');
    	$Mensajesenviados = $this->presupuesto_model->correoenviado_presu($codigo);
    	$primerMesanje="";
    	...
    	$data['primerMesanje']=$primerMesanje;
    	$datos_usuario = $this->persona_model->obtener_datosPersona($persona1);
    	if ($datos_usuario) {
    		$emailusuario = $datos_usuario[0]->PERSC_Email;
    	}//14 es el codigo del documento
    	$data['listState']=$this->cmbListState(13,1);
    	$data['cbreportefinal'] =  $this->OPTION_generador($this->state_model->buscar_reportefinal(), 'REPORFIN_Codigo', 'REPORFIN_Nombre', '');
    	$compania = $this->somevar['compania'];
    	$data['compania'] = $compania;
    	$data['titulo'] = "SEGUIMIENTO DE COTIZACION";
    	$data['formulario'] = "frmPresupuestoCorreo";
    	$data['url_action'] = base_url() . "index.php/ventas/presupuesto/Enviarcorreo";
    	$data['hoy'] = date('d/m/Y');
    	$data['codigo'] = $codigo;
    
    	$data['listTable']=$this->presupuesto_model->mostrar_seguimiento_table($codigo);
    	$data['tipo_codificacion'] = 1;
    	$this->load->view('ventas/seguimiento_ventana_cotizacion', $data);
    }
    public function cmbListState($codigo,$index){
    	$array_area = $this->state_model->listar_estadosCotizacion($codigo);
    	$arreglo = array();
    	foreach($array_area as $indice=>$valor){
    		$indice1   = $valor->STATE_Codigo;
    		$valor1  = $valor->STATE_Estado;
    		$arreglo[$indice1] = $valor1;
    	}
    	$resultado = $this->html->optionHTML($arreglo,$index,array('','::Seleccione::'));
    	return $resultado;
    }
    public function gaurdar_seguimiento_cotizacion(){
    	$filter = new stdClass();
    	$filter->STATE_Codigo  = $this->input->post('STATE_Codigo');
    	$filter->PRESUP_Codigo =$this->input->post('PRESUP_Codigo');
    	$filter->COMPA_Codigo  =$this->input->post('COMPA_Codigo');
    	$filter->SEGUIPRE_Consultar  =$this->input->post('consultar');
    	$filter->SEGUIPRE_Tipo  =$this->input->post('tipo');
    	$filter->REPORFIN_Codigo  =$this->input->post('reportefinal');
    	$filter->SEGUIPRE_FechaSistema =human_to_mysql($this->input->post('SEGUIPRE_FechaSistema'));
    	$filter->SEGUIPRE_Comentario =$this->input->post('SEGUIPRE_Comentario');
    	$filter->SEGUIPRE_FechaRegistro = date("Y-m-d");
    	$filter->SEGUIPRE_Flag = '1';
    	$this->presupuesto_model->gaurdar_seguimiento_cotizacion($filter);
    }
    
    
    public function mostrar_seguimiento_table($codPresupusto){
    	$resultado="";
    	$listar=$this->presupuesto_model->mostrar_seguimiento_table($codPresupusto);
    	echo json_encode($listar);
    }
    
    public function eliminardetalleseguimi(){
    	$codigo= $this->input->post('codigo');
    	$this->presupuesto_model->eliminar_seguimiento($codigo);
    }
