...
class Presupuesto extends Controller {
	...
	public function presupuestos($j = '0', $limpia = '') {
		...
		$data['titulo_tabla'] = "RELACI�N DE COTIZACIONES";
	...
	public function obtener_lista_detalles($codigo) {
		...
		if (count($detalle) > 0) {
			foreach ($detalle as $indice => $valor) {
				...
				$nombre_unidadsimbolo = $datos_unidad[0]->UNDMED_Simbolo;
				$objeto = new stdClass();
				...
				$objeto->UNDMED_Simbolo	 = $nombre_unidadsimbolo;
...
// 		$nombre_contacto = $nombre_cliente;
		...
		if (count($data_tipocambio) > 0)
			...
			$xls = utf8_decode_seguro('<b>Cotizaci�n: ') . $codificacion . '</b>';


// 			$xls .= "<table>
// 		<tr><td>" . utf8_decode_seguro('Se�or(es) :') . " </td><td>$nombre_cliente</td><td></td><td>R.U.C.</td><td>$ruc</td><td>Fecha : </td><td>$fecha</td></tr>
// 		<tr><td>" . utf8_decode_seguro('Direcci�n :') . " </td><td>$direccion</td><td></td><td></td><td></td><td></td></tr>
// 		<tr><td>" . utf8_decode_seguro('Atenci�n Sr(a) :') . " </td><td>$nombre_contacto " . ($nombre_area != '' ? ' - AREA: ' . $nombre_area : '') . "</td><td></td>&nbsp;&nbsp;&nbsp;<td></td><td></td><td></td><td></td><td></td></tr>
// 		<tr><td>" . utf8_decode_seguro('Tel�fono : ') . "</td><td>$telefono</td><td></td><td></td><td></td><td>E-mail :</td><td>$email</td></tr>
// 		</table><br><br>
// 		";
...
// 			$date = date('Y-m-d') . '-' . $ruc . '-Presupuesto';
			...
			$xls .= "
		<table border=1>
			<tr><th>Item</th><th>Marca</th><th>" . utf8_decode_seguro('Descripci�n') . "</th><th>Uni.</th><th>Cant.</th><th>Precio Uni.</th><th>Precio Total</th></tr>
		";
		...
		";
			...
			<tr><td>" . utf8_decode_seguro('Tipo de Cambio del D�a :') . "</td><td>" . ($tipo_cambio > 0 ? round($tipo_cambio, 2) : '' ) . "</td></tr>
			...
			<tr><td>" . utf8_decode_seguro('Cta. Cte. en D�lares') . "</td><td>" . utf8_decode_seguro('N� 191-1466829-1-62') . "</td></tr>
	...
	$vendedor_nombre = $datos_persona[0]->PERSC_Nombre . ' ' . $datos_persona[0]->PERSC_ApellidoPaterno . ' ' . $datos_persona[0]->PERSC_ApellidoMaterno;
		...
		if (count($data_tipocambio) > 0)
			...
			$this->cezpdf->ezText(utf8_decode_seguro('<b>Cotizaci�n de Compra: ') . $codificacion . '</b>', 17, array("leading" => 40, 'left' => 185));
			...
			foreach ($detalle_presupuesto as $indice => $valor) {
				$db_data[] = array(
						...
						'cols4' => $valor->UNDMED_Descripcion,
			...
			$col_names = array(
					'cols1' => 'Item',
					'cols2' => 'Marca',
					'cols3' => utf8_decode_seguro('Descripci�n'),
					'cols4' => 'Uni.',
					'cols5' => 'Cant.',
					'cols6' => 'Precio Uni.',
					'cols7' => 'Precio Total'
	...
	$vendedor_nombre = $datos_persona[0]->PERSC_Nombre . ' ' . $datos_persona[0]->PERSC_ApellidoPaterno . ' ' . $datos_persona[0]->PERSC_ApellidoMaterno;
			...
			$db_data = array(array('cols0' => '<b>CONDICIONES DE VENTA:</b>', 'cols1' => ''),
					...
					array('cols0' => utf8_decode_seguro('Cta. Cte. en D�lares'), 'cols1' => ': ' . utf8_decode_seguro('N� 191-1466829-1-62')),
...
$cabecera = array('Content-Type' => 'application/pdf', 'Content-Disposition' => $codificacion . '.pdf', 'Expires' => '0', 'Pragma' => 'cache', 'Cache-Control' => 'private');
	...
	public function presupuesto_ver_pdf_formato2($codigo) {
		...
		if (count($data_tipocambio) > 0)
			...
			$this->cezpdf->ezText('', '', array("leading" => 100));
			$this->cezpdf->ezText(utf8_decode_seguro('<b>Cotizaci�n: ') . $codificacion . '</b>', 17, array("leading" => 40, 'left' => 185));
			$this->cezpdf->ezText('', '', array("leading" => 10));


			/* Datos del cliente */
			$db_data = array(array('cols1' => utf8_decode_seguro('Se�or(es)'), 'cols2' => ': ' . utf8_decode_seguro($nombre_cliente), 'cols3' => 'R.U.C.', 'cols4' => ': ' . $ruc . '       Fecha: ' . $fecha),
					array('cols1' => utf8_decode_seguro('Direcci�n'), 'cols2' => ': ' . utf8_decode_seguro($direccion), 'cols3' => '', 'cols4' => ''),
					array('cols1' => utf8_decode_seguro('Atenci�n Sr(a)'), 'cols2' => ': ' . utf8_decode_seguro($nombre_contacto . ($nombre_area != '' ? ' - AREA: ' . $nombre_area : '')), 'cols3' => '', 'cols4' => ''),
					array('cols1' => utf8_decode_seguro('Tel�fono'), 'cols2' => ': ' . $telefono, 'cols3' => 'E-mail', 'cols4' => ': ' . $email)
			);
			$this->cezpdf->ezTable($db_data, "", "", array(
					'width' => 525,
					'showLines' => 0,
	...
	public function presupuesto_ver_pdf_formato4($codigo) {
		...
		if (count($data_tipocambio) > 0)
			...
			$db_data = array(array('cols0' => '<b>CONDICIONES DE VENTA:</b>', 'cols1' => ''),
					...
					array('cols0' => 'Tiempo de Entrega', 'cols1' => ': ' . $tiempo_entrega),
					array('cols0' => 'Lugar de Entrega', 'cols1' => ': ' . utf8_decode_seguro($lugar_entrega)),
					array('cols0' => utf8_decode_seguro('Garant�a'), 'cols1' => ': ' . utf8_decode_seguro($garantia)),
					array('cols0' => 'Validez de la Oferta', 'cols1' => ': ' . utf8_decode_seguro($validez)),
					array('cols0' => 'Contacto', 'cols1' => ': ' . utf8_decode_seguro($vendedor_nombre . ($vendedor_nombre_area != '' ? ' - AREA: ' . $vendedor_nombre_area : '' )))
			);
			$this->cezpdf->ezTable($db_data, "", "", array(
	...
	public function presupuesto_ver_pdf_conmenbrete_formato3($codigo) {
		...
		$this->cezpdf_horizontal->ezText(utf8_decode_seguro('Lima: Av. Chorrillos N� 200 Chorrillos - Lima  Telf.: 251 6727  Fax: 252 7547'), 8, array("left" => 15));
		...
		$this->cezpdf_horizontal->ezText(utf8_decode_seguro('<b>Cotizaci�n Nro.  ') . ($codigo_usuario != '' ? $codigo_usuario : $numero) . '</b>', 17, array("leading" => 30, "left" => 300));
		$this->cezpdf_horizontal->ezText('', '');


		/* Datos del cliente */
		$db_data = array(array('cols1' => utf8_decode_seguro('Se�or(es)'), 'cols2' => ': ' . utf8_decode_seguro($nombre_cliente), 'cols3' => 'R.U.C.', 'cols4' => ': ' . $ruc, 'cols5' => 'Fecha', 'cols6' => ': ' . $fecha, 'cols7' => ''),
				array('cols1' => utf8_decode_seguro('Direcci�n'), 'cols2' => ': ' . $direccion, 'cols3' => '', 'cols4' => '', 'cols5' => '', 'cols6' => '', 'cols7' => ''),
				...
				array('cols1' => utf8_decode_seguro('Tel�fono'), 'cols2' => ': ' . $telefono, 'cols3' => 'E-mail', 'cols4' => ': ' . $email, 'cols5' => '', 'cols6' => '', 'cols7' => '')
		);
		...
		$col_names = array(
				'cols1' => 'Item',
				'cols2' => utf8_decode_seguro('C�digo'),
				'cols3' => 'Marca',
				'cols4' => 'Modelo',
				'cols5' => utf8_decode_seguro('Descripci�n'),
				'cols6' => 'Uni.',
				'cols7' => 'Cant.',
				'cols8' => 'Precio Uni.',
				'cols9' => 'Precio Total'
		...
		$db_data = array(array('cols0' => '<b>Condiciones de venta:</b>', 'cols1' => '', 'cols2' => ''),
				...
				array('cols0' => utf8_decode_seguro('Banco de Cr�dito Soles'), 'cols1' => ':', 'cols2' => 'Cta. Cte. No. 285-1178292-0-25'),
				array('cols0' => utf8_decode_seguro('Banco de Cr�dito D�lares'), 'cols1' => ':', 'cols2' => 'Cta. Cte. No. 285-1202278-1-18'),
				...
				array('cols0' => utf8_decode_seguro('Garant�a'), 'cols1' => ':', 'cols2' => utf8_decode_seguro($garantia)),
