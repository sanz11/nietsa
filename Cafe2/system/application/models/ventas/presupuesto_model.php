    ...
    public function buscar_presupuestos($filter, $number_items = '', $offset = '') {
        ...
        $sql = "SELECT p.PRESUC_Fecha,
                ...
                LEFT JOIN cji_empresa e ON e.EMPRP_Codigo=c.EMPRP_Codigo AND c.CLIC_TipoPersona='1'
        		 left join cji_seguimiento_presupuesto sepre on sepre.PRESUP_Codigo = p.PRESUP_Codigo
...
//       stv         }
    ...
    public function  buscarseguimiento_presupuesto($filter){
    	$sql = "SELECT * FROM cji_seguimiento_presupuesto WHERE PRESUP_Codigo = $filter";
    	$query = $this->db->query($sql);
    	if ($query->num_rows > 0) {
    		foreach ($query->result() as $fila) {
    			$data[] = $fila;
    ...
    public function  listaseguipresu(){//Sube esto
    	$sql = "SELECT * FROM cji_seguimiento_presupuesto where SEGUIPRE_Flag = 1";
    	$query = $this->db->query($sql);
    	if ($query->num_rows > 0) {
    		foreach ($query->result() as $fila) {
    			$data[] = $fila;
    ...
    public function mostrar_seguimiento_table($cod){
    	$this->db->select('SEGUIPRE_Codigo,se.STATE_Codigo,STATE_Estado,DOCUP_Codigo,STATE_Color,SEGUIPRE_FechaSistema,SEGUIPRE_Comentario');
    	$this->db->join('cji_estate es','es.STATE_Codigo=se.STATE_Codigo');
    
    	$this->db->where('PRESUP_Codigo',$cod);
    	$this->db->where('SEGUIPRE_Flag','1');
    	$query = $this->db->get('cji_seguimiento_presupuesto se');
    	if ($query->num_rows > 0) {
    		foreach ($query->result() as $fila) {
    			$data[] = $fila;
    		}
    ...
    public function gaurdar_seguimiento_cotizacion($filter=null){
    	$this->db->insert("cji_seguimiento_presupuesto",(array)$filter);
    }
    public function eliminar_seguimiento($codigo) {
    	$data  = array("SEGUIPRE_Flag"=>'0');
    	$where = array("SEGUIPRE_Codigo"=>$codigo);
    	$this->db->where($where);
    	$this->db->update('cji_seguimiento_presupuesto',$data);
    }
}
