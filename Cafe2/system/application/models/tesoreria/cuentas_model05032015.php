...
//            $where .= ' and cp.CPC_Fecha BETWEEN "' . human_to_mysql($filter->fechai) . '" AND "' . human_to_mysql($filter->fechaf) . '"';
    ...
    public function buscar_notas_credito($tipo_cuenta, $codigo) {
        $this->db->select('CRED_Codigo,CRED_Serie,CRED_Numero')->from('cji_credito')->where('CLIP_Codigo', $codigo);
        $query = $this->db->get();
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
    ...
    public function buscar($tipo_cuenta, $codigo, $estado = array('V', 'A'), $f_ini = '', $f_fin = '') {
        $compania = $this->somevar['compania'];
        $where = array("cji_cuentas.CUE_TipoCuenta" => $tipo_cuenta, 'cji_cuentas.COMPP_Codigo' => $this->somevar['compania']);
        if ($tipo_cuenta == '1')
        ...
        if (isset($filter->codigo) && $filter->FORPAC_Descripcion != '')
         ...
         $this->db->order_by('cji_cuentas.CUE_FechaOper', 'asc')
