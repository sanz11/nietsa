...
('', 15, NULL, 0, NOW(), ".$codcompania.", '1');";
    ...
    public function modificar_datosEmpresaCodificado($empresa, $tipocodigo, $ruc, $razon_social, $telefono, $movil, $web, $email, $direccion){
    		$data = array(
    				"TIPCOD_Codigo"     => $tipocodigo,
    				"EMPRC_Ruc"         => $ruc,
    				"EMPRC_RazonSocial" => strtoupper($razon_social),
...
//     		$query = $this->db->where('EMPRC_Ruc', $ordAdj)->get('cji_empresa'); 
...
/*public function listCuentaEmpresa($codigo,$filter=null, $number_items='',$offset=''){
  $this->db->select('c.CUENT_Codigo,c.CUENT_NumeroEmpresa,c.CUENT_Titular,
                      c.CUENT_TipoPersona,c.CUENT_FechaRegistro,b.BANC_Nombre
                     ,m.MONED_Descripcion,c.CUENT_TipoCuenta');
   // $this->db->from('cji_cuentasempresas c ','INNER');
    ...
    $this->db->where('CUENT_FlagEstado',"1");
    $this->db->order_by('CUENT_FechaRegistro','ASC');
    $this->db->order_by('CUENT_FechaModificacion','ASC');
    $query= $this->db->get('cji_cuentasempresas c ', $number_items,$offset);
    if($query->num_rows>0){
            foreach($query->result() as $fila){
                $data[] = $fila;
...
public function listCuentaEmpresaCodigo($codigo){
 $this->db->select('c.CUENT_Codigo,c.CUENT_NumeroEmpresa,c.CUENT_Titular,
                      c.CUENT_TipoPersona,c.CUENT_FechaRegistro,b.BANC_Nombre
                     ,m.MONED_Descripcion,c.CUENT_TipoCuenta,b.BANP_Codigo,m.MONED_Codigo,c.CUENT_Oficina,c.CUENT_Sectoriza,c.CUENT_Interbancaria');
    $this->db->from('cji_cuentasempresas c ','INNER');
    $this->db->join('cji_banco b','b.BANP_Codigo=c.BANP_Codigo');
    $this->db->join('cji_moneda m','m.MONED_Codigo=c.MONED_Codigo');
...
public function listChikera($codigo){
 $this->db->select('ck.CHEK_Codigo,ce.CUENT_Codigo ,ce.CUENT_NumeroEmpresa 
                    ,ck.SERIP_Codigo,ck.CHEK_Numero,ck.CHEK_FechaRegistro');
