...
//}
    ...
    public function obtener_unidad($producto)
    {
    	$sql = "select UNDMED_Descripcion from cji_productounidad prouni
					inner join cji_unidadmedida unidad on unidad.UNDMED_Codigo = prouni.UNDMED_Codigo
...
//Modificar proveedor
    ...
    public function obtener_img($filter){
    	$sql="select * from cji_producto where PROD_Codigo = $filter";
    	$query = $this->db->query($sql);
    	if ($query->num_rows > 0) {
    		foreach ($query->result() as $fila) {
    			$data[] = $fila;
