...
//PRESUPUESTO EXCEL VENTA COTIZACION
    ...
    public function presupuesto_ver_pdf_conmenbrete_formato1($codigo, $img,$codigotipopresu) {
        ...
        $this->cezpdf = new backgroundPDF('a4', 'portrait', 'image', array('img' => 'images/cabeceras/fondo.png'));
         
        $this->cezpdf->addInfo($datacreator);
        if ($_SESSION['empresa'] == '3') {
        	$this->cezpdf->ezImage("images/cabeceras/presupuestoyuan.jpg", -10, 0, 'none', 'left');
        } else {
        	$this->cezpdf->ezImage("images/cabeceras/logocafe.jpg", -10, 100, 'none', 'left');
        ...
        if($codigotipopresu == 2){ //TIPO DE PRESUPUESTO 1:SERVICIO  ; 2:PRODUCTO
...
//         	$pdf->SetSubject('Tutorial TCPDF');
        	...
        	$pdf->Image(base_url().'images/cabeceras/logocafe.jpg',$x+5,$y+5, 0, 0, '', '', '', true, 90, '', false, false, 0, false, false, false);
        	
        	$html = '<h1 align="center">Presupuesto :'.$codificacion.'</h1>';
        	$pdf->writeHTML($html, true, false, true, false, '');
        	
        		
        	$tbl_header1='<table border="0" width="100%" >';
        	$tabla1.='<tr>';
        	$tabla1.='<td style="width:20%"><b>Senior(es)</b></td>';
        	$tabla1.='<td style="width:2%">:</td>';
        	$tabla1.='<td style="width:30% ;">'.utf8_decode_seguro($nombre_cliente).'</td>';
        	$tabla1.='<td><b>Ruc</b></td>';
        	$tabla1.='<td style="width:2%">:</td>';
        	$tabla1.='<td style="width:30% ;" >'.$ruc.'</td>';
        	$tabla1.='</tr>';
        	$tabla1.='<tr>';
        	$tabla1.='<td><b>Fechas</b></td>';
        	$tabla1.='<td>:</td>';
        	$tabla1.='<td style="font-size:10px;" >'.$fecha.'</td>';
        	$tabla1.='<td><b>Direccion</b></td>';
        	$tabla1.='<td>:</td>';
        	$tabla1.='<td style="font-size:10px;" >'.utf8_decode_seguro($direccion).'</td>';
        	$tabla1.='</tr>';
        	$tabla1.='<tr>';
        	$tabla1.='<td><b>Vend.</b></td>';
        	$tabla1.='<td>:</td>';
        	$tabla1.='<td style="font-size:10px;" >'.$vendedor_nombre.'</td>';
        	$tabla1.='</tr>';
        	$tabla1.='<tr>';
        	$tabla1.='<td><b>Atencion Sr(a)</b></td>';
        	$tabla1.='<td>:</td>';
        	$tabla1.='<td>'.utf8_decode_seguro($nombre_contacto . ($nombre_area != '' ? ' - AREA: ' . $nombre_area : '')).'</td>';
        	$tabla1.='</tr>';
        	$tabla1.='<tr>';
        	$tabla1.='<td><b>Telefonso</b></td>';
        	$tabla1.='<td>:</td>';
        	$tabla1.='<td  >'.$telefono.'</td>';
        	$tabla1.='<td><b>E-mail</b></td>';
        	$tabla1.='<td>:</td>';
        	$tabla1.='<td>'.$email.'</td>';
        	$tabla1.='</tr>';
        	$tbl_footer1='</table>';
         	...
         	/************ Segunda Tabla ***********/
           	...
           	$tbl_header2='<table border="1">';
         	$tabla2.='<tr>';
         	$tabla2.='<td style="width:5%" >Item</td>';
         	$tabla2.='<td style="width:17%">Imagen</td>';
         	$tabla2.='<td style="width:40%">Descripcion</td>';
         	$tabla2.='<td style="width:10%">Cantidad</td>';
         	$tabla2.='<td>Valor venta</td>';
         	$tabla2.='<td>Importe Total</td>';
         	$tabla2.='</tr>';
         	foreach ($detalle_presupuesto as $indice => $valor) {
         		$producto = $valor->PROD_Codigo;
         		$marca_prod = $this->producto_model->obtener_marca_modelo_por_producto($producto);
         		...
         		foreach ($obtenerimgprod as $indicando => $proce){
		         	...
		         	$tabla2.='<tr>';
					$tabla2.='<td>'.($indice + 1).'</td>';
					$tabla2.='<td><img align="center" border="1" style="width:100px; height:100px" src="images/img_db/'.$img.'"></td>';						
		         	$tabla2.='<td>'.utf8_decode_seguro($valor->PROD_Nombre . ($valor->PROD_Modelo != '' ? ' - ' . $valor->PROD_Modelo : '')).'</td>';
		         	$tabla2.='<td>'.$valor->PRESDEC_Cantidad.'</td>';
		         	$tabla2.='<td>'. number_format(($modo_impresion == '1' || $tipo_docu == 'B' ? $valor->PRESDEC_Pu_ConIgv : $valor->PRESDEC_Pu), 2).'</td>';
		         	$tabla2.='<td>'.number_format($valor->PRESDEC_Cantidad * ($modo_impresion == '1' || $tipo_docu == 'B' ? $valor->PRESDEC_Pu_ConIgv : $valor->PRESDEC_Pu), 2).'</td>';
		         	$tabla2.='</tr>';
         	}
         	$tbl_footer2='</table>';
         	//style="width:21%"
	         	$pdf->writeHTMLCell(0,0,$x+10,$y+90,$tbl_header2 . $tabla2 . $tbl_footer2,0,1,false,true,'L',true);
	         	...
	         	$tabla3.='</tr>';
	         	$tabla3.='<tr>';
	         	$tabla3.='<td style="width:10%" >Impuesto</td>';
	         	...
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:100%" >Condicion De Venta</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Tipo de Canbio del Dia</td>';
	         	$tabla4.='<td style="width:50%">: '. ($tipo_cambio > 0 ? round($tipo_cambio, 2) : '').'</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Moneda</td>';
	         	$tabla4.='<td style="width:50%">: '. $moneda_nombre .'</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	...
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Los Precios de los Productos</td>';
	         	$tabla4.='<td style="width:50%">: '. ($modo_impresion == '1' ? 'CONTIENEN IGV' : 'NO CONTIENEN IGV') .'</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	...
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Garantia</td>';
	         	$tabla4.='<td style="width:50%">: '. $garantia .'</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Validez de la Oferta</td>';
	         	$tabla4.='<td style="width:50%">: '. utf8_decode_seguro($validez) .'</td>';
	         	$tabla4.='</tr>';
	         	$tabla4.='<tr>';
	         	$tabla4.='<td style="width:30%" >Contacto</td>';
	         	$tabla4.='<td style="width:50%">: '. utf8_decode_seguro($vendedor_nombre . ($vendedor_nombre_area != '' ? ' - AREA: ' . $vendedor_nombre_area : '')) .'</td>';
	         	$tabla4.='</tr>';
	         	$tbl_footer4='</table>';
	         	//style="width:21%"
	         	$pdf->writeHTMLCell(0,0,$x+10,$y+210,$tbl_header4 . $tabla4 . $tbl_footer3,0,1,false,true,'L',true);
