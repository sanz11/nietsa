...
class Directivo extends Controller {
    ...
    public function directivos($j = 0) {
        ...
        if (count($listado_directivos) > 0) {
            foreach ($listado_directivos as $indice => $valor) {
                ...
                $lista[] = array($item, $numdoc, $nombres, $empresa, $cargo, $contrato, $inicio, $fin, $editar, $ver, $eliminar);
                $item++;
            }
        }
        $data['lista'] = $lista;
        ...
        if (!$this->upload->do_upload('foto')) {
            ...
            if ($tipo_persona == 0) {//Persona                
                ...
                if ($personacod != '' && $personacod != '0') {
                    $persona = $personacod;
                    $this->persona_model->modificar_datosPersona($persona, $ubigeo_nacimiento, $ubigeo_domicilio, $estado_civil, $nacionalidad, $nombres, $paterno, $materno, $ruc_persona, $tipo_documento, $numero_documento, $direccion, $telefono, $movil, $email, $domicilio, $sexo, $fax, $web, $fechanac);
    ...
    public function editar_directivo($id) {
        ...
        if ($tipo_persona == 0) {//Persona
            ...
            $data['fechanac'] = $datos_persona[0]->PERSC_FechaNac;
    ...
    public function modificar_directivo() {
        ...
        if ($tipo_persona == 0) {//Persona
            if ($personacod != '' && $personacod != '0') {
                $persona = $personacod;
                $this->persona_model->modificar_datosPersona($persona, $ubigeo_nacimiento, $ubigeo_domicilio, $estado_civil, $nacionalidad, $nombres, $paterno, $materno, $ruc_persona, $tipo_documento, $numero_documento, $direccion, $telefono, $movil, $email, $direccion, $sexo, $fax, $web);
            } else {
