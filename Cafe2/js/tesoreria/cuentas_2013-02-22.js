...
function mostrar_cuentas(aplica_pago){
    ...
    url_notascredito= base_url+"index.php/tesoreria/cuentas/JSON_notas_credito_pendientes/"+tipo_cuenta+"/"+codigo;
    var select="<option value=''>::Seleccione::</option>";
    $.getJSON(url_notascredito,function(data){
        $.each(data, function(i,item){
            
            select+="<option value='"+item.nc_codigo+"'>NC -> "+item.ser_num+"</option>";
        });
        $('#notaCredito').html(select)
    });  
