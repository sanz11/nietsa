...
//                }
 ...
 $data['registros'] = count($this->comprobante_model->busqueda_comprobante($tipo_oper, $tipo_docu, $filter)); //count($this->comprobante_model->buscar_comprobantes($tipo_oper, $tipo_docu, $filter, NULL, '', ''));
        ...
        if (count($listado_comprobantes) > 0) {
            foreach ($listado_comprobantes as $indice => $valor) {
                ...
                if ($this->somevar['rol'] == '4' && $estado == '2') {
    ...
    public function comprobantes($tipo_oper = '', $tipo_docu = '', $j = '0', $limpia = ''){
        ...
        $data['registros'] = count($this->comprobante_model->busqueda_comprobante($tipo_oper, $tipo_docu, $filter));
        
        //count($this->comprobante_model->buscar_comprobantes($tipo_oper, $tipo_docu, $filter, NULL, '', ''));
        $conf['base_url'] = site_url('ventas/comprobante/comprobantes/' . $tipo_oper . '/' . $tipo_docu);
        $conf['per_page'] = 50;
        $conf['num_links'] = 3;
        $conf['first_link'] = "&lt;&lt;";
        ...
        $conf['uri_segment'] = 6;
        $offset = (int) $this->uri->segment(6);
      $listado_comprobantes = $this->comprobante_model->buscar_comprobantes($tipo_oper, $tipo_docu, $filter, $conf['per_page'], $offset, date('Y-m-d'));
        ...
        if (count($listado_comprobantes) > 0) {
            foreach ($listado_comprobantes as $indice => $valor) {
                ...
                $numero = $valor->CPC_Numero;
                $numero_ref=$serie.'-'.$numero;
                $OCOMP_Codigo=$valor->OCOMP_Codigo;
                $usu=$valor->USUA_Codigo;
                //guia de remision incluido en factura
                
    $listar_Orden=$this->guiarem_model->getNumeroSerieOrderVenta($OCOMP_Codigo);
            ...
            if ($valor->CPC_DocuRefeCodigo !='' ) {
                ...
                if (count($list_guirem) > 0) {
                    ...
                    $guiarem_codigo = '';
                    $docurefe_codigo = '';
                    //$orden_compra=$OCOMP_Codigo;
                    }
                
                }
                
              if(count($listar_Orden)>0)  {

                $orden_compra="<a >" . $listar_Orden[0]->OCOMC_Serie." - ".$listar_Orden[0]->OCOMC_Numero ."</a>";

            }else{ $orden_compra="";}
                //$guiarem_codigo = $valor->CPC_GuiaRemCodigo;
                
$modifCuentaDepen=$codigo;//'<a href="javascript:;" onclick="modifcarDependi('.$codigo .');">MODEFICA</a>';    
 ...
 if($usuarioNom[0]->ROL_Codigo==0){
    ...
    $nomusuario= strtolower($explorar[0]);
                ...
                $ver2 = "<a href='javascript:;' onclick='comprobante_ver_pdf_conmenbrete(" . $codigo . ")' target='_parent'><img src='" . base_url() . "images/pdf.png' width='16' height='16' border='0' title='Ver PDF'></a>";
                if($this->session->userdata('rol')==4 && $estado == '2'){
                $eliminar = "<a href='javascript:;' onclick='eliminar_comprobante_general(" . $codigo . ")' target='_parent'><img src='" . base_url() . "images/eliminar.png' width='16' height='16' border='0' title='Eliminar'></a>";
                }else{
                    $eliminar ="";
                }
                if ($estado == '2')
        ...
        $lista[] = array($item++, $fecha, $serie, $numero, $guiarem_codigo, $docurefe_codigo, $nombre, $total, $img_estado, $editar, $ver, $ver2, $disparador, $estado, $codigo, $codigo_canje,$eliminar,$orden_compra,$modifCuentaDepen,$nomusuario);}//EL ULTIMO ES N° 19
                else{ 
                    $lista[] = array($item++, $fecha, $serie, $numero, $guiarem_codigo, $docurefe_codigo, $nombre, $total, $img_estado, $editar, $ver, $ver2, $disparador, $estado, $codigo, $codigo_canje,$orden_compra,$modifCuentaDepen,$nomusuario);
                }
            
            }
        }
        $data['titulo_tabla'] = "RELACION DE " . strtoupper($this->obtener_tipo_documento($tipo_docu)) . "S";
...
/*
        ...
        $data['registros'] = $this->comprobante_model->contar_comprobantes($tipo_oper, $tipo_docu, NULL, '', '');
        $conf['base_url'] = site_url('ventas/comprobante/comprobantes/' . $tipo_oper . '/' . $tipo_docu);
        $conf['per_page'] = 15;
        $conf['num_links'] = 3;
        $conf['first_link'] = "&lt;&lt;";
    ...
    public function comprobante_nueva($tipo_oper = '', $tipo_docu = '')
    {
        ...
        if($tipoDocumento == '')
        {
           ...
           $data['cboVendedor'] = $this->OPTION_generador($this->directivo_model->listar_directivo($this->session->userdata('empresa'), '4'), 'DIREP_Codigo', array('PERSC_ApellidoPaterno', 'PERSC_ApellidoMaterno', 'PERSC_Nombre'), '', array('', '::Seleccione::'), ' ');
            $data['cboVendedorCod'] = $this->OPTION_generador($this->directivo_model->listar_directivo($this->session->userdata('empresa'), '4'), 'PERSP_Codigo', array('PERSC_ApellidoPaterno', 'PERSC_ApellidoMaterno', 'PERSC_Nombre'), '', array('', '::Seleccione::'), ' ');
...
//VERIFICO SI TODAS LAS SERIES HAN SIDO INGRESADAS
        ...
        if ($this->input->post('txtVendero') != '' && $this->input->post('txtVendero') != '0') {

            $filter->CPC_VendedorPersona = $this->input->post('txtVendero');
...
//            }
        ...
        $cant_total = 0;
        $detalle = $this->ocompra_model->obtener_detalle_ocompra($this->input->post('ordencompra'));
        ...
        if (is_array($prodcodigo)) {
            ...
            foreach ($prodcodigo as $indice => $valor) {
                ...
                $cantidad_entregada = calcular_cantidad_entregada_x_producto($tipo_oper, $tipo_oper, $this->input->post('ordencompra'), $filter->PROD_Codigo);
...
public function ingresarConfiguracio($codigo){
   $datos_comprobante = $this->comprobante_model->optenerComprbanteNumero($codigo);
  $numero="";
  $serie="";
  $codigs="";
  $tipo_oper ="";
  ...
  if (count($datos_comprobante)>0) {
    foreach ($datos_comprobante as $key => $value) {
      ...
      $codigs=$value->CPP_Codigo;
      $tipo_oper=$value->CPC_TipoOperacion;
      $tipo_docu=$value->CPC_TipoDocumento;
    }
   }
    if ($tipo_docu == 'F') {
                $tipo = 8;
            ...
            if (count($detalle) > 0) {
                foreach ($detalle as $indice => $valor) {
                    ...
                    if ($tipo_oper == 'V') {
                        ...
                        $filter4->GUIASADETC_Costo = $pu_conigv;        //$subtotal_conigv;
                        ...
                        $filter4->GUIIAINDETC_GenInd = $GenInd;     //$subtotal_conigv;
                        $filter4->GUIAINDETC_Costo = $pu_conigv;    //$costo;   // No estoy muy seguro de si debe agarrar este precio, porque puede ser $costo, $venta
...
//VERIFICO SI TODAS LAS SERIES HAN SIDO INGRESADAS
        ...
        $filter->CPC_descuento100 = $this->input->post('descuento');
        $filter->CPC_igv100 = $this->input->post('igv');
        $filter->CPC_FlagEstado = 2;//$this->input->post('estado');
        if ($tipo_oper == 'V')
            $filter->CLIP_Codigo = $this->input->post('cliente');
        else
            $filter->PROVP_Codigo = $this->input->post('proveedor');
        ...
        if ($cliente != '' && $cliente != '0') {
          ...
          if ($guiarem_codigo) {
                   ...
                   // $guiasap_codigo = $list_guiare[0]->GUIASAP_Codigo;
                    ...
                    $codigo_ocompra = $list_guiare[0]->OCOMP_Codigo;
                    $serie_numero_ocompra = $list_guiare[0]->GUIAREMC_OCompra;
                    
                }
                if ($codigo_ocompra) {
                    $list_orden_venta=$this->guiarem_model->getNumeroSerieOrderVenta($codigo_ocompra);
                }elseif ($ordencompra) {
                   $list_orden_venta=$this->guiarem_model->getNumeroSerieOrderVenta($ordencompra);
                }
//
if(count($list_orden_venta)>0){
    foreach ($list_orden_venta as $key => $value) {
    $serie_numero_orden=$value->OCOMC_Serie. "-".$value->OCOMC_Numero;

    }
}
$data["serie_numero_orden"]=$serie_numero_orden;
//Para cambio comprobante_A
        $data['cambio_comp'] = "0";
        $data['total_det'] = "0";
//---------------------------------------
    ...
    public function obtener_lista_detalles($codigo)
    {
        ...
        if (count($detalle) > 0) {
            foreach ($detalle as $indice => $valor) {
                ...
                $datos_producto = $this->producto_model->obtener_producto($producto);
                $flagBS = $datos_producto[0]->PROD_FlagBienServicio;
                $marca = $this->producto_model->obtener_marca_modelo_por_producto($producto);
                $nombre_marca = $marca[0]->MARCC_Descripcion;
                $datos_unidad = $this->unidadmedida_model->obtener($unidad);
                $GenInd = $valor->CPDEC_GenInd;
                $costo = $valor->CPDEC_Costo;
                $nombre_producto = ($valor->CPDEC_Descripcion != '' ? $valor->CPDEC_Descripcion : $datos_producto[0]->PROD_Nombre);
                //$nombre_marca = ($valor->CPDEC_Descripcion != '' ? $valor->CPDEC_Descripcion : $datos_producto[0]->MARCP_Codigo);
                //$nombre_marca = ($valor->CPDEC_Descripcion != '' ? $valor->CPDEC_Descripcion : $datos_producto[0]->MARCC_Descripcion);
                $nombre_producto = str_replace('\\', '', $nombre_producto);
                $codigo_interno = $datos_producto[0]->PROD_CodigoUsuario;
                $codigo_usuario = $datos_producto[0]->PROD_CodigoUsuario;
                $nombre_unidad = is_array($datos_unidad) ? $datos_unidad[0]->UNDMED_Descripcion : 'SERV';
 ...
 $this->comprobante_ver_pdf_conmenbrete_formato1_boleta1($tipo_oper, $codigo, $tipo_docu, $img);
  ...
  // $this->comprobante_ver_pdf_conmenbrete_formato1_com($codigo, $img);
    ...
    public function comprobante_ver_pdf_conmenbrete_formato1_com($codigo, $img){
        $datos_comprobante = $this->comprobante_model->obtener_comprobante($codigo);
        $tipoOpera=$datos_comprobante[0]->CPC_TipoOperacion;
        $presupuesto = $datos_comprobante[0]->PRESUP_Codigo;
        $serie = $datos_comprobante[0]->CPC_Serie;
        $numero = $datos_comprobante[0]->CPC_Numero;
        $proveedor = $datos_comprobante[0]->CLIP_Codigo;
        $proveedor2 = $datos_comprobante[0]->PROVP_Codigo;
        $subtotal = $datos_comprobante[0]->CPC_subtotal;
        $descuento = $datos_comprobante[0]->CPC_descuento;
        $igv = $datos_comprobante[0]->CPC_igv;
        $igv100 = $datos_comprobante[0]->CPC_igv100;
        ...
        if ($tipo == 0) {
            ...
            $datos_empresa = $this->empresa_model->obtener_datosEmpresa($proveedor2);
            $nombre_cliente = $datos_empresa[0]->EMPRC_RazonSocial;
            $ruc = $datos_empresa[0]->EMPRC_Ruc;
            $telefono = $datos_empresa[0]->EMPRC_Telefono;
            $movil = $datos_empresa[0]->EMPRC_Movil;
        ...
        if ($tipoOpera=="V") {
             if ($img == 0) {
            $notimg = "nota_compra.jpg"; //madyplac_com.jpg
            $this->cezpdf = new backgroundPDF('a4', 'portrait', 'image', array('img' => 'images/img_db/' . $notimg));
            
            $this->cezpdf->addText(340, 750, 14, utf8_decode_seguro($serie.'   '));
            $this->cezpdf->addText(420, 750, 14, $numero);    
        
        } else {
            $notimg = "";//nota_compra.jpg";
            $this->cezpdf = new backgroundPDF('a4', 'portrait', 'image', array('img' => 'images/img_db/' . $notimg));
             $this->cezpdf->addText(340, 750, 14, utf8_decode_seguro($serie.'   '));
            $this->cezpdf->addText(420, 750, 14, $numero); 
               ...
               if ($img == 0) {
            $notimg = "nota_credito_proveedor.jpg"; //madyplac_com.jpg
            $this->cezpdf = new backgroundPDF('a4', 'portrait', 'image', array('img' => 'images/img_db/' . $notimg));
            
            $this->cezpdf->addText(420, 740, 14, utf8_decode_seguro($serie.'   '));
            $this->cezpdf->addText(480, 740, 14, $numero); 
...
$this->cezpdf->addText(280,  ($postY*50)-89, 10, utf8_decode_seguro(substr($fecha, 0, 2)));
$this->cezpdf->addText(310,  ($postY*50)-89, 10, utf8_decode_seguro(substr($fecha, 3, 2)));
$this->cezpdf->addText(344,  ($postY*50)-89, 10, utf8_decode_seguro(substr($fecha, 8, 4)));
$this->cezpdf->addText(130, 690, 8, utf8_decode_seguro($nombre_cliente));
...
//$this->cezpdf->addText(100, 690, 9, utf8_decode_seguro($ruc));
...
//            $this->cezpdf->addText(378,790, 11, utf8_decode_seguro('X'));
                /////
//$this->cezpdf->addText(60, 700, 8, utf8_decode_seguro(substr($direccion,0,58)));
...
$this->cezpdf->addText(190, $y, 8, utf8_decode_seguro($producto . ' - ' . $unidad));

/*
$this->cezpdf->addText(450, $y, 9, utf8_decode_seguro($simb) . ' ' . number_format($valor->CPDEC_Subtotal,3));*/
$this->cezpdf->addText(360,$y,8, $simb);
$this->cezpdf->addTextWrap(370 ,$y, 35, 8,$pu, 'right');
...
if($tipoOpera=="C"){
    $datos_empresa = $this->empresa_model->obtener_datosEmpresa($proveedor2);
            ...
            $movil = $datos_empresa[0]->EMPRC_Movil;
            $fax = $datos_empresa[0]->EMPRC_Fax;
            $empresCs=$datos_empresa[0]->EMPRP_Codigo;
         $emp_direccion = $this->empresa_model->obtener_establecimientosEmpresa_principal($empresCs);
if(count($emp_direccion)>0){

$direccion=substr($emp_direccion[0]->EESTAC_Direccion,0, 80); 

}else{

$direccion=substr($datos_empresa[0]->EMPRC_Direccion,0, 50); 
 
}

     $this->cezpdf->addText(480, (30*27)+7, 15, utf8_decode_seguro($ruc));
     if ($img != 0) {


    $postY=16;
    $fecha_text = utf8_decode_seguro(substr($fecha, 0, 2) . '             ' . (substr($fecha, 3, 2)) . '                            ' . substr($fecha, 8, 4));
$this->cezpdf->addText(500,  ($postY*50)-95, 10, utf8_decode_seguro(substr($fecha, 0, 2))." - ");
$this->cezpdf->addText(530,  ($postY*50)-95, 10, utf8_decode_seguro(substr($fecha, 3, 2))." - ");
$this->cezpdf->addText(554,  ($postY*50)-95, 10, utf8_decode_seguro(substr($fecha, 8, 4)));
$this->cezpdf->addText(80, 735, 8, utf8_decode_seguro($nombre_cliente));
                        // $ruc = substr($ruc, 0, 1) . '    ' . substr($ruc, 1, 1) . '   ' . substr($ruc, 2, 1) . '   ' . substr($ruc, 3, 1) . '  ' . substr($ruc, 4, 1) . '   ' . substr($ruc, 5, 1) . '   ' . substr($ruc, 6, 1) . '   ' . substr($ruc, 7, 1) . '    ' . substr($ruc, 8, 1) . '   ' . substr($ruc, 9, 1) . '   ' . substr($ruc, 10, 1);
$this->cezpdf->addText(50, 690, 9, utf8_decode_seguro($ruc));
...
//            $this->cezpdf->addText(378,790, 11, utf8_decode_seguro('X'));
                /////
$this->cezpdf->addText(60, 670, 8, utf8_decode_seguro(substr($direccion,0,58)));
...
$this->cezpdf->addText(130, $y, 8, utf8_decode_seguro($producto . ' - ' . $unidad));
...
// $this->cezpdf->addText(26, 296, 8, 'SON: ' . strtoupper(num2letras(round($total, 2))) . ' ' . $moneda_nombre);
// = $datos_comprobante[0]->CPC_subtotal;
       /* $descuento = $datos_comprobante[0]->CPC_descuento;
        $igv = $datos_comprobante[0]->CPC_igv;
        $igv100 = $datos_comprobante[0]->CPC_igv100;
        $descuento100 = $datos_comprobante[0]->CPC_descuento100;
...
//$_SESSION['compania']=='6' dragon yuan andahuaylas
            ...
            $this->cezpdf->addText($posiciongeneralx+430, $posiciongeneraly+740, 15, $numero);*/
                    ...
                    $this->cezpdf->addText($posiciongeneralx + 88, $posiciongeneraly + 682, 9, utf8_decode_seguro($ruc));
                    //$this->cezpdf->addText(400, 670, 9, utf8_decode_seguro($hora));
                    ///***************datos de empresa
...
//********************
            ...
            if ($tipo_oper == 'V') {
                ...
                $datos_comprobante = $this->comprobante_model->obtener_comprobante($codigo);
                //$datoscomprobante_detalle = $this->comprobante_model->obtener_comprobantedetalle($codigo);
                $datos_vendedor=$datos_comprobante[0]->CPC_VendedorPersona;
                $datos_forPago=$datos_comprobante[0]->FORPAP_Codigo;
                $presupuesto = $datos_comprobante[0]->PRESUP_Codigo;
                $serie = $datos_comprobante[0]->CPC_Serie;
                $numero = $datos_comprobante[0]->CPC_Numero;
                $proveedor = $datos_comprobante[0]->CLIP_Codigo;
                ...
                $guiarem_codigo = $datos_comprobante[0]->GUIAREMP_Codigo;
                $modo_impresion = $datos_comprobante[0]->CPC_ModoImpresion;
                $OCOMP_Codigo=$datos_comprobante[0]->OCOMP_Codigo;
                //$pu_sinigv = $datoscomprobante_detalle[0]->CPDEC_Pu;
...
//magdalena
            ...
            $posiciongeneraly=-6;
                ...
                /* Cabecera */
                $this->cezpdf->ezText('', '', array('leading' => 205));
                if ($img != 1) {
                    $this->cezpdf->ezText($serie, 20, array("leading" => -30 - $posiciongeneraly, 'left' => 390));
                    $this->cezpdf->ezText('- ' . $numero, 20, array("leading" => -0, 'left' => 440));
                } else {
    ...
    $listaVendedor=$this->comprobante_model->getNombreVendedor($datos_vendedor);
                foreach ($listaVendedor as $key => $value) {
                    $nombre_vendedor=$value->PERSC_Nombre;
                    $apP_vendedor=$value->PERSC_ApellidoPaterno;
                    $apM_vendedor=$value->PERSC_ApellidoMaterno;
    
                }
$lisFor=$this->comprobante_model->getNombreForPago($datos_forPago);
                foreach ($lisFor as $key => $value) {
                   $desForPago=$value->FORPAC_Descripcion;
                }
 if ($OCOMP_Codigo) {
         $listarOrden=$this->ocompra_model->getSeria_andNumero($OCOMP_Codigo);   
}elseif ($codigo_ocompra) {
         $listarOrden=$this->ocompra_model->getSeria_andNumero($codigo_ocompra); 
}               

foreach ($listarOrden as $key => $value) {
   $serie_numero_orden=$value->OCOMC_Serie."-".$value->OCOMC_Numero;
}
if($serie_numero_orden){
    $this->cezpdf->addText($posiciongeneralx + 270, $posiciongeneraly + 595, 10, utf8_decode_seguro($serie_numero_orden));
}else{
   $this->cezpdf->addText($posiciongeneralx + 300, $posiciongeneraly + 580, 10, utf8_decode_seguro(""));  
}
              
if($datos_vendedor!=0){
    $this->cezpdf->addText($posiciongeneralx + 258, $posiciongeneraly + 566, 10, utf8_decode_seguro($apP_vendedor ." ". $apM_vendedor." ".$nombre_vendedor));
}else{
   $this->cezpdf->addText($posiciongeneralx + 258, $posiciongeneraly + 566, 10, utf8_decode_seguro("------"));  
}
//$this->cezpdf->addText($posiciongeneralx + 258, $posiciongeneraly + 566, 10, utf8_decode_seguro("vendedor"));  
$this->cezpdf->addText($posiciongeneralx + 520, $posiciongeneraly + 580, 9, utf8_decode_seguro($desForPago));  

    $this->cezpdf->addText($posiciongeneralx + 260  , $posiciongeneraly + 630, 10, utf8_decode_seguro($fecha_dia)." / ");
    $this->cezpdf->addText($posiciongeneralx + 280, $posiciongeneraly + 630, 10, utf8_decode_seguro($fecha_mes)." / ");
    $this->cezpdf->addText($posiciongeneralx + 300, $posiciongeneraly + 630, 10, utf8_decode_seguro($fecha_año));
                //**************************************
                // $this->cezpdf->addText(180+$posiciongeneralx, 436+$posiciongeneraly, 9, utf8_decode_seguro($fecha_text2));

                //$this->cezpdf->addText(270, 706, 9, utf8_decode_seguro("Vendedor : " . $vendedor));
               //- $this->cezpdf->addText(500, 687, 9, utf8_decode_seguro($nombre_formapago));
//      $this->cezpdf->addText(400, 644, 9, utf8_decode_seguro($telefono));
$this->cezpdf->addText(400+$posiciongeneralx, 580+$posiciongeneraly, 10, utf8_decode_seguro(strtoupper($docurefe_codigo)));
                  //$this->cezpdf->addText(460+$posiciongeneralx, 580+$posiciongeneraly, 8, utf8_decode_seguro(strtoupper("condiciones")));
//$this->cezpdf->addText(500,200,30,'HOLAAAA');
$this->cezpdf->addText($posiciongeneralx + 55, $posiciongeneraly + 566, 10, utf8_decode_seguro($ruc));


  /*if($serie_numero_orden){
    $this->cezpdf->addText($posiciongeneralx + 270, $posiciongeneraly + 595, 9, utf8_decode_seguro($serie_numero_orden));
}else{
   $this->cezpdf->addText($posiciongeneralx + 300, $posiciongeneraly + 580, 9, utf8_decode_seguro(""));  
}
              
if($datos_vendedor!=0){
    $this->cezpdf->addText($posiciongeneralx + 258, $posiciongeneraly + 566, 9, utf8_decode_seguro($apP_vendedor ." ". $apM_vendedor." ".$nombre_vendedor));
}else{
   $this->cezpdf->addText($posiciongeneralx + 258, $posiciongeneraly + 566, 9, utf8_decode_seguro("------"));  
}
//$this->cezpdf->addText($posiciongeneralx + 258, $posiciongeneraly + 566, 9, utf8_decode_seguro("vendedor"));  
$this->cezpdf->addText($posiciongeneralx + 520, $posiciongeneraly + 580, 9, utf8_decode_seguro($desForPago));  

    $this->cezpdf->addText($posiciongeneralx + 260  , $posiciongeneraly + 630, 8, utf8_decode_seguro($fecha_dia));
    $this->cezpdf->addText($posiciongeneralx + 280, $posiciongeneraly + 630, 8, utf8_decode_seguro($fecha_mes));
    $this->cezpdf->addText($posiciongeneralx + 325, $posiciongeneraly + 630, 8, utf8_decode_seguro($fecha_año));
                //**************************************
                // $this->cezpdf->addText(180+$posiciongeneralx, 436+$posiciongeneraly, 9, utf8_decode_seguro($fecha_text2));
               ...
               //- $this->cezpdf->addText(500, 687, 9, utf8_decode_seguro($nombre_formapago));
//      $this->cezpdf->addText(400, 644, 9, utf8_decode_seguro($telefono));
                $this->cezpdf->addText(400+$posiciongeneralx, 580+$posiciongeneraly, 9, utf8_decode_seguro(strtoupper($docurefe_codigo)));
                  //$this->cezpdf->addText(460+$posiciongeneralx, 580+$posiciongeneraly, 8, utf8_decode_seguro(strtoupper("condiciones")));
//$this->cezpdf->addText(500,200,30,'HOLAAAA');
      $this->cezpdf->addText($posiciongeneralx + 55, $posiciongeneraly + 566, 10, utf8_decode_seguro($ruc));*/
                //NOMBRE EMPRESA
                //$this->cezpdf->addText($posiciongeneralx + 76, $posiciongeneraly + 615, 6.6, utf8_decode_seguro($nombre_cliente));

                //DIRECCION
                //$this->cezpdf->addText($posiciongeneralx + 39, $posiciongeneraly + 595, 6.6, substr($direccion,0,62));


                $this->cezpdf->ezText('', '', array("leading" => -33));
                            $db_data = array(
                                      array('cols1' => utf8_decode_seguro(),'cols2' => utf8_decode_seguro("                    ".$nombre_cliente)),
                                      array('cols1' => utf8_decode_seguro(),'cols2' => utf8_decode_seguro($direccion))
                            ...
                            $this->cezpdf->ezText('', '5');
                             ...
                             'fontSize' => 10,
                             'cols' => array(
                                    'cols1' => array('width' => 205, 'justification' => 'left'),
                                    'cols2' => array('width' => 230, 'justification' => 'left')
...
//            $this->cezpdf->addText(400, 670, 9, utf8_decode_seguro($hora));
               ...
               $pdf->AddPage();
                ...
                foreach ($detalle_comprobante as $indice => $valor) {
                    ...
                    if ($modo_impresion==1) {
                            if ($valor->CPDEC_Pu_ConIgv != ''){
                            $pu_conigv = $pu_conigv = $valor->CPDEC_Pu_ConIgv;
                            $putotal_conigv=$valor->CPDEC_Total;}
                            else{ 
                            $pu_conigv = $valor->CPDEC_Pu + $valor->CPDEC_Pu * $valor->CPDEC_Igv100 / 100;
                            $putotal_conigv=$valor->CPDEC_Total;}
                        }
                            ...
                            if ($valor->CPDEC_Pu != '')
                            $pu_conigv = $valor->CPDEC_Pu;
                            $putotal_conigv = $pu_conigv*$valor->CPDEC_Cantidad;
                        }
//                $this->cezpdf->addText(50, $i, 9, $producto);
                    ...
                    // $this->cezpdf->addText(95+$posiciongeneralx, 676+$posiciongeneraly, 9, utf8_decode_seguro($nombre_cliente));
                        ...
                        //$cadena = explode($longitud, $valor->PROD_Nombre);
                        //$cadena = str_split($valor->PROD_Nombre,50);
                        
                        //$this->cezpdf->addText(120, $i-210, 6.6, $cadena);
                        //$this->cezpdf->Setposition
                        //25-08-2016
                        /*$nom_producto = $valor->PROD_Nombre;
                        if (strlen($nom_producto)>57&&strlen($nom_producto)<=200) {
                             $pdf->SetY( $posicionY + $articuloY);
                             $pdf->SetX($posicionX + 45 + $posicionXArticulo);
                             $pdf->MultiCell(100,3,utf8_decode_seguro(mb_strtoupper($nom_producto).'.'));
                             //$articuloY += 9;
                            
                            
                            
                        }*/
                        $prod_nombre = $valor->PROD_Nombre;// FACTURA ABAD IMPRE
                        //$prod_nombre = $valor->MARCC_Descripcion;
                        /*if (strlen($prod_nombre)>60) 
                        {*/
                            //split(pattern, $prod_nombre);
                           
                            //$newtext = wordwrap($prod_nombre,20,"\n",true);

                            //$this->cezpdf->addText(118,$i-210, 7, chunk_split($prod_nombre,10)."\n");
                            //$cadena = count(explode(" ",$prod_nombre));
                           
                  $this->cezpdf->addText(122,$i-190, 9.5,utf8_decode(substr($prod_nombre, 0,55)));



                            

                                //$this->c->addTeezpdfxt(118,$i-210, 7,$prod_nombre);

                           

                            

                            //$this->cezpdf->addText(118,$i-210, 7, $cadena);
                        //}
                   ...
                   //- $prod_nombreimei = utf8_decode_seguro($valor->PROD_Nombre) . ' / ' . $prod_nombreimei;
                    $this->cezpdf->addText(77+$posiciongeneralx, $i-190, 8, utf8_decode_seguro($unidad));
                    ///
...
//              $this->cezpdf->addText(120, $i, 9, utf8_decode_seguro($array_producto[0] . '  --- ' . $unidad));
                  //-  $this->cezpdf->addText(70 + $posiciongeneralx, $i + $posiciongeneraly, 9, $valor->CPDEC_Cantidad);
                   ...
                   //- $i -= 18;
                    ...
                    //-$posicionY -= 17;
        $this->cezpdf->addText(33, $i-190, 10, $valor->CPDEC_Cantidad);
...
$this->cezpdf->addTextWrap(538, $i-190, 45, 10,number_format($putotal_conigv, 4), 'right');
                  /*
                    ...
                    {
                        $this->cezpdf->addText(440, $i-210, 9, $moneda_simbolo);
                        $this->cezpdf->addTextWrap(440, $i-210, 45, 9, number_format($pu_sinigv, 2), 'right');

                        $this->cezpdf->addTextWrap(470, $i, 40, 9, $moneda_simbolo . '' . number_format($pu_sinigv, 2), "right");
                        $this->cezpdf->addText(525, $i-210, 9, $moneda_simbolo);
                        $this->cezpdf->addTextWrap(530, $i-210, 45, 9, number_format($valor->CPDEC_Total, 2), 'right');
...
$this->cezpdf->addText($posiciongeneralx + 117, $posiciongeneraly + 111, 9, strtoupper(num2letras(round($total, 2))) . ' ' . $moneda_nombre);
                  //$this->cezpdf->addText($posiciongeneralx + 117, $posiciongeneraly + 110, 9, strtoupper(num2letras(round($total, 2))) . ' ' . $moneda_nombre);
//            $this->cezpdf->addText(500, 110, 9, $moneda_simbolo . ' ' . number_format($subtotal, 2));
                //$this->cezpdf->addText(462, 306, 9, 'DCTO: '. $moneda_simbolo . ' ' . number_format($descuento, 2));
...
$this->cezpdf->addTextWrap($posiciongeneralx + 516, $posiciongeneraly + 34, 60, 10,  number_format(($total), 4), 'right');
               ...
               /* $this->cezpdf->addText($posiciongeneralx + 520, $posiciongeneraly + 88, 9, $moneda_simbolo);
                ...
                $this->cezpdf->addText($posiciongeneralx + 520, $posiciongeneraly + 59, 9, $moneda_simbolo);
                $this->cezpdf->addTextWrap($posiciongeneralx + 510, $posiciongeneraly + 59, 60, 9, number_format($igv, 2), 'right');
                $this->cezpdf->addText($posiciongeneralx + 520, $posiciongeneraly + 34, 9, $moneda_simbolo);
                $this->cezpdf->addTextWrap($posiciongeneralx + 510, $posiciongeneraly + 34, 60, 9, number_format(($total), 2), 'right');*/
...
/*
            ...
            $this->cezpdf->addText(90, 620, 6,  utf8_decode_seguro($prod_nombreimei));*/
                ...
                $igv = $datos_comprobante[0]->CPC_igv;
                $total = $datos_comprobante[0]->CPC_total;
                $ordenComCodigo=$datos_comprobante[0]->OCOMP_Codigo;
                
                $fecha = mysql_to_human($datos_comprobante[0]->CPC_Fecha);
                $datos_proveedor = $this->proveedor_model->obtener_Proveedor($proveedor);
                $ruc = $datos_proveedor[0]->EMPRC_Ruc;
                $empresa = $datos_proveedor[0]->EMPRC_RazonSocial;
                $emprE_direcion=$datos_proveedor[0]->EMPRC_Direccion;
                $guiainp_codigo = $datos_comprobante[0]->GUIAINP_Codigo;
                $guiasap_codigo = $datos_comprobante[0]->GUIASAP_Codigo;
                $guiarem_codigo = $datos_comprobante[0]->GUIAREMP_Codigo;
                if ($guiarem_codigo !== Null) {
                    ...
                    $guiasap_codigo = $list_guiare[0]->GUIASAP_Codigo;
                    $guiainp_codigo = $list_guiare[0]->GUIAINP_Codigo;
                     $guiarem_serie_numero= $list_guiare[0]->GUIAREMC_Serie." - ". $list_guiare[0]->GUIAREMC_Numero;

                }
                 $listarOrden=$this->ocompra_model->getSeria_andNumero($ordenComCodigo);
/*}elseif ($OCOMP_Codigos) {
   $listarOrden=$this->ocompra_model->getSeria_andNumero($OCOMP_Codigos);
}*/
  $serie_numero_orden="";//$listarOrden[0]->$value->OCOMC_Serie;

  if (count($listarOrden)>0) {
      foreach ($listarOrden as $key => $value) {
   $serie_numero_orden=$value->OCOMC_Serie." - ".$value->OCOMC_Numero;
        }  
    }
            ...
            /* Cabecera */
                ...
                $this->cezpdf->ezText(utf8_decode_seguro(substr($fecha, 6, 4)), 9, array("leading" => 0, "left" => 515));
                $this->cezpdf->ezText('-----', 8, array("leading" => 16, "left" => 333));
                $this->cezpdf->ezText($serie_numero_orden, 8, array("leading" => 15, "left" => 333));
                $this->cezpdf->ezText($serie, 18, array("leading" => -55, 'left' => 395));
                $this->cezpdf->ezText($numero, 18, array("leading" => 0, 'left' => 440));
                /* Listado de detalles */
                $posicionX = 0;
                $posicionY = 640;
                $db_data = array();
                $this->cezpdf->addText($posicionX+520, $posicionY+70, 9, $guiarem_serie_numero);
                $this->cezpdf->addText($posicionX+50, $posicionY+70, 9,$emprE_direcion);
                foreach ($detalle_comprobante as $indice => $valor) {
                    $c = 0;
                    $array_producto = explode('/', $valor->PROD_Nombre);
                    $producto = $valor->PROD_CodigoUsuario;
...
//$_SESSION['compania']=='6' dragon yuan andahuaylas
                    ...
                    $this->cezpdf->addText($posiciongeneralx + 88, $posiciongeneraly + 680, 8, utf8_decode_seguro($ruc));
                    //$this->cezpdf->addText(400, 670, 9, utf8_decode_seguro($hora));
                    ///***************datos de empresa
...
//************************
            ...
            if ($tipo_oper == 'V') {
                ...
                $hora = $datos_comprobante[0]->CPC_Hora;
                $observacion = $datos_comprobante[0]->CPC_Observacion;
                $OCOMP_Codigos=$datos_comprobante[0]->OCOMP_Codigo;
                $fecha = mysql_to_human($datos_comprobante[0]->CPC_Fecha);
                $vendedor = $datos_comprobante[0]->USUA_Codigo;
                $datos_proveedor = $this->cliente_model->obtener_datosCliente($proveedor);
                $empresa = $datos_proveedor[0]->EMPRP_Codigo;
                ...
                if ($guiarem_codigo !== Null) {
                    ...
                    $guiasap_codigo = $list_guiare[0]->GUIASAP_Codigo;
                    $guiainp_codigo = $list_guiare[0]->GUIAINP_Codigo;
                    $ocompra_codigo = $list_guiare[0]->OCOMP_Codigo;
                }
                ...
                if ($tipo == 0) {
                    ...
                    $emp_direccion = $this->empresa_model->obtener_establecimientosEmpresa_principal($datos_empresa[0]->EMPRP_Codigo);
                ...
                if ($img != 1) {
                   // $this->cezpdf->ezText($serie.'-' , 22, array("leading" => -108, 'left' => 390));
                    $this->cezpdf->ezText('0005'.$numero, 25, array("leading" => -80, 'left' => 380));
                } else {
                    $this->cezpdf->ezText('', 15, array("leading" => -110, 'left' => 330));
                    $this->cezpdf->ezText('', 15, array("leading" => 0, 'left' => 390));
                }
                ...
                $fecha_dia = substr($fecha, 0, 2);
                $fecha_mes = substr($fecha, 3, 2);
                $fecha_aNI = substr($fecha, 6, 8);
                //ABAD 2016 FACTURA PDF
$this->cezpdf->addText($posiciongeneralx + 280, $posiciongeneraly + 606, 10, utf8_decode_seguro($fecha_dia)." / ");
$this->cezpdf->addText($posiciongeneralx + 300, $posiciongeneraly + 606, 10, utf8_decode_seguro($fecha_mes)." / ");
$this->cezpdf->addText($posiciongeneralx + 320, $posiciongeneraly + 606, 10, utf8_decode_seguro($fecha_aNI));

                //**************************************
               // $this->cezpdf->addText($posiciongeneralx + 300, $posiciongeneraly + 100, 9, utf8_decode_seguro($fecha_text2));

                //$this->cezpdf->addText(270, 706, 9, utf8_decode_seguro("Vendedor : " . $vendedor));
                //$this->cezpdf->addText($posiciongeneralx + 470, $posiciongeneraly + 690, 9, utf8_decode_seguro($nombre_formapago));
//      $this->cezpdf->addText(400, 644, 9, utf8_decode_seguro($telefono));
            ...
            $listaVendedor=$this->comprobante_model->getNombreVendedor($datos_vendedor);
                ...
                $this->cezpdf->addText($posiciongeneralx + 75, $posiciongeneraly + 552, 9, utf8_decode_seguro($ruc));
if ($ocompra_codigo) {
   $listarOrden=$this->ocompra_model->getSeria_andNumero($ocompra_codigo);
}elseif ($OCOMP_Codigos) {
   $listarOrden=$this->ocompra_model->getSeria_andNumero($OCOMP_Codigos);
}
...
if($serie_numero_orden){
    $this->cezpdf->addText($posiciongeneralx + 300, $posiciongeneraly + 580, 9, utf8_decode_seguro($serie_numero_orden));
}else{
   $this->cezpdf->addText($posiciongeneralx + 300, $posiciongeneraly + 580, 9, utf8_decode_seguro(""));  
}
if($datos_vendedor!=0){
    $this->cezpdf->addText($posiciongeneralx + 300, $posiciongeneraly + 552, 9, utf8_decode_seguro($apP_vendedor ." ". $apM_vendedor." ".$nombre_vendedor));
}else{
   $this->cezpdf->addText($posiciongeneralx + 300, $posiciongeneraly + 552, 9, utf8_decode_seguro("-----------"));  
}
$this->cezpdf->addText(430+$posiciongeneralx, 570+$posiciongeneraly, 9, utf8_decode_seguro(strtoupper($docurefe_codigo)));
                
                $this->cezpdf->addText($posiciongeneralx + 80, $posiciongeneraly + 607, 8, utf8_decode_seguro($nombre_cliente));
                //Cliente Oculto
    $this->cezpdf->addText($posiciongeneralx + 80, $posiciongeneraly + 596, 8, substr($direccion,0,62));
//            $this->cezpdf->addText(400, 670, 9, utf8_decode_seguro($hora));
...
//                $this->cezpdf->addText(50, $i, 9, $producto);
                    ...
                    $this->cezpdf->addText(120, $i-210, 8, substr($valor->PROD_Nombre,0,70));

                    //$this->cezpdf->addText(340, $i, 8, utf8_decode_seguro($unidad));
                    //7
...
/*
                    ...
                    //$this->cezpdf->addTextWrap(470, $i, 40, 9, $moneda_simbolo . '' . number_format($pu_conigv, 2), "right");//revisar http://pubsvn.ez.no/doxygen/4.0/html/classCpdf.html#a4c3091f0936a733aa7e7ff98b876f3b1
                    $this->cezpdf->addText(500, $i-210, 8, $moneda_simbolo);
                    $this->cezpdf->addTextWrap(500, $i-210, 45, 8, number_format($valor->CPDEC_Total, 2), 'right');
                    //$this->cezpdf->addTextWrap(520, $i, 44, 9, $moneda_simbolo . ' ' . number_format($valor->CPDEC_Total, 2),"right");*/
                    $i -= 18;
...
//                $valortotal = strlen($observacion);
                ...
                $emprE_direcion ="";// $datos_empresa[0]->EMPRC_Direccion;
               $empresac = $datos_proveedor[0]->EMPRP_Codigo;
                
               $emp_direccion = $this->empresa_model->obtener_establecimientosEmpresa_principal($empresac);
       
            if(count($emp_direccion)>0){
              $emprE_direcion=$emp_direccion[0]->EESTAC_Direccion;
           }else{
               $emprE_direcion = $datos_proveedor[0]->EMPRC_Direccion;
...
//            /* Cabecera */
                ...
                $this->cezpdf->ezText('-----', 8, array("leading" => 16, "left" => 333));
                $this->cezpdf->ezText($serie_numero_orden, 8, array("leading" => 15, "left" => 333));
                $this->cezpdf->ezText($serie, 18, array("leading" => -50, 'left' => 395));
                $this->cezpdf->ezText($numero, 18, array("leading" => 0, 'left' => 440));
                ...
                $posicionY = 640;
                $db_data = array();
 $this->cezpdf->addText($posicionX+520, $posicionY+40, 9, $guiarem_serie_numero);

 $this->cezpdf->addText($posicionX+50, $posicionY+40, 9,substr($emprE_direcion, 0,53));
...
//dragon yuan mafgdalena
                    ...
                    ///***************datos de empresa
                    $this->cezpdf->addText(95 + $posiciongeneralx, 711 + $posiciongeneraly, 9, utf8_decode_seguro($nombre_cliente));
                    $this->cezpdf->addText(95 + $posiciongeneralx, 696 + $posiciongeneraly, 9, utf8_decode_seguro($direccion));
                    $this->cezpdf->addText(103 + $posiciongeneralx, 681 + $posiciongeneraly, 10, utf8_decode_seguro($ruc));
                    $this->cezpdf->addText(110 + $posiciongeneralx, 681 + $posiciongeneraly, 9, $dni);
                    //$this->cezpdf->addText(400, 670, 9, utf8_decode_seguro($hora));
                    ///***************datos de empresa
...
//dragotek
            if ($tipo_oper == 'V') {
                if ($img == 1) {
                    $notimg ="";// "boleta.jpg";//IMPRIMIR PDF
                } else {
                    $notimg = "boleta.jpg";   //fullcolor_boleta.jpg
                }
            } else {
                if ($img == 1) {
                    $notimg = "";
...
/*
                ...
                if ($img != 1) {
$this->cezpdf->ezText(utf8_decode_seguro($num_ser), 15, array("leading" => 90, "left" => 330));
$this->cezpdf->ezText(utf8_decode_seguro($num_doc), 15, array("leading" => 0, "left" => 400));
            // CODIGO  PARA VISUALIZAR EN EL PDF.....    
                } else {
                 $this->cezpdf->ezText(utf8_decode_seguro($num_ser), 15, array("leading" => 90, "left" => 330));
                $this->cezpdf->ezText(utf8_decode_seguro($num_doc), 15, array("leading" => 0, "left" => 400));
                //CODIGO PARA VISAULIZAR EN BLANCO PDF
                //....
                }
///BOLETA PDF
                ...
                $fechas = date($fecha_emision);
                $a = substr(date("Y", strtotime($fechas)), 2, 2);
                $m = (date("m", strtotime($fechas)));
                $d = date("d", strtotime($fechas));
                $this->cezpdf->addText(428 + $posiciongeneralx, 625 + $posiciongeneraly, 9, $d);
                $this->cezpdf->addText(460 + $posiciongeneralx, 625 + $posiciongeneraly, 9, $m);
                $this->cezpdf->addText(500 + $posiciongeneralx, 625 + $posiciongeneraly, 9, $a);

                
                $this->cezpdf->addText(160 + $posiciongeneralx, 594 + $posiciongeneraly, 7, utf8_decode_seguro($nombre_cliente));
//substr($direccion, 0,50) 
          
                $this->cezpdf->addText(415 + $posiciongeneralx, 570 + $posiciongeneraly, 8, $ruc);

                $this->cezpdf->addText(415 + $posiciongeneralx, 570 + $posiciongeneraly, 9, $dni);
                $this->cezpdf->addText(415 + $posiciongeneralx, 677 + $posiciongeneraly, 9, $num_guia);
$db_data = array();
                $positiony = 540;
                $positionx = 0;
                $serie_producto = "00000000001, 00000000002";
                ...
                foreach ($detalle_comprobante as $indice => $valor) {
                    ...
                    $ser = "";
                    $positionx += 0;
                   $this->cezpdf->addText(100 + $positionx + $posiciongeneralx, $positiony + $posiciongeneraly, 7, $valor->CPDEC_Cantidad);
                    ...
                    if ($valornombreproducto <= 25) {
                        ...
                        $this->cezpdf->addText(150 + $positionx + $posiciongeneralx, $positiony + $posiciongeneraly, 9, strtoupper(utf8_decode_seguro($nombreproducto)));
...
//            $presupuesto = $datos_comprobante[0]->PRESUP_Codigo;
                ...
                $ruc = $datos_proveedor[0]->EMPRC_Ruc;
                $empresa = $datos_proveedor[0]->EMPRC_RazonSocial;
                $empresCs=$datos_proveedor[0]->EMPRP_Codigo;
            $emp_direccion = $this->empresa_model->obtener_establecimientosEmpresa_principal($empresCs);
if(count($emp_direccion)>0){
$direccion=substr($emp_direccion[0]->EESTAC_Direccion,0, 80); 
}else{
$direccion=substr($datos_proveedor[0]->EMPRC_Direccion,0, 50); 
}
...
//            /* Cabecera */
                $this->cezpdf->ezText(utf8_decode_seguro($empresa), 8, array("leading" => 93, "left" => 15));
                $this->cezpdf->ezText(utf8_decode_seguro($ruc), 12, array("leading" => -90, "left" => 442));
                $this->cezpdf->ezText(utf8_decode_seguro(""), 9, array("leading" => 104, "left" => 10));
                $this->cezpdf->ezText($ruc, 9, array("leading" => 15, "left" => 10));
           $this->cezpdf->ezText(utf8_decode_seguro($direccion), 8, array("leading" => 11, "left" => 12));
//            $this->cezpdf->ezText(utf8_decode_seguro($vendedor), 7, array("leading" => -13, "left" => 333));
                $this->cezpdf->ezText(utf8_decode_seguro($nombre_formapago), 7, array("leading" => -16, "left" => 333));
                $this->cezpdf->ezText(utf8_decode_seguro($num_ser), 15, array("leading" => -35, "left" => 400));
...
//$listado = $this->comprobante_model->buscar_comprobante_compras();
        ...
        $clieProveedor="";
        if($tipo=="V"){
            $clieProveedor='CLIENTES';
          $listado = $this->comprobante_model->estadisticas_compras_ventas($tipo, $anio);
  
        }
        elseif($tipo=="C"){
            $clieProveedor='PROVEEDOR';
         $listado = $this->comprobante_model->Estadistita_compra($tipo, $anio);   
        }
...
//$listado = $this->comprobante_model->buscar_comprobante_compras();
        ...
        if ($tipo == "C") {
            $r = ' COMPRAS';
            $listado = $this->comprobante_model->estadisticas_compras_ventas_mensualfINAL($tipo, $anio, $mes);
...
public function JSON_ModificarCuenta($codigo,$estado=""){
 $variable=$this->comprobante_model->JSON_ModificarCuentasPago($codigo);
 $data_client=$this->comprobante_model->getcomprobante($codigo);
//echo "hola mundo<br>";
$cod=null;
foreach ($variable as $key => $value) {  
echo $value->CUE_CodDocumento." ".$value->CUE_Codigo."<br>"; 
 $cod=$value->CUE_Codigo; 
 $cuantas_pago=$this->comprobante_model->buscarCuentaspagos($cod);
 if(count($cuantas_pago)>0){  
 foreach ($cuantas_pago as $keys => $values) {  
    if($values->CUE_Codigo==$value->CUE_Codigo){
        echo "<h1>Contiene cuantas pagos no se puede modeficar</h1>";
    }else{
        echo "as tu modificacion "; 
 } 
}  
}else{
    $filter = new stdClass();
    $filter->CUE_FechaOper=human_to_mysql($this->input->post("fecha"));
    //  mdate("%Y-%m-%d ", time());
    $filter->CUE_FlagEstadoPago=$estado;
    $filter->CUE_Monto=$this->input->post("importetotal");
    $this->comprobante_model->UpdateCuentas($value->CUE_CodDocumento,$filter);
    echo "<h1>Solo se actualizo la fecha y monto<br></h1>";
    
    if($estado!="V"){ 
   
        $filter4 = new stdClass();
        $filter4->TIPCAMC_Fecha = $data_client[0]->CPC_Fecha;
        ...
        $filter3=new stdClass();
        $filter3->PAGC_TipoCuenta = $data_client[0]->CPC_TipoOperacion == 'V' ? 1 : 2;
        $filter3->PAGC_FechaOper = $data_client[0]->CPC_Fecha;
        if ($filter3->PAGC_TipoCuenta == 1){ 
            $filter3->CLIP_Codigo = $data_client[0]->CLIP_Codigo;
        }
        else{
            $filter3->PROVP_Codigo = $data_client[0]->PROVP_Codigo;
            }
        $filter3->PAGC_TDC = $tdc;
        $filter3->PAGC_Monto =   $data_client[0]->CPC_total;
        $filter3->MONED_Codigo = $data_client[0]->MONED_Codigo;
        $filter3->PAGC_FormaPago = '1'; //Efectivo
        $filter3->PAGC_Obs = ($data_client[0]->CPC_TipoOperacion == 'V' ? 'INGRESO GENERADO' : 'SALIDA GENERADA') . ' AUTOMATICAMENTE POR EL PAGO AL CONTADO';
        $filter3->PAGC_Saldo = '0';

        $cod_pago = $this->pago_model->insertar($filter3, '', '', '');
        
        $filter5 = new stdClass();
        $filter5->CUE_Codigo   = $value->CUE_Codigo;
        $filter5->PAGP_Codigo  =$cod_pago;
        $filter5->CPAGC_TDC    = $tdc;
        $filter5->CPAGC_Monto  = $value->CUE_Monto;
        $filter5->MONED_Codigo =$value->MONED_Codigo;
        $this->cuentaspago_model->insertar($filter5);
        echo "<h1>CLIENTE ". $data_client[0]->CLIP_Codigo."<br/></h1";
        echo "<h1>puedes modificar el contenido</h1>";
 }else{
            echo "<h1>Es V no se pude modificar</h1>";
...
public function verPdf($tipo_oper = '', $tipo_docu = '',$dataEviar=""){
    ...
    elseif($tipo_docu=='N'){
     ...
     $this->somevar['compania'];
        ...
        $this->cezpdf->ezText($titulo ."  ". $subTitulo, 17, $options);
