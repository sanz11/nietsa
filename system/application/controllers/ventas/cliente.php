...
class Cliente extends Controller
{
    ...
    function JSON_buscar_cliente($numdoc)
    {
        ...
        $resultado = '[{"CLIP_Codigoo":"0","EMPRC_Ruc":"","EMPRC_RazonSocial":""}]';
        if (count($datos_empresa) > 0) {
            $empresa = $datos_empresa[0]->EMPRP_Codigo;
            $razon_social = $datos_empresa[0]->EMPRC_RazonSocial;
            $datosCliente = $this->cliente_model->obtener_datosCliente2($empresa);
...
//                                OR p.PERSC_ApellidoPaterno LIKE '%".$keyword."%'";
        ...
        while ($data = mysql_fetch_assoc($query)) {
            //------------------------------------------------------------------------------------------	
            if ($data['CLIC_TipoPersona'] == '0') {
                $nombre = $data['PERSC_Nombre'] . ' ' . $data['PERSC_ApellidoPaterno'];
                $ruc = $data['PERSC_Ruc'];
            } else {
                $nombre = $data['EMPRC_RazonSocial'];
                $ruc = $data['EMPRC_Ruc'];
                $empresa = $data['EMPRP_Codigo'];
            }
            $result[] = array("value" => $nombre, "codigo" => $data['CLIP_Codigo'], "ruc" => $ruc, "empresa" => $empresa);
        }
    ...
    public function autocompletado_ruc()
    {
        ...
        if ($consulta != NULL) {
            foreach ($consulta AS $cliente => $value) {
                ...
                $result[] = array("value" => $ruc . ' ' . $nombre, "nombre" => $nombre, "codigo" => $value->CLIP_Codigo, "ruc" => $ruc);
            }
        }
        echo json_encode($result);
    }
    ...
    public function autocomplete_ruc()
    {
        ...
        while ($data = mysql_fetch_assoc($query)) {
            ...
            $result[] = array("value" => $ruc . ' ' . $nombre, "nombre" => $nombre, "codigo" => $data['CLIP_Codigo'], "ruc" => $ruc);
        }
