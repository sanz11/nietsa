    ...
    public function presupuestos($j = '0', $limpia = ''){
        ...
        if (count($listado_presupuestos) > 0) {
            foreach ($listado_presupuestos as $indice => $valor) {
                ...
                $ver2 = "<a href='javascript:;' onclick='ver_presupuesto_ver_pdf_conmenbrete(" . $codigo . ",1)' target='_parent'><img src='" . base_url() . "images/pdf.png' width='16' height='16' border='0' title='Ver PDF'></a>";
...
///GCBQ Ventana Presupuesto para comprobante segun tipo
    ...
    public function presupuesto_nueva($tipo_docu)
    {
        ...
        if (isset($numero_predt[0]->PRESUC_Numero))
        ...
        $data['numero_suger'] = $cofiguracion_datos[0]->CONFIC_Numero + 1;
...
<div style="padding-bottom:14px;">' . utf8_encode($xls) . '
    ...
    <tr>
        ...
        if ($this->input->post('pdf') != '') {
           ...
           // $pdf = $this->presupuesto_ver_pdf_conmenbrete_formato1($codigo,"0");
            $pdfcode = $pdf;
    ...
    public function presupuesto_ver_xls_correo($codigo)    {
      ...
      <td>" . number_format(($modo_impresion == '1' ? $valor->PRESDEC_Pu_ConIgv : $valor->PRESDEC_Pu), 4) . "</td>
      <td>" . number_format($valor->PRESDEC_Cantidad * ($modo_impresion == '1' ? $valor->PRESDEC_Pu_ConIgv : $valor->PRESDEC_Pu), 4) . "</td></tr>";
        }
        $xls .= "</table><br><br>";
        if (FORMATO_IMPRESION == 3) {
        ...
        $xls .= "
		 <table>
			<tr><td colspan=5><b>SON : " . strtoupper(num2letras(round($total, 4))) . " $moneda_nombre</td>
      <td><b>$subtotal_text</b></td><td><b>" . number_format($subtotal, 4) . "</b></td></tr>
			<tr><td colspan=5></td><td><b>$igv_text</b></td><td><b>" . number_format($igv, 4) . "</b></td></tr>
       ...
       if (FORMATO_IMPRESION == 3) {
            $xls .= "<tr><td>" . utf8_decode_seguro('Banco de Crédito Soles') . "</td><td>" . utf8_decode_seguro('N°  191-1739994-0-95 ') . "</td></tr>
        <tr><td>" . utf8_decode_seguro('Banco de Crédito Dólares') . "</td><td>" . utf8_decode_seguro('N° 191-2357918-1-61 ') . "</td></tr>";
        } else {
           $xls .= "<tr><td>" . utf8_decode_seguro('Cta. Cte. en Soles') . "</td><td>" . utf8_decode_seguro('N°  191-1739994-0-95') . "</td></tr>
        <tr><td>" . utf8_decode_seguro('Cta. Cte. en Dólares') . "</td><td>" . utf8_decode_seguro('N° 191-2357918-1-61') . "</td></tr>";
        }
...
//**********************************************
        if ($img == 0) {
            $this->cezpdf->ezImage("images/cabeceras/presupuestotek.png");

           // $this->cezpdf->ezImage("images/img_db/ferremax_cabe.jpg", -10, 555, 'none', 'left');
        }else{
           $this->cezpdf->ezImage("images/cabeceras/presupuestotek.png"); 
        }
    $lisCuentCorriente=$this->cliente_model->optenercuentaEmpresa($this->session->userdata('empresa'));
  
$cuenEmpresoles="";
$cuenEmpreDolares="";
    if (count($lisCuentCorriente)>0) {
        foreach ($lisCuentCorriente as $key => $value) {
            $cuenEmpresoles=$value->EMPRC_CtaCteSoles;
            $cuenEmpreDolares=$value->EMPRC_CtaCteDolares;
        }
    }
//////////////////////////////
    ...
    /*    if ($img == 0) {
            if ($this->somevar['compania'] == 1) {
                $this->cezpdf->ezImage("images/cabeceras/presupuestotek.jpg");
                //$this->cezpdf->ezImage("images/img_db/ferremax_cabe.jpg", -10, 555, 'none', 'left');
            } else {
                $this->cezpdf->ezImage("images/cabeceras/presupuestotek.jpg");
                //$this->cezpdf->ezImage("images/img_db/ferremax_cabe.jpg", -10, 555, 'none', 'left');
...
//******************************************
     ...
     $db_data = array(array('cols0' => '<b>CONDICIONES DE VENTA:</b>', 'cols1' => ''),
            ...
            array('cols0' => 'Tiempo de Entrega', 'cols1' => ': ' . $tiempo_entrega),
            array('cols0' => utf8_decode_seguro('Cta. Cte. en Soles'), 'cols1' => ': ' . $cuenEmpresoles),
            array('cols0' => utf8_decode_seguro('Cta. Cte. en Dólares '), 'cols1' => ': ' . $cuenEmpreDolares),
            ...
            array('cols0' => 'Contacto', 'cols1' => ': ' . utf8_decode_seguro($nombre_contacto  . ($vendedor_nombre_area != '' ? ' - AREA: ' . $vendedor_nombre_area : '')))
        );
 ...
 $cabecera = array('Content-Type' => 'application/pdf', 'Content-Disposition' => $codificacion . '.pdf', 'Expires' => '0', 'Pragma' => 'cache', 'Cache-Control' => 'private');
    ...
    $this->cezpdf->ezStream($cabecera);
          // $this->cezpdf->ezOutput($cabecera);
 }
if($img==0){
$cabecera = array('Content-Type' => 'application/pdf', 'Content-Disposition' => $codificacion . '.pdf', 'Expires' => '0', 'Pragma' => 'cache', 'Cache-Control' => 'private');
//$this->cezpdf->ezStream($cabecera);
 //ob_end_clean();
//$this->cezpdf->ezStream($cabecera);//abrir el pdf de forma automatica
//$output = $this->cezpdf->ezOutput($cabecera); //Salida de archivo
//file_put_contents('./personal/mipdf.pdf', $output);
   ...
   ////////////////
    ...
    public function obtener_lista_detalles($codigo){
        ...
        if (count($detalle) > 0) {
            foreach ($detalle as $indice => $valor) {
                ...
                //nombre producto
                $objeto->PROD_Nombre =$datos_producto[0]->PROD_Nombre;// $nombre_producto;
    ...
    public function presupuesto_ver_xls($codigo){
      ...
      <td><b>$subtotal_text</b></td><td><b>" . number_format($subtotal, 4) . "</b></td></tr>
        ...
        if (FORMATO_IMPRESION == 3) {
            $xls .= "<tr><td>" . utf8_decode_seguro('Banco de Crédito Soles') . "</td><td>" . utf8_decode_seguro('N°  191-1739994-0-95 ') . "</td></tr>
        <tr><td>" . utf8_decode_seguro('Banco de Crédito Dólares') . "</td><td>" . utf8_decode_seguro('N°  191-2357918-1-61 ') . "</td></tr>
        <tr><td>" . utf8_decode_seguro('Banco Continental BBVA') . "</td><td>" . utf8_decode_seguro('N°  0566-0100000944-77 ') . "</td></tr>";
        } elseif (FORMATO_IMPRESION !=3) {
            $xls .= "<tr><td>" . utf8_decode_seguro('Banco de Crédito Soles') . "</td><td>" . utf8_decode_seguro('N°  191-1739994-0-95 ') . "</td></tr>
        <tr><td>" . utf8_decode_seguro('Banco de Crédito Dólares') . "</td><td>" . utf8_decode_seguro('N°  191-2357918-1-61 ') . "</td></tr>
        <tr><td>" . utf8_decode_seguro('Banco Continental BBVA') . "</td><td>" . utf8_decode_seguro('N°  0566-0100000944-77') . "</td></tr>";
        } else{
            $xls .= "<tr><td>" . utf8_decode_seguro('Cta. Cte. en Soles') . "</td><td>" . utf8_decode_seguro('N°  191-1739994-0-95  ') . 
            "</td></tr><tr><td>" . utf8_decode_seguro('Cta. Cte. en Dólares') . "</td><td>" . utf8_decode_seguro('N° 191-2357918-1-61 ') .  "</td></tr>
        <tr><td>" . utf8_decode_seguro('Banco Continental BBVA') . "</td><td>" . utf8_decode_seguro('N°  0566-0100000944-77 ') . "</td></tr>";
        }
    ...
    public function presupuesto_ver_pdf_conmenbrete($codigo, $img)
    {
        ...
        $this->presupuesto_ver_pdf_correo($codigo, $img);
            ...
            case 6:
                ...
                break;
            default:
                presupuesto_ver_pdf_correo($codigo, $img);
                break;
        }
    }
...
public function presupuesto_ver_pdf_conmenbrete_formato1($codigo, $img){
        ...
        $area = $datos_presupuesto[0]->AREAP_Codigo;
        $vendedor_persona = $datos_presupuesto[0]->PRESUC_VendedorPersona;
        //agregar telefono de vendedor_persona PERSC_Telefono
    ...
    $detalle_presupuesto = $this->obtener_lista_detalles($codigo);
        ...
        if ($_SESSION['empresa'] == '3') {
            ...
            /*$this->cezpdf->ezText('RAZON SOCIAL: ELECTRO NIETSA E.I.R.L.', 9, array("leading" => -2, 'left' => 160,'right'=>0)); 
               $this->cezpdf->ezText('', '');
            $this->cezpdf->ezText('RUC: 20518952073', 10, array("leading" => -0, 'left' => 160,'right'=>0)); 

                
            $this->cezpdf->ezText('', '');
$this->cezpdf->ezText(utf8_decode_seguro('DIRECCIÓN: Jr Los Arquitectos Mz "f" lot "5" San Juan de Lurigancho -lima'), 10, array("leading" => -0, 'left' => 160,'right'=>0));  
            $this->cezpdf->ezText('', '');
            $this->cezpdf->ezText('TELEFONO: 719-0868 / 719-0865', 9, array("leading" => -1, 'left' => 160,'right'=>0));*/  
$this->cezpdf->ezImage("images/cabeceras/presupuestotek.jpg");
}
       ...
       $this->cezpdf->ezTable($db_data, $col_names, '', array(
        ...
        if ($vendedor_persona != '' && $vendedor_persona != '0') {
            ...
            if (count($datos_persona) > 0)
                $vendedor_nombre = $datos_persona[0]->PERSC_Nombre . ' ' . $datos_persona[0]->PERSC_ApellidoPaterno . ' ' . $datos_persona[0]->PERSC_ApellidoMaterno;
            // 03-09-2016
                $vendedor_telefono = $datos_persona[0]->PERSC_Telefono;
            }
        $vendedor_nombre_area = '';
        if ($vendedor_area != '' && $vendedor_area != '0') {
            $datos_area = $this->area_model->obtener_area($vendedor_area);
        ...
        $db_data = array(array('cols0' => '<b>CONDICIONES DE VENTA:</b>', 'cols1' => ''),
            ...
            array('cols0' => 'Telefono', 'cols1' => ': ' . utf8_decode_seguro($vendedor_telefono . ($vendedor_nombre_area != '' ? ' - AREA: ' . $vendedor_nombre_area : ''))),

            array('cols0' => utf8_decode_seguro('Cta. Cte. BCP Soles'), 'cols1' => ': ' . utf8_decode_seguro("191-1739994-0-95 ")),
            array('cols0' => utf8_decode_seguro('Cta. Cte. BCP Dólares'), 'cols1' => ': ' . utf8_decode_seguro("191-2357918-1-61")),
            array('cols0' => utf8_decode_seguro('Cta. Cte. BBVA'), 'cols1' => ': ' . utf8_decode_seguro("0566-0100000944-77")),
        );
        $this->cezpdf->ezTable($db_data, "", "", array(
            'width' => 525,
            'showLines' => 0,
    ...
    public function presupuesto_ver_pdf_conmenbrete_formato4($codigo)
    {
        ...
        foreach ($detalle_presupuesto as $indice => $valor) {
            ...
            $db_data[] = array(
                ...
                'cols5' => $valor->PRESDEC_Cantidad,
                'cols6' => number_format(($modo_impresion == '1' || $tipo_docu == 'B' ? $valor->PRESDEC_Pu_ConIgv : $valor->PRESDEC_Pu), 4),
                'cols7' => number_format($valor->PRESDEC_Cantidad * ($modo_impresion == '1' || $tipo_docu == 'B' ? $valor->PRESDEC_Pu_ConIgv : $valor->PRESDEC_Pu), 24)
            );
        }
        $col_names = array(
            'cols1' => 'Item',
...
$detalle =$this->presupuestodetalle_model->listar($presupuesto); //$this->presupuestodetalle_model->listar_data($presupuesto);
            ...
            if (count($detalle) > 0) {
                foreach ($detalle as $indice => $valor) {
                  ...
                  /*$codigo_interno = $valor->PROD_CodigoInterno;
                    $nombre_producto = ($valor->PRESDEC_Descripcion != '' ? $valor->PRESDEC_Descripcion : $valor->PROD_Nombre);
                    $nombre_producto = str_replace('"', "''", $nombre_producto);
                    $flagGenInd = $valor->PROD_GenericoIndividual;
                    $flagBS = $valor->PROD_FlagBienServicio;
                    $costo ="00";// $valor->PROD_CostoPromedio;
...
foreach ($datos_umedida as $key => $value) {
  $nombre_unidad =$value->UNDMED_Descripcion ;
                    ...
                    $objeto->UNDMED_Simbolo =$nombre_unidad;//$unidad_medida;//$datos_umedida[0]->UNDMED_Simbolo;//"HOL";// $nombre_unidad;
        ...
        if (count($datos_presupuesto) > 0) {
            ...
            if (count($detalle) > 0) {
                foreach ($detalle as $indice => $valor) {
                    ...
                    $igv100 = $valor->PRESDEC_Igv100;
                    $pu = $valor->PRESDEC_Pu;
                    $subtotal = round($valor->PRESDEC_Subtotal, 4);
                    $igv = $valor->PRESDEC_Igv;
