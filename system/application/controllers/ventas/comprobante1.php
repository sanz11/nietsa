...
//                }
    ...
    public function buscar($tipo_oper = '', $tipo_docu = '')
    {
        ...
        //agregado
        $data['registros'] = count($this->comprobante_model->buscar_comprobantes($tipo_oper, $tipo_docu, $filter, NULL, '', ''));
            ...
            if ($valor->CPC_DocuRefeCodigo !='' ) {
              ...
              if(count($listar_Orden)>0)  {
                ...
                if ($estado == '2')
                    ...
                    $disparador = "";
                if ($tipo_oper == 'V')
                    $lista[] = array($item++, $fecha, $serie, $numero, $guiarem_codigo, $docurefe_codigo, $nombre, $total, $img_estado, $editar, $ver, $ver2, $disparador, $estado, $codigo, $codigo_canje,$eliminar,$orden_compra);
                else
                    $lista[] = array($item++, $fecha, $serie, $numero, $guiarem_codigo, $docurefe_codigo, $nombre, $total, $img_estado, $editar, $ver, $ver2, $disparador, $estado, $codigo, $codigo_canje,$orden_compra);
...
//prep_pdf();
    ...
    public function comprobante_ver_pdf_conmenbrete_formato1_com($codigo, $img)
    {
        ...
        ///////stv
        if ($img == 0) {
            $notimg = "Boleta_venta.jpg"; //madyplac_com.jpg
...
//              $this->cezpdf->addText(120, $i, 9, utf8_decode_seguro($array_producto[0] . '  --- ' . $unidad));
                  //-  $this->cezpdf->addText(70 + $posiciongeneralx, $i + $posiciongeneraly, 9, $valor->CPDEC_Cantidad);
                   ...
                   //- $i -= 18;
                    ...
                    //-$posicionY -= 17;
                    $this->cezpdf->addText(33, $i-190, 9, $valor->CPDEC_Cantidad);
                    
                    //if($modo_impresion=='1'){
                        $this->cezpdf->addText(440, $i-190, 9, $moneda_simbolo);
                        $this->cezpdf->addTextWrap(440, $i-190, 45, 9, number_format($pu_conigv, 2), 'right');

                        //$this->cezpdf->addTextWrap(470, $i, 40, 9, $moneda_simbolo . '' . number_format($pu_conigv, 2), "right");
                        $this->cezpdf->addText(540, $i-190, 9, $moneda_simbolo);
                        $this->cezpdf->addTextWrap(540, $i-190, 45, 9, number_format($putotal_conigv, 2), 'right');
                  /*
...
*/
               ...
               //- $this->cezpdf->addText(66 + $posiciongeneralx, 395 + $posiciongeneraly, 9, strtoupper(num2letras(round($total, 2))) . ' ' . $moneda_nombre);
                  $this->cezpdf->addText($posiciongeneralx + 117, $posiciongeneraly + 105, 9, strtoupper(num2letras(round($total, 2))) . ' ' . $moneda_nombre);
...
//************************
            ...
            if ($tipo_oper == 'V') {
                ...
                $this->cezpdf->addText($posiciongeneralx + 280, $posiciongeneraly + 606, 8, utf8_decode_seguro($fecha_dia));
                $this->cezpdf->addText($posiciongeneralx + 300, $posiciongeneraly + 606, 8, utf8_decode_seguro($fecha_mes));
                $this->cezpdf->addText($posiciongeneralx + 340, $posiciongeneraly + 606, 8, utf8_decode_seguro($fecha_año));
                //**************************************
...
//                $this->cezpdf->addText(120, $i, 9, utf8_decode_seguro($array_producto[0] . '  --- ' . $unidad));
                ...
                /* Totales */
                // $this->cezpdf->addText(20, 360, 9, "Tipo de cambio " . $TDC[0]->TIPCAMC_FactorConversion . utf8_decode_seguro(" valido solo ") . $fecha . " // S/. " . ($total * $TDC[0]->TIPCAMC_FactorConversion) . " NUEVOS SOLES");
                $this->cezpdf->addText($posiciongeneralx + 70, $posiciongeneraly + 122, 9, strtoupper(num2letras(round($total, 2))) . ' ' . $moneda_nombre);
//            $this->cezpdf->addText(500, 110, 9, $moneda_simbolo . ' ' . number_format($subtotal, 2));
                //$this->cezpdf->addText(462, 306, 9, 'DCTO: '. $moneda_simbolo . ' ' . number_format($descuento, 2));
...
//            /* Cabecera */
 ...
 $this->cezpdf->addText($posicionX+50, $posicionY+40, 9,$emprE_direcion);
                foreach ($detalle_comprobante as $indice => $valor) {
                    $c = 0;
                    $array_producto = explode('/', $valor->PROD_Nombre);
                    $producto = $valor->PROD_CodigoUsuario;
...
//dragotek
            if ($tipo_oper == 'V') {
                if ($img == 1) {
                    ...
                    $notimg = "Boleta_venta2.jpg";   //fullcolor_boleta.jpg
                }
            } else {
                if ($img == 1) {
                    $notimg = "";
            ...
            if ($tipo_oper == 'V') {
                ...
                if ($img != 1) {
                    $this->cezpdf->ezText(utf8_decode_seguro($num_ser)."-", 15, array("leading" => 52, "left" => 280));
//                    $this->cezpdf->ezText(utf8_decode_seguro("N°"), 15, array("leading" => 0, "left" => 390));
                    $this->cezpdf->ezText(utf8_decode_seguro($num_doc), 15, array("leading" => 0, "left" => 340));
                } else {
                    $this->cezpdf->ezText(utf8_decode_seguro(""), 15, array("leading" => 67, "left" => 118));
                    $this->cezpdf->ezText(utf8_decode_seguro(""), 15, array("leading" => 0, "left" => 158));
                    $this->cezpdf->ezText(utf8_decode_seguro(""), 15, array("leading" => 0, "left" => 174));
            ...
            $posiciongeneraly=20;
                ...
                $m = mes_textual(date("m", strtotime($fechas)));
                $d = date("d", strtotime($fechas));
                $this->cezpdf->addText(376 + $posiciongeneralx, 659 + $posiciongeneraly, 9, $d);
                $this->cezpdf->addText(395 + $posiciongeneralx, 659 + $posiciongeneraly, 9, $m);
                $this->cezpdf->addText(428 + $posiciongeneralx, 659 + $posiciongeneraly, 9, $a);
                $this->cezpdf->addText(95 + $posiciongeneralx, 676 + $posiciongeneraly, 7, utf8_decode_seguro($nombre_cliente));
                $this->cezpdf->addText(93 + $posiciongeneralx, 669 + $posiciongeneraly, 7, substr($direccion, 0, 20));
                $this->cezpdf->addText(376 + $posiciongeneralx, 676 + $posiciongeneraly, 8, $ruc);

                $this->cezpdf->addText(95 + $posiciongeneralx, 648 + $posiciongeneraly, 9, $dni);
                ...
                foreach ($detalle_comprobante as $indice => $valor) {
                    ...
                    $this->cezpdf->addText(14 + $positionx + $posiciongeneralx, $positiony + $posiciongeneraly, 7, $valor->CPDEC_Cantidad);
                    ...
                    if ($valornombreproducto <= 25) {
                        ...
                        $this->cezpdf->addText(30 + $positionx + $posiciongeneralx, $positiony + $posiciongeneraly, 9, strtoupper(utf8_decode_seguro($nombreproducto)));
                        ...
                        $this->cezpdf->addText(25 + $positionx + $posiciongeneralx, $positiony + $posiciongeneraly, 6,substr($nombreproducto,0,50));
                        $positionx += 150;
                        $this->cezpdf->addText(130 + $positionx + $posiciongeneralx, $positiony + $posiciongeneraly, 7, $moneda_simbolo);
                        $this->cezpdf->addTextWrap(125 + $positionx + $posiciongeneralx, $positiony + $posiciongeneraly, 35, 7, number_format($valor->CPDEC_Pu_ConIgv, 2), 'right');
                        $positionx += 40;
                        $this->cezpdf->addText(140 + $positionx + $posiciongeneralx, $positiony + $posiciongeneraly, 7, $moneda_simbolo);

                        $this->cezpdf->addTextWrap(150 + $positionx + $posiciongeneralx, $positiony + $posiciongeneraly, 35, 7, number_format($valor->CPDEC_Total, 2), 'right');
                ...
                $positionx = 305;
                $positiony = 438 + $delta;
                $this->cezpdf->addText($positionx + 55, $positiony - 54 + $posiciongeneraly, 8, $moneda_simbolo);
                $this->cezpdf->addText($positionx + 80, $positiony - 54 + $posiciongeneraly, 8, number_format($total, 1));
