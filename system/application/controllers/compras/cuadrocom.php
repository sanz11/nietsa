...
class Cuadrocom extends Controller{
	...
	public function index()
	{
        ...
        public function cuadros($j=0){
            ...
            $data['action'] = base_url()."index.php/compras/cuadrocom/buscar_cuadros";
            $conf['base_url']   = site_url('maestros/compras/cuadrocom/');
		...
		if(count($listado_pedidos)>0){
				foreach($listado_pedidos as $indice=>$valor){
					...
					$pedido = $valor->PEDIC_Numero . ' ' .$valor->PEDIC_Observacion;
          ...
          $eliminar = '';
					$lista[]        = array($item,$pedido,$observacion,$fecha,$img_estado,$editar,$ver,$eliminar);
					$item++;
				}
			}
          $data['lista'] = $lista;
        ...
        public function buscar_cuadros($j=0){
            $filter = new stdClass();
			$filter->observacion = $this->input->post('txtNombre');
            $data['observacion']    = $filter->observacion;
            $data['titulo_tabla']    = "RESULTADO DE BÚSQUEDA DE CUADROS COMPARATIVOS";
            $data['registros']  = count($this->cuadrocom_model->buscar_cuadros($filter));
            $data['action'] = base_url()."index.php/compras/cuadrocom/buscar_cuadros";
            $conf['base_url']   = site_url('maestros/compras/cuadrocom/');
            $conf['total_rows'] = $data['registros'];
            $conf['per_page']   = 50;
            ...
            $this->pagination->initialize($conf);
            $data['paginacion'] = $this->pagination->create_links();
            $listado_pedidos = $this->cuadrocom_model->buscar_cuadros($filter,$conf['per_page'],$j);
            $item            = $j+1;
            $lista           = array();
			if(count($listado_pedidos) > 0){
					foreach($listado_pedidos as $indice=>$valor){
