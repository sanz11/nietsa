...
class Proveedor extends Controller
{
    ...
    public function insertar_proveedor()
    {
        ...
        $banSoles=$this->input->post('bancoSoles');
        $banDolares=$this->input->post('bancoDolares');
        $ctactesoles1 = $this->input->post('ctactesoles');

        $ctactedolares1 = $this->input->post('ctactedolares');

        $ctactesoles =$ctactesoles1 .'-_-'. $banSoles;
        $ctactedolares = $ctactedolares1 .'-_-'. $banDolares;
    ...
    public function editar_proveedor($id){
        ...
        if ($tipo_persona == 0) {//Persona
            ...
            /*Mejorar esto*/
            $bancosoles = $datos_persona[0]->PERSC_CtaCteSoles;
            $bancodolares= $datos_persona[0]->PERSC_CtaCteDolares;

            $ExplorarSoles=explode('-_-', $bancosoles);
            $ExplorarDolares=explode('-_-',$bancodolares);
            
            $ctactesoles=$ExplorarSoles[0];
            $ctactedolares=$ExplorarDolares[0];
            
            $bancosoles1 = $ExplorarSoles[1];
            $bancodolares2= $ExplorarDolares[1];
            $objeto = new stdClass();
            ...
            $objeto->ctactesoles = $ctactesoles;
            $objeto->ctactedolares = $ctactedolares;
            $objeto->bancosoles1=$bancosoles1 ;
            $objeto->bancodolares2=$bancodolares2;
            ...
            $bancosoles = $datos[0]->EMPRC_CtaCteSoles;
            $bancodolares= $datos[0]->EMPRC_CtaCteDolares;
            ...
            if (count($datos_empresaSucursal) > 0) {
                ...
                $direccion =$datos[0]->EMPRC_Direccion;;
            }
            ...
            $objeto->tipo = "1";
            $objeto->dni = "";
            $objeto->direccionEmpres=$datos[0]->EMPRC_Direccion;
...
//        /* Listado de detalles */
    ...
    public function autocomplete()
    {
        ...
        $consulta = "SELECT c.PROVP_Codigo, c.PROVC_TipoPersona, c.EMPRP_Codigo, c.PERSP_Codigo, p.PERSC_Nombre,
                                ...
                                WHERE
                                e.EMPRC_RazonSocial LIKE '%" . $keyword . "%'
                                OR p.PERSC_Nombre LIKE '%" . $keyword . "%'
                                OR p.PERSC_ApellidoPaterno LIKE '%" . $keyword . "%'";

        ///////////// cortado ce.COMPP_Codigo=".$compania." and proveedores no separados por compañia


//        $consulta = "SELECT c.CLIP_Codigo, c.CLIC_TipoPersona, c.EMPRP_Codigo, c.PERSP_Codigo, p.PERSC_Nombre,
//                                p.PERSC_ApellidoPaterno, p.PERSC_Ruc,
//                                e.EMPRC_RazonSocial, e.EMPRC_Ruc
//                                FROM cji_cliente c
//                                INNER JOIN cji_empresa e ON c.EMPRP_Codigo = e.EMPRP_Codigo
//                                INNER JOIN cji_persona p ON c.PERSP_Codigo = p.PERSP_Codigo
//                                WHERE e.EMPRC_RazonSocial LIKE '%".$keyword."%'
//                                OR p.PERSC_Nombre LIKE '%".$keyword."%'
//                                OR p.PERSC_ApellidoPaterno LIKE '%".$keyword."%'";
        //echo $consulta;
        ...
        while ($data = mysql_fetch_assoc($query)) {
            ...
            $result[] = array("value" => $nombre, "codigo" => $data['PROVP_Codigo'], "ruc" => $ruc);
        }
    ...
    public function autocomplete_ruc()
    {
        ...
        while ($data = mysql_fetch_assoc($query)) {
            ...
            $result[] = array("value" => $ruc . ' ' . $nombre, "nombre" => $nombre, "codigo" => $data['PROVP_Codigo'], "ruc" => $ruc);
        }
