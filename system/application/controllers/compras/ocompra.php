...
//                              exit ('{"result":"error2", "msj":"No ha ingresado todos los número de series de :\n'.$proddescri[$indice].'"}');
//                    }else
//                         exit ('{"result":"error2", "msj":"No ha ingresado los número de series de :\n'.$proddescri[$indice].'"}');
//                   }
//              }
        ...
        //Datos cabecera de la guiasa. bloqueado gcbq
        /*if ($tipo_oper == 'V') {
        ...
        if (is_array($prodcodigo)) {
            foreach ($prodcodigo as $indice => $valor) {
                ...
                /* if ($accion == "n") {
                     ...
                     //Actualizamos la tabla productoproveedor
                     if ($tipo_oper == 'C') {
                         $productoproveedor = $this->productoproveedor_model->obtener($producto, $proveedor);
                         if (count($productoproveedor) == 0) {
                             $filter1 = new stdClass();
                             $filter1->PROVP_Codigo = $proveedor;
                             $filter1->PROD_Codigo = $producto;
                             $this->productoproveedor_model->insertar($filter1);
                         }
                     }
                 }*/
                $this->ocompradetalle_model->insertar($filter);
                /*  if ($tipo_oper == 'C' && $cotizacion != '0') {
                      $detcotizacion = $this->cotizacion_model->obtener_detcotizacion_producto($producto, $cotizacion);
                      $this->cotizacion_model->modificar_detcotizacion_flagCompra($detcotizacion->COTDEP_Codigo);
                  }*/
            }
        }
        /* if ($tipo_oper == 'C' && $cotizacion != '0')
             $this->cotizacion_model->modificar_cotizacion_flagCompra($cotizacion);*/
...
$data['cboMiContacto'] = form_dropdown("formapago", $this->directivo_model->listar_directivo($empresa, ($tipo_oper == 'V' ? '5' : '4')), $formapago, " class='comboMedio' id='formapago' onfocus='javascript:this.blur();return false;'");
...
//$data['cboContacto'] = $this->OPTION_generador($this->directivo_model->listar_directivo($empresa, ($tipo_oper == 'V' ? '5' : '4')), 'DIREP_Codigo', array('PERSC_ApellidoPaterno', 'PERSC_ApellidoMaterno', 'PERSC_Nombre'), $contacto, array('', '::Seleccione::'), ' ');
   ...
   $nombre_almacen = '';
        ...
        $fecha = $arrfecha[0];
        $flagIngreso = $datos_ocompra[0]->OCOMC_FlagIngreso;
        $cuentaSolesFinal="";$cuantaDolaresFinal="";
        ...
        $personaEmpresa=$this->ocompra_model->getPersonaEmpresa($proveedor);

        $empresa_contacto=$this->ocompra_model->getEmpresaContacto($personaEmpresa[0]->EMPRP_Codigo);
...
//        }else{
        ...
        $data_footer = array(
            ...
            array('cols0' => utf8_decode_seguro("Contacto"), 'cols1' => ':' . $empresa_contacto[0]->PERSC_Nombre),
            ...
            array('cols0' => utf8_decode_seguro("Cuenta Proveedor"), 'cols1' => '' ),
            array('cols0' => utf8_decode_seguro('Cta. Cte. BCP Soles'), 'cols1' => ':' . $ctactesoles),
            array('cols0' => utf8_decode_seguro('Cta. Cte. BCP Dólares'), 'cols1' => ':' .    $ctactedolares)
        );
//abad richard
        $this->cezpdf->ezTable($data_footer, "", "", array(
            ...
            'cols' => array(
                ...
                'cols1' => array('width' => '410', 'justification' => 'left'),
            )
        ));
