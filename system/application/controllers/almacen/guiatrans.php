        ...
        if ($datos_guiatrans) {
            ...
            if ($datos_detallegtrans != NULL) {
                ...
                foreach ($datos_detallegtrans as $indice => $valor) {
                    ...
                    $filterGuiasaDet->GUIASADETC_GenInd = 'G';
                    //$filterGuiasaDet->GUIASADETC_FechaRegistro = date("Y-m-d H:i:s");
                    //$filterGuiasaDet->GUIASADETC_FlagEstado = 1;

                    $insertGuiasa = $this->guiasadetalle_model->insertar_2015($filterGuiasaDet);
    ...
    public function insertar_guiaintrans($id_guiatrans, $codUsuario, $tipoDocumento, $guiaAlmacen)
    {
        ...
        if (is_array($datos_detallegtrans)) {
            ...
            foreach ($datos_detallegtrans as $indice => $valor) {
                ...
                $insertGuiain = $this->guiaindetalle_model->insertar_2015($filterGuiainDet, 'TRANSFERENCIA');
                if ($insertGuiain) {
                    $contDetalles++;
                }
            }
