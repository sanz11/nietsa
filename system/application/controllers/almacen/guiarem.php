...
class Guiarem extends controller{
    ...
    public function listar($tipo_oper = 'V', $j = 0, $limpia = '') {
        ...
        $data['registros'] = count($this->guiarem_model->buscar($tipo_oper, $filter));
       ...
       $list_com = $this->comprobante_model->obtener_comprobante_ref2($codigo);
                ...
                if (count($list_com) > 0) {
                    $tipo_o = $list_com[0]->CPC_TipoOperacion;
                    $tipo_d = $list_com[0]->CPC_TipoDocumento;
                    $comp_id = $list_com[0]->CPP_Codigo;
                    $comp = $list_com[0]->CPC_Serie . '-' . $list_com[0]->CPC_Numero;
                    if ($tipo_d == "F") {
                ...
                if ($oc != "" || count($list_com) > 0 || $numeroref != "") {
                    ...
                    $eliminar = ($estado == '1' || $estado == '2' ? "<a href='" . base_url() . "index.php/seguridad/usuario/ventana_confirmacion_usuario2/guiarem/" . $codigo . "' id='linkVerProveedor'><img src='" . base_url() . "images/eliminar.png' alt='Activo' title='Activo' /></a> " : "<img src='" . base_url() . "images/inactive.png' alt='Anulado' title='Anulado' />");
                }
...
$lista[] = array($item++, $fecha, $serie, $numero, $codigo_usuario, $nombre_almacen, $nombre, $img_estado, $editar, $ver, $ver2, $eliminar, $orden_compra, $comprobante, $boleta, $disparador, $view_estado,$nomusuario);//comprobante, boleta
    ...
    public function editar($codigo, $tipo_oper = 'V')
    {
        ...
        $list_orden_venta=$this->guiarem_model->getNumeroSerieOrderVenta($ocompra);
...
$data["serie_numero_orden"]=$serie_numero_orden;
        ...
        //el tipo_oper asigna la variable-----------------------------------------
        $data['guia'] = $guiasa_id;
        /////
    ...
    public function grabar(){
        ...
        $codGiar=$this->input->post("txtCodigoG");
        ...
        $guiarem_id = $this->input->post("guiarem_id");
        //$guiarem_id1 = $this->input->post("guiarem_id");
        $guia_id = $this->input->post("guiasa_id");  //Puede ser guía de salida o guía de ingreso
...
//            $filter3->GUIASAC_Fecha = $this->_hoy;
...
//            $filter3->GUIAINC_Fecha = human_to_mysql($fecha);

//            $filter3->GUIAINC_FechaModificacion = $this->_hoy;
           ...
           // if ($accion == "m") {
               ...
               // $this->guiaremdetalle_model->eliminar2($guiarem_id);
                ...
                /*    if ($tipo_oper == 'V')
                  ...
                  $this->guiasadetalle_model->eliminar2($guia_id);

                  else

                  $this->guiaindetalle_model->eliminar2($guia_id); */
...
//            }
        ...
        if (is_array($prodcodigo)) {
            ...
            foreach ($prodcodigo as $indice => $valor) {
                ...
                $giaDetalle=$detacodi[$indice];
                //gcbq agrgar flagestado de terminado ocompra 
                if ($ordencompra != '') {

                    $cantidad_entregada = calcular_cantidad_entregada_x_producto($tipo_oper, $tipo_oper, $this->input->post('ordencompra'), $prodcodigo[$indice]);
...
//                    $filter4->GUIASADETC_GenInd = $detflag;
...
//                    $filter4->GUIIAINDETC_GenInd = $detflag;

//                }


                // echo " TOTAL : ".$cantidad_total_orden."<br />";

                // echo " ENTREGADOS ".$cantidad_total_productos_entregados."<br />";

                // echo " POR ENTREGAR ".$cantidad_total_productos_x_entregar."<br />";

                // echo " ENTREGADOS Y POR ENTREGAR ".$prod_entre_y_x_entregar."<br />";


               // if ($guiarem_id == "") {
        if($accion == 'e'){
            if($giaDetalle!=""){
                $this->guiaremdetalle_model->eliminarDetalle($giaDetalle); 
               
            }
                
        }elseif ($accion == 'm') {
                    if($giaDetalle!=""){
                         $this->guiaremdetalle_model->modificar($giaDetalle,$filter2);
...
//                            $this->guiaindetalle_model->insertar($filter4);
                      ...
                      //  $this->guiaremdetalle_model->modificar($filter2);
...
////        bloqueado stv   fatura nomas movera stock
...
//                            //bloqueado stv
                    ...
                    if ($tipo_oper == 'V') {
                        ...
                        $filter4->GUIASADETC_Costo = $pu_conigv;
                        ...
                        $filter4->GUIAINDETC_Costo = $pu_conigv;
    ...
    public function obtener_detalle_guiarem($guiarem, $tipo_oper = 'V', $almacen = 1)
    {
        ...
        $proveedor = $datos_guiarem[0]->PROVP_Codigo;
        $OCOMP_Codigo=$datos_guiarem[0]->OCOMP_Codigo;
       $listOcompra= $this->guiarem_model->obterner_order($OCOMP_Codigo);
        if ($tipo_oper == 'V')
...
$objeto->OCOMP_Codigo=$listOcompra[0]->OCOMC_Serie." - ".$listOcompra[0]->OCOMC_Numero;
                $objeto->GUIAREMC_CodigoUsuario = $codigo_usuario;
                //$objeto->OCOMP_Codigo= $OCOMP_Codigo;
            ...
            $objeto->GUIAREMC_Numero = $numero;
            //$objeto->OCOMP_Codigo= $OCOMP_Codigo;//$listOcompra[0]->OCOMC_Serie." ".$listOcompra[0]->OCOMC_Numero;
...
//             $this->guiarem_ver_pdf_formato1($codigo, $tipo_oper);
    ...
    {
//**************************************************************************   
// METODO PARA LA IMPRESION DE GUIAS DE REMISION
        $this->load->library('cezpdf');
        $this->load->helper('pdf_helper');
...
//            prep_pdf();
                   ...
                   $this->cezpdf->addText($posicionX + 129 + $posiciongeneralx, $posicionY + 685 + $posiciongeneraly, 8, substr($fecha,0,2));
                    $this->cezpdf->addText($posicionX + 156 + $posiciongeneralx, $posicionY + 685  + $posiciongeneraly, 8, substr($fecha,3,2));
                    $this->cezpdf->addText($posicionX + 190 + $posiciongeneralx, $posicionY + 685 + $posiciongeneraly, 8, substr($fecha,8,2));

                    //FECHA DE TRASLADO
                    $this->cezpdf->addText($posicionX + 286 + $posiciongeneralx, $posicionY + 685 + $posiciongeneraly, 8, substr($fecha_traslado, 0, 2));
                    $this->cezpdf->addText($posicionX + 309 + $posiciongeneralx, $posicionY + 685 + $posiciongeneraly, 8, substr($fecha_traslado, 3, 2));
                    $this->cezpdf->addText($posicionX + 345 + $posiciongeneralx, $posicionY + 685 + $posiciongeneraly, 8, substr($fecha_traslado, 8, 4));

                    //direccion partida


                            
                    $this->cezpdf->addText($posicionX +40, $posicionY +650, 8, utf8_decode_seguro(substr($arr_punt_part[0], 0, 37)));


                     $direccion_destino = substr($arr_punt_lleg[0], 0, 37);
                    $this->cezpdf->addText($posicionX +320, $posicionY +650, 8, utf8_decode_seguro(isset($direccion_destino) ? $direccion_destino : ''));
                    //empresa de transporte
                    $this->cezpdf->addText($posicionX + 85, $posicionY + 115, 7, utf8_decode_seguro($nombre_emprtrans));
                    ...
                    $this->cezpdf->addText($posicionX + 200, $posicionY + 610, 7, $placa);
                    $this->cezpdf->addText($posicionX + 150, $posicionY + 595, 7, $certificado);
                    $this->cezpdf->addText($posicionX + 140, $posicionY + 580, 7, $licencia);
                    $this->cezpdf->addText($posicionX + 150, $posicionY + 610, 7, $marca);
                    //Destinatario
                    $this->cezpdf->addText($posicionX + 390, $posicionY + 620, 8, substr($razon_social, 0,25));
                    $this->cezpdf->addText($posicionX + 75, $posicionY + 125, 8, $nombre_conductor);
                    $this->cezpdf->addText($posicionX + 430, $posicionY + 600, 8, $ruc);
...
//          $this->cezpdf->addText($posicionX + 68, $posicionY + 616, 9, "x");
                    ...
                    if (count($datos_detalle_guiarem) > 0) {
                        ...
                        foreach ($datos_detalle_guiarem as $indice => $valor) {
                            ...
                            ///$prod_unidad
                            $this->cezpdf->addText($posicionX + 130, $posicionY + 550, 8, utf8_decode_seguro($descri));   //$array_producto[0]
                            ///stv
                            $this->cezpdf->addText($posicionX + 70, $posicionY + 550, 6, utf8_decode_seguro($prod_unidad));
                            ///
                            // $this->cezpdf->addText($posicionX + 85, $posicionY + 540, 9, $prod_unidad);

                            $this->cezpdf->addTextWrap($posicionX + 50, $posicionY + 550, 35, 9, $prod_cantidad);
...
//            prep_pdf();
                    ...
                    $this->cezpdf->addText(88, '', 9, '');
                    if ($img == 0) {
                        //$this->cezpdf->addText($posicionX + 340, $posicionY + 720, 18, $serie." -");

                        //$this->cezpdf->addText($posicionX + 455, $posicionY + 720, 18, $numero);
                    } else {

                        //$this->cezpdf->addText($posicionX + 396, $posicionY + 685, 18, "$serie"." -");

                        //$this->cezpdf->addText($posicionX + 480, $posicionY + 685, 18, "$numero");

                    }
$this->cezpdf->addText($posicionX + 125, $posicionY + 724, 10, substr($fecha,0,2)." / ".substr($fecha,3,2)." / ".substr($fecha,6,4));
//$this->cezpdf->addText($posicionX + 136 , $posicionY + 719, 10, substr($fecha,3,2)." / ");
//$this->cezpdf->addText($posicionX + 155 , $posicionY + 719 , 10, substr($fecha,6,4));
//FECHA DE TRASLADO
 $this->cezpdf->addText($posicionX + 298 , $posicionY + 724, 10, substr($fecha_traslado, 0, 2)." / ".substr($fecha_traslado, 3, 2)." / ".substr($fecha_traslado, 6, 4));
//$this->cezpdf->addText($posicionX + 310 , $posicionY + 719, 10, substr($fecha_traslado, 3, 2)." / ");
//$this->cezpdf->addText($posicionX + 325 , $posicionY + 719, 10, substr($fecha_traslado, 6, 4));
                    //$this->cezpdf->addText($posicionX + 330, $posicionY + 673, 9, $fecha_traslado);
                    //direccion partida
$direccion_partida = substr($punto_partida, 0, 200);
                    //$this->cezpdf->addText($posicionX + 78, $posicionY + 636 + $posiciongeneraly, 8, utf8_decode_seguro( substr($arr_punt_part[0], 0, 49)));
//
//        $this->cezpdf->addText($posicionX + 62, $posicionY + 663, 8, utf8_decode_seguro(isset($arr_punt_part[1]) ? $arr_punt_part[1] . '321321321' : '9999999999999999'));
////direccion destino
$direccion_destino = substr($punto_llegada, 0, 200);
                    //$this->cezpdf->addText($posicionX + 330 + $posiciongeneralx, $posicionY + 636 + $posiciongeneraly, 8, utf8_decode_seguro(isset($direccion_destino) ? $direccion_destino : ''));*/                   
  $this->cezpdf->ezText('', '', array("leading" =>60));
    $db_data = array( array('cols1' => utf8_decode_seguro(),'cols2' => utf8_decode_seguro($direccion_partida),'cols3' => utf8_decode_seguro(), 'cols4' => utf8_decode_seguro($direccion_destino))
                                     // array('cols1' => utf8_decode_seguro(),'cols2' => utf8_decode_seguro(),'cols3' => utf8_decode_seguro(),'cols4' => utf8_decode_seguro()),   

                                      //array('cols1' => utf8_decode_seguro(),'cols2' => utf8_decode_seguro(),'cols3' => utf8_decode_seguro(),'cols4' => utf8_decode_seguro()),   
                                      //array('cols1' => utf8_decode_seguro(),'cols2' => utf8_decode_seguro(),'cols3' => utf8_decode_seguro(),'cols4' => utf8_decode_seguro($razon_social))
                             );
 ...
 $this->cezpdf->ezTable($db_data, "", "", array(
                             ...
                             'fontSize' => 8,
                             'cols' => array(
                                    'cols1' => array('width' => 30, 'justification' => 'left'),
                                    'cols2' => array('width' => 240, 'justification' => 'left'),
                                    'cols3' => array('width' => 30, 'justification' => 'left'),
                                    'cols4' => array('width' => 257, 'justification' => 'left')

                              
                                )
                            ));
$this->cezpdf->ezText('', '', array("leading" => 34));

 $razon_social_c= substr($razon_social, 0,37);

$db_data = array(array('cols1' => utf8_decode_seguro(),'cols2' => utf8_decode_seguro(),'cols3' => utf8_decode_seguro("".$razon_social_c)));

//$this->cezpdf->addText($posicionX + 478 , $posicionY + 614, 10,substr($razon_social, 0,30) );

$this->cezpdf->addText($posicionX + 478 , $posicionY + 627, 10,substr($razon_social, 0,19) );
$this->cezpdf->addText($posicionX + 340 , $posicionY + 617, 10,substr($razon_social, 19,200) );

$this->cezpdf->addText($posicionX + 95 , $posicionY + 80 , 8, $nombre_conductor);

$this->cezpdf->addText($posicionX + 230 , $posicionY + 600 , 7, $placa);
$this->cezpdf->addText($posicionX + 180 , $posicionY + 600 ,7, $marca);
$this->cezpdf->addText($posicionX + 180 , $posicionY + 580 , 7, $certificado); //
$this->cezpdf->addText($posicionX + 170 , $posicionY + 570 , 7, $licencia); //
$this->cezpdf->addText($posicionX + 505, $posicionY + 617, 8, $ruc);

$this->cezpdf->addText($posicionX + 500, $posicionY + 600, 8, $dni);
                    // $this->cezpdf->addText($posicionX + 68, $posicionY + 616, 9, "x");
                    /* Detalle */
                    $posicionY+=12;
                     $yy=540;
                    ...
                    if (count($datos_detalle_guiarem) > 0) {
                        ...
                        foreach ($datos_detalle_guiarem as $indice => $valor) {
                            ...
                            $this->cezpdf->addText($posicionX + 115, $posicionY+$yy, 8, utf8_decode_seguro(substr($descri, 0, 70))); 

                            $this->cezpdf->addText($posicionX + 70, $posicionY + $yy , 6, utf8_decode_seguro($prod_unidad));

                            $this->cezpdf->addTextWrap($posicionX + 30, $posicionY + $yy , 35, 9, $prod_cantidad, 'right');
...
$this->cezpdf->addText(600 + $posiciongeneralx, 100 + $posiciongeneraly, 9, utf8_decode_seguro($nombre_emprtrans2 != '' ? $nombre_emprtrans . ' k' : $nombre_emprtrans . ' L'));
                    ...
                    // ? que es
                    $this->cezpdf->addText($posx + 55  +$posiciongeneralx, $posy +42 + $posiciongeneraly, 14, '');
...
//            prep_pdf(); -
                    ...
                    $this->cezpdf->addTextWrap($posicionX + 75, $posicionY + 695,200, 9, substr($fecha,0,2));
                    $this->cezpdf->addTextWrap($posicionX + 95, $posicionY + 695,200, 9, substr($fecha,3,2));
                    $this->cezpdf->addTextWrap($posicionX + 115, $posicionY + 695,200, 9, substr($fecha,8,2));

                    $this->cezpdf->addText($posicionX + 240, $posicionY + 695, 9, $fecha_traslado);

                    //direccion partida

                    $this->cezpdf->addTextWrap($posicionX + 35, $posicionY + 665,200, 8, utf8_decode_seguro(substr($a3, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 35, $posicionY + 650,200, 8, utf8_decode_seguro(substr($a2, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 130, $posicionY + 650,200, 8, utf8_decode_seguro(substr($a1, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 210, $posicionY + 650,200, 8, utf8_decode_seguro(substr($a, 0, 37)));
...
//
                    ...
                    $this->cezpdf->addTextWrap($posicionX + 280, $posicionY + 665,200, 7, utf8_decode_seguro(substr($b3, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 280, $posicionY + 650,200, 7, utf8_decode_seguro(substr($b2, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 320, $posicionY + 650,200, 7, utf8_decode_seguro(substr($b1, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 340, $posicionY + 650,200, 7, utf8_decode_seguro(substr($b, 0, 37)));
                    ...
                    $this->cezpdf->addText($posicionX + 60, $posicionY + 108, 8, utf8_decode_seguro($nombre_conductor));
...
//            $this->cezpdf->addTextWrap($posicionX + 489, $posicionY + 689,200, 7, $placa);
                    ...
                    $this->cezpdf->addTextWrap($posicionX + 120, $posicionY + 608,200, 7, utf8_decode($marca." / ".$placa));
                    $this->cezpdf->addTextWrap($posicionX + 128, $posicionY + 595,200, 7, $certificado);
                    $this->cezpdf->addTextWrap($posicionX + 115, $posicionY + 583,200, 7, $licencia);
//Destinatario
                    $this->cezpdf->addText($posicionX + 395, $posicionY + 610, 8, $razon_social);
//          $this->cezpdf->addText($posicionX + 32, $posicionY + 80, 8, $nombre_conductor);
                    $this->cezpdf->addText($posicionX + 425, $posicionY + 598, 8, $ruc);
//          $this->cezpdf->addText($posicionX + 68, $posicionY + 616, 9, "x");
                    ...
                    if (count($datos_detalle_guiarem) > 0) {
                        ...
                        foreach ($datos_detalle_guiarem as $valor) {
                            ...
                            $this->cezpdf->addTextWrap($posicionX+1, $posicionY + 540,35, 9, $prod_cantidad,'right');
                            $this->cezpdf->addTextWrap($posicionX+75, $posicionY + 540,200, 9, $prod_nombre,'left');
                            $this->cezpdf->addTextWrap($posicionX-139, $posicionY + 540,200, 9, $prod_unidad,'right');
...
//VENTA GUIAREM PDF
                    if ($img == 0) {
                        //02-08-2016
                        $this->cezpdf->addTextWrap($posicionX + 370, $posicionY + 702,200, 20, $serie."-");
                        $this->cezpdf->addTextWrap($posicionX + 430, $posicionY + 702,200, 20, $numero);
                    ...
                    $this->cezpdf->addTextWrap($posicionX + 80, $posicionY + 698,200, 9, substr($fecha,0,2));
                    $this->cezpdf->addTextWrap($posicionX + 100, $posicionY + 698,200, 9, substr($fecha,3,2));
                    $this->cezpdf->addTextWrap($posicionX + 120, $posicionY + 698,200, 9, substr($fecha,8,2));

                    //$this->cezpdf->addText($posicionX + 330, $posicionY + 673, 9, $fecha_traslado);

                    //direccion partida

                    $this->cezpdf->addTextWrap($posicionX + 35, $posicionY + 650,200, 8, utf8_decode_seguro(substr($a3, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 35, $posicionY + 660,200, 8, utf8_decode_seguro(substr($a2, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 130, $posicionY + 660,200, 8, utf8_decode_seguro(substr($a1, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 200, $posicionY + 660,200, 8, utf8_decode_seguro(substr($a, 0, 37)));
...
//
                    ...
                    //$this->cezpdf->addTextWrap($posicionX + 300, $posicionY + 645,200, 7, utf8_decode_seguro(isset($direccion_destino) ? $direccion_destino : ''));
                    $this->cezpdf->addTextWrap($posicionX + 290, $posicionY + 650,200, 7, utf8_decode_seguro(substr($b3, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 290, $posicionY + 660,200, 7, utf8_decode_seguro(substr($b2, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 340, $posicionY + 660,200, 7, utf8_decode_seguro(substr($b1, 0, 37)));
                    $this->cezpdf->addTextWrap($posicionX + 420, $posicionY + 660,200, 7, utf8_decode_seguro(substr($b, 0, 37)));
...
//            $this->cezpdf->addTextWrap($posicionX + 489, $posicionY + 689,200, 7, $placa);
                    ...
                    $this->cezpdf->addTextWrap($posicionX + 150, $posicionY + 607,200, 7, utf8_decode($marca." / ".$placa));
...
//Destinatario
                    $this->cezpdf->addText($posicionX + 400, $posicionY + 610, 8, substr($razon_social, 0,20));
                       $this->cezpdf->addText($posicionX + 280, $posicionY + 600, 8, substr($razon_social, 21,200));
//          $this->cezpdf->addText($posicionX + 32, $posicionY + 80, 8, $nombre_conductor);
                    $this->cezpdf->addText($posicionX + 430, $posicionY + 597, 8, $ruc);
//          $this->cezpdf->addText($posicionX + 68, $posicionY + 616, 9, "x");
                    ...
                    if (count($datos_detalle_guiarem) > 0) {
                        ...
                        foreach ($datos_detalle_guiarem as $valor) {
                            ...
                            $this->cezpdf->addTextWrap($posicionX-5, $posicionY + 542,35, 9, $i,'right');
                            $this->cezpdf->addTextWrap($posicionX+400, $posicionY + 542,35, 9, $prod_cantidad,'right');
                            $this->cezpdf->addTextWrap($posicionX+80, $posicionY + 542,200, 9, $prod_nombre,'left');
                            $this->cezpdf->addTextWrap($posicionX+150, $posicionY + 542,200, 9, $prod_unidad,'right');
...
//            prep_pdf();
                ...
                if ($img == 0) {
                    ...
                    $this->cezpdf->addTextWrap($posicionX + 435, $posicionY + 565,100, 18, $serie);

                    $this->cezpdf->addTextWrap($posicionX + 480, $posicionY + 565,100, 18, $numero);
                ...
                $this->cezpdf->addTextWrap($posicionX + 70, $posicionY + 495,100, 9, substr($fecha,0,2));

                $this->cezpdf->addTextWrap($posicionX + 90, $posicionY + 495,100, 9, substr($fecha,3,2));

                $this->cezpdf->addTextWrap($posicionX + 110, $posicionY + 495,100, 9, substr($fecha,6,4));
...
//            $this->cezpdf->addText($posicionX + 260, $posicionY + 650, 7, $marca);
                ...
                $this->cezpdf->addText($posicionX + 46, $posicionY + 560, 8, $razon_social);

//            $this->cezpdf->addText($posicionX + 32, $posicionY + 80, 8, $nombre_conductor);

                $this->cezpdf->addText($posicionX + 490, $posicionY + 640, 12, $ruc);
                $this->cezpdf->addText($posicionX + 30, $posicionY + 505, 8, $ruc);
                ...
                if (count($datos_detalle_guiarem) > 0) {
                    ...
                    foreach ($datos_detalle_guiarem as $indice => $valor) {
                        ...
                        //   $this->cezpdf->addText($posicionX + 20, $posicionY + 540, 9, $prod_unidad);
                        //stv
                        $this->cezpdf->addTextWrap($posicionX + 20, $posicionY + 450,100, 6, utf8_decode_seguro($prod_codigo));
                        ///$prod_unidad
                        $this->cezpdf->addTextWrap($posicionX + 140, $posicionY + 450,100, 8, utf8_decode_seguro($prod_nombre));   //$array_producto[0]


                        ///stv
                        $this->cezpdf->addTextWrap($posicionX + 500, $posicionY + 450,100, 6, utf8_decode_seguro($prod_unidad));

                        ///

                        //  $this->cezpdf->addText($posicionX + 85, $posicionY + 540, 9, $prod_unidad);

                        $this->cezpdf->addTextWrap($posicionX + 60, $posicionY + 450,100, 9, $prod_cantidad);
...
////                    $this->cezpdf->addText($posicionX + 500, $posicionY + 548, 9, $moneda_simbolo . ' ' . $valor->GUIAREMDETC_Total);
                ...
                $this->cezpdf->addText(550, 140, 8, $moneda_simbolo . ' ' . $total);
...
//gcbq
    public function ventana_muestra_guiarem($tipo_oper, $codigo = '', $select = '', $tipo_doc = '', $almacen = '', $comprobante = '')
    {
        ...
        $lista_guiarem = $this->guiarem_model->buscar_asociados($tipo_oper, $filter);
...
public function verPdfGias($tipo_oper = '',$dataEviar=""){
     $titulo=""; $subTitulo="";
    if($tipo_oper=='V'){
     ...
     $listado = $this->guiarem_model->buscar_verPdf($tipo_oper, $filter);
...
$db_data[] = array(
                ...
                'col4' => $numero,
                'col6' => $nombre,//$nombre,
                'col7' => $valor->GUIAREMC_total,
                'col8' => $nomusuario//$nomusuario
            );
            $tolta_suma= $tolta_suma+$valor->GUIAREMC_total;
