    ...
    public function productos($flagBS = 'B', $j = '0',$limpia = ''){
       $data['compania'] = $this->somevar['compania'];
        ...
        if ($limpia == '1') {
            ...
            $this->session->unset_userdata('txtNombredos');
            $this->session->unset_userdata('txtFamilia');
            $this->session->unset_userdata('familiaid');
        ...
        if (count($_POST) > 0) {
            ...
            $filter->familiaid=$this->input->post('familiaid');
            $filter->nombredos=$this->input->post('txtNombredos');
        $this->session->set_userdata(array('txtNombre'=>$filter->nombre,'txtFamilia'=>$filter->familia,'txtCodigo'=>$filter->codigo,'txtMarca'=>$filter->marca,'familiaid'=>$filter->familiaid,'txtNombredos'=>$filter->nombredos));
        }else{
        ...
        $filter->nombredos=$this->session->userdata('txtNombredos');
         $filter->familiaid=$this->session->userdata('familiaid');
        }
        $data['nombredos']=$filter->nombredos;
        $data['codigo'] =$filter->codigo;
        $data['nombre'] =$filter->nombre;
        $data['familia']=$filter->familia;
        $data['familiaid']= $filter->familiaid;
...
//                        $esgenerico,
    ...
    public function buscar($flagBS = 'B', $j = 0)
    {
        ...
        $data['flagBS'] = $flagBS;
        $data['titulo_tabla'] = "RELACI&Oacute;N de " . ($flagBS == 'B' ? 'ARTICULO' : 'SERVICIO');
        $this->pagination->initialize($conf);//datos pagina
        $data['paginacion'] = $this->pagination->create_links();;
    ...
    public function buscar_productos($flagBS = 'B', $j = '0', $flag_nuevo = false)
    {
        ...
        $conf['base_url'] = site_url('almacen/producto/buscar_productos/' . $flagBS);
        $conf['per_page'] = 50;
        $conf['num_links'] = 3;
        $conf['first_link'] = "&lt;&lt;";
        $conf['last_link'] = "&gt;&gt;";
...
//    }
    ...
    public function ventana_selecciona_producto($tipo_oper = 'V', $flagBS = 'B', $buscar_producto = '', $j = '0')
    {
        $data['flagBS'] = $flagBS;
        $filter = new stdClass();
        $filter->flagBS = $flagBS;
        ...
        $data['action'] = base_url() . "index.php/almacen/producto/ventana_selecciona_producto/" . $tipo_oper . "/" . $flagBS . "/" . $buscar_producto;
        $conf['base_url'] = site_url('almacen/producto/ventana_selecciona_producto/' . $tipo_oper . "/" . $flagBS . "/" . $buscar_producto);
        $conf['total_rows'] = $data['registros'];
        $conf['per_page'] = 20;
        $conf['num_links'] = 3;
        $conf['next_link'] = "&gt;";
 ...
 */
                ...
                $seleccionar = '<a href="#" onclick="seleccionar_producto(' . $codigo . ',\'' . $interno . '\',\'' . $nombre_familia . '\', \'' . $stock . '\', \'' . $ultimo_costo . '\', \'' . $flagGenInd . '\')"><img src="' . base_url() . 'images/convertir.png"  border="0" title="Seleccionar"></a>';
    ...
    public function ventana_producto_serie($producto_id, $cantidad, $guia = '', $edit = '')
    {
        ...
        /* for($i=0;$i<$cantidad;$i++){
          $serie = "serie[$i]";
          $value = isset($serial[$i])?$serial[$i]:"";
          $numero_serie[] = form_input(array("name"=>$serie,"id"=>$serie,"class"=>"cajaMedia","value"=>$value));
          $cbo_serie[]    = "";
          } */
        //mostrar las series ya seleccionadas-----------------------------------------------------------------

        $numero_serie = array();

        $datos_serie = $this->seriemov_model->buscar_x_guiainp($guia, $producto_id);
        if (count($datos_serie) > 0) {
            for ($i = 0; $i < count($datos_serie); $i++) {
                $numero_serie[] = $datos_serie[$i]->SERIC_Numero;
            }
        } else {
    ...
    public function validarserie($serie)
    {

        $total = $this->serie_model->validarserie($serie);
    ...
    public function ventana_producto_series($producto_id, $cantidad, $guia = '')
    {
        $serie_value = $this->session->userdata('serie');
        $serial = array();
        if (count($serie_value) > 0 && $serie_value != "") {
        ...
        $data['form_hidden'] = form_hidden(array("base_url" => base_url(), "producto_id" => $producto_id, "cantidad" => $cantidad));
        $data['form_close'] = form_close();
        $this->load->view('almacen/ventana_producto_series', $data);
    }

    //Para sacar del almacen
    public function ventana_producto_serie2($producto_id, $cantidad, $almacen_id, $guia = '', $guiain = '', $tipo = '')
    {
        $this->load->model('almacen/serie_model');
        $serie_value = $this->session->userdata('serie');
        $series_sesion = array();
    ...
    public function ventana_producto_serie_grabar()
    {
        ...
        $serie = array_unique($ser);
        $producto_id = $this->input->post('producto_id');
        unset($_SESSION['serie'][$producto_id]);
        $data = array();
        $marca = 0;
        if (is_array($serie)) {
        ...
        $_SESSION['serie'][$producto_id] = $data;
        if ($marca == 1) {
            echo "<script>window.opener.document.frmGuiain.GenInd.value='';</script>";
            echo "<script>window.opener.document.frmGuiasa.GenInd.value='';</script>";
        } elseif ($marca == 0) {
            echo "<script>window.opener.document.frmGuiain.GenInd.value='I'</script>";
            echo "<script>window.opener.document.frmGuiasa.GenInd.value='I'</script>";
        }
...
//                    }
    ...
    public function listar_precios_x_producto_unidad($producto, $unidad, $moneda)
    {
        ...
        if (is_array($producto_precio) && count($producto_precio > 0)) {
            ...
            foreach ($producto_precio as $value) {
                ...
                $filter->establecimiento = substr($value->EESTABC_Descripcion, 0, 5);
    ...
    public function autocompletado_producto_x_nombre()
    {
        ...
        if($cargarProductos != NULL){
            foreach($cargarProductos AS $productos => $value){
                ...
                $result[] = array("value" => $codProUsuario . "  - " . $nombPro, "codigo" => $codPro, "codinterno" => $codProUsuario, "pcosto" => $costoPro, "stock" => $stock);
            }
        }
        echo json_encode($result);
    }
    ...
    public function autocomplete($f, $com)
    {
        $keyword = $this->input->post('term');
        $repartir=explode("-", $keyword);
        $query = mysql_query("SELECT DISTINCT(cji_producto.PROD_Codigo),`cji_producto`.*  
	  FROM (`cji_productocompania`)
	  LEFT JOIN `cji_producto` ON `cji_producto`.`PROD_Codigo` = `cji_productocompania`.`PROD_Codigo` AND `cji_producto`.`PROD_FlagActivo` = 1 
	AND  `cji_producto`.`PROD_FlagBienServicio` = '" . $f . "'  AND `cji_productocompania`.`COMPP_Codigo` = " . $com . "
	  LEFT JOIN `cji_compania` ON `cji_compania`.`COMPC_FlagEstado` = 1
	  ...
	  LIKE '" . $keyword. "%' OR `cji_producto`.`PROD_Nombre` LIKE '" .$repartir[0] . "%" .$repartir[1] . "%".$repartir[2]."%".$repartir[3] ."%' and cji_marca.MARCC_Descripcion like '%".$repartir[4] ."%' ORDER BY `cji_producto`.`PROD_Nombre` limit 0,50");
        ...
        while ($data = mysql_fetch_assoc($query)) {
            ...
            //stock
            $consulta = mysql_fetch_assoc(mysql_query("SELECT ALMPROD_Stock FROM `cji_almacenproducto` LEFT JOIN cji_almacen on cji_almacen.ALMAP_Codigo=cji_almacenproducto.ALMAC_Codigo WHERE PROD_Codigo=" . $cod_prod . " and cji_almacen.ALMAC_Descripcion not LIKE '%merma%'"));
            //ATRIBUTO 2
            ...
            $marcas=mysql_fetch_assoc(mysql_query("SELECT m.MARCC_Descripcion FROM cji_producto p JOIN cji_marca m on p.MARCP_Codigo=m.MARCP_Codigo WHERE  PROD_Codigo=".$cod_prod));

            //$pcosto = mysql_fetch_assoc(mysql_query("SELECT gd.GUIAINDETC_Costo FROM  cji_guiaindetalle gd  WHERE gd.PRODCTOP_Codigo=". $cod_prod ." "));
            $pcosto = mysql_fetch_assoc(mysql_query("SELECT gd.GUIAINDETC_Costo FROM  cji_guiaindetalle gd, cji_guiain g  WHERE gd.GUIAINP_Codigo=g.GUIAINP_Codigo and g.GUIAINC_FlagEstado=1 and gd.PRODCTOP_Codigo=" . $cod_prod . " order by gd.GUIAINDETP_Codigo desc limit 1"));
            ////

            //------------------------------------------------------------------------------------------	
            $result[] = array("value" => $data['PROD_CodigoUsuario'] . "  - " . $data['PROD_Nombre'], "codigo" => $data['PROD_Codigo'], "codinterno" => $data['PROD_CodigoUsuario'], "pcosto" => $pcosto['GUIAINDETC_Costo']
                ,"stock" => $consulta['ALMPROD_Stock'],"marcaNombre"=>$marcas['MARCC_Descripcion']);
        }
    ...
    /////,$familia='',$familiaid='',$marca='',$publicacion='',$codigo=''
...
//        /* Listado de detalles */
    ...
    ////
    public function registro_productos_pdf2($flagbs = 'B', $codigo,  $nombre,  $idfami, $marca)
    {
             $this->load->library('cezpdf');
        $this->load->helper('pdf_helper');
        //prep_pdf();
...
//        /* Listado de detalles */
        ...
        $listado_productos = $this->producto_model->listar_productos_pdf2($flagbs , $codigo,  $nombre,  $idfami, $marca);
