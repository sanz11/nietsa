...
//            $where .= ' and cp.CPC_Fecha BETWEEN "' . human_to_mysql($filter->fechai) . '" AND "' . human_to_mysql($filter->fechaf) . '"';
        ...
        /*if (isset($filter->producto) && $filter->producto != "")
            $where.=" AND cji_producto.PROD_CodigoInterno=" . $filter->producto;*/
       ...
       //25-08-2016
       if (isset($filter->fechai) && $filter->fechai != '' && isset($filter->fechaf) && $filter->fechaf != '')
            $where .= ' and cji_cuentas.CUE_FechaOper BETWEEN "' . human_to_mysql($filter->fechai) . '" AND "' . human_to_mysql($filter->fechaf) . '"';
    
         if (isset($filter->numero) && $filter->numero != '')
            $where.=' and cji_comprobante.CPC_Numero=' . $filter->numero;
    ...
    public function verificarxCuenta(){
        $where = array('CUE_Codigo');
        $query = $this->db->where->get('cji_cuentas');
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
            }
...
public function reporte_cuentas($tipo_cuenta = '1',$filter = NULL, $cond_pago = '', $comprobante = '') {
        $where = '';
        if ($comprobante != '' && isset($comprobante)) {
            if ($comprobante == 9) {
                $where.= " AND cji_cuentas.DOCUP_Codigo =9 ";
       ...
       //25-08-2016
       if (isset($filter->fechai) && $filter->fechai != '' && isset($filter->fechaf) && $filter->fechaf != '')
            $where .= ' and cji_cuentas.CUE_FechaOper BETWEEN "' .$filter->fechai . '" AND "' . $filter->fechaf. '"';
