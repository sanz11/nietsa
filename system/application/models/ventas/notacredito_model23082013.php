...
class Notacredito_model extends Model {
    ...
    public function buscar_comprobantes($tipo_oper = 'V', $tipo_docu = 'F', $filter = NULL, $number_items = '', $offset = '') {
        $compania = $this->somevar['compania'];
        ...
        /* if ($where == '') {
          ...
          $where = " AND DATE_FORMAT(cp.CRED_FechaRegistro,'%Y-%m-%d')='" . date('Y-m-d') . "' ";
          } */
  ...
  if ($query->num_rows > 0) {
    ...
    public function obtener_comprobante($comprobante) {
        $query = $this->db->where('CRED_Codigo', $comprobante)->get('cji_credito');
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
            }
