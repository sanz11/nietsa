    ...
    public function buscar_ocompra($tipo_oper, $fechai, $fechaf, $proveedor, $producto, $aprobado, $ingreso, $number_items = '', $offset = '')
    {
        ...
        $sql = "SELECT DATE_FORMAT(o.OCOMC_FechaRegistro, '%d/%m/%Y') fecha,
                         ...
                         o.OCOMC_Numero,
                           ...
                           (CASE WHEN o.COTIP_Codigo =0 THEN '***'
                           ELSE CAST(ct.COTIC_Numero AS char) END) cotizacion,
                         (CASE " . ($tipo_oper != 'C' ? "p.PROVC_TipoPersona" : "p.PROVC_TipoPersona") . "  WHEN '1' THEN e.EMPRC_Ruc ELSE pe.PERSC_NumeroDocIdentidad end) numdoc,
                       (CASE " . ($tipo_oper != 'C' ? "p.PROVC_TipoPersona" : "p.PROVC_TipoPersona") . "  WHEN '1' THEN e.EMPRC_RazonSocial ELSE CONCAT(pe.PERSC_Nombre , ' ', pe.PERSC_ApellidoPaterno, ' ', pe.PERSC_ApellidoMaterno) end) nombre,
                      m.MONED_Simbolo,
                       o.OCOMC_total,
                       (CASE o.OCOMC_FlagAprobado 
                                WHEN '0' THEN 'Pend.'
				...
				" . ($tipo_oper != 'C' ? "INNER JOIN cji_proveedor p ON p.PROVP_Codigo= o.PROVP_Codigo" : "LEFT JOIN cji_proveedor p ON p.PROVP_Codigo= o.PROVP_Codigo") . "
                LEFT JOIN cji_persona pe ON pe.PERSP_Codigo=p.PERSP_Codigo AND " . ($tipo_oper != 'C' ? "p.PROVC_TipoPersona" : "p.PROVC_TipoPersona") . " ='0'
                LEFT JOIN cji_empresa e ON e.EMPRP_Codigo=p.EMPRP_Codigo AND " . ($tipo_oper != 'C' ? "p.PROVC_TipoPersona" : "p.PROVC_TipoPersona") . "='1'
				 
				LEFT JOIN cji_cotizacion ct ON ct.COTIP_Codigo=o.COTIP_Codigo
                WHERE o.OCOMC_FlagEstado='1' " . $where . " AND o.OCOMC_TipoOperacion='C'
                AND o.COMPP_Codigo = '" . $compania . "'
                GROUP BY o.OCOMP_Codigo
                ORDER BY o.OCOMC_Numero DESC " . $limit . "
                ";//" . $tipo_oper . "
...
public function getSeria_andNumero($codigo){
    $this->db->select("OCOMC_Serie,OCOMC_Numero");
    $this->db->from("cji_ordencompra");
    $this->db->where('OCOMP_Codigo', $codigo);
    //$this->db->where('OCOMC_TipoOperacion','V');
    $query= $this->db->get();
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
...
public function getEmpresaContacto($codigo){
    $this->db->select("*");
    $this->db->from("cji_emprcontacto e");
    $this->db->join("cji_persona p","ECONC_Persona=p.PERSP_Codigo");
    $this->db->where("EMPRP_Codigo",$codigo);
    $this->db->where("ECONC_FlagEstado","1");
  $query= $this->db->get();
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
...
public function getPersonaEmpresa($codigo){
    $this->db->select("*");
    $this->db->from("cji_proveedor e");
    $this->db->join("cji_empresa p","p.EMPRP_Codigo=e.EMPRP_Codigo");
    $this->db->where("PROVP_Codigo",$codigo);
    $this->db->where("EMPRC_FlagEstado","1");
  $query= $this->db->get();
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
