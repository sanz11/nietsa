...
class Guiarem_Model extends Model
{
    ...
    // en desuso, reemplazadp por buscar(GUIA TRANSLOGINT)
    public function buscar_totalFilas($tipo_oper, $filter = NULL, $number_items = '', $offset = '')
    {
        ...
        $sql = "SELECT COUNT(g.GUIAREMP_Codigo) as total
                FROM cji_guiarem g
 ...
 if (isset($filter->serie) && $filter->serie != '')
    ...
    public function buscar_asociados($tipo_oper = 'V', $filter = NULL, $number_items = '', $offset = '')
    {
        $compania = $this->somevar['compania'];
        ...
        $sql = "SELECT cp.GUIAREMP_Codigo,
                ...
                LEFT JOIN cji_persona pe ON pe.PERSP_Codigo=c.PERSP_Codigo AND " . ($tipo_oper != 'C' ? "c.CLIC_TipoPersona" : "c.PROVC_TipoPersona") . " ='0'
                LEFT JOIN cji_empresa e ON e.EMPRP_Codigo=c.EMPRP_Codigo AND " . ($tipo_oper != 'C' ? "c.CLIC_TipoPersona" : "c.PROVC_TipoPersona") . "='1'
                WHERE  g.GUIAREMP_Codigo not in(SELECT cd.GUIAREMP_Codigo FROM cji_comprobante cd JOIN cji_guiarem g on g.GUIAREMP_Codigo = cd.GUIAREMP_Codigo) and
                 g.GUIAREMC_TipoOperacion='" . $tipo_oper . "'
        ...
        $sql = "SELECT g.GUIAREMC_Fecha,
                ...
                WHERE g.GUIAREMC_TipoOperacion='" . $tipo_oper . "'
                    AND g.GUIAREMC_FlagEstado = 1
					  ...
					  GROUP BY g.GUIAREMP_Codigo
                ORDER BY " . ($tipo_oper == 'V' ? 'g.GUIAREMC_Serie DESC, g.GUIAREMC_Numero DESC' : 'g.GUIAREMC_FechaRegistro DESC') . " " . $limit;
//  AND g.GUIAREMC_NumeroRef = ''  and cp.GUIAREMP_Codigo IS NULL   no se muestra los seleccionados
        //echo $sql;
        /*
          $sql = "SELECT g.GUIAREMC_Fecha,
          g.GUIAREMP_Codigo,
...
public function obterner_order($id){
        $where = array("OCOMP_Codigo" => $id);
$query = $this->db->where($where)->get('cji_ordencompra',1);
        if ($query->num_rows > 0)
            return $query->result();
        else
            return array();
...
public function getNumeroSerieOrderVenta($codigo){
