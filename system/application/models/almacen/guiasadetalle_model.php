...
//                $cantidad = ($cantidad * $factor)/$factorprin;
     ...
     * @return mixed
     */
    public function insertar_2015(stdClass $filter = null)
    {
        ...
        if (count($lotes) > 0) {
            ...
            $filter2->KARDC_Costo = $costo_anterior;
            $filter2->KARDC_CostoPromedio = $costo;
            //$filter2->LOTP_Codigo = "NULL"; // VERIFICAR SI ES CORRECTO HACER ESTO
            $insertarKardex = $this->kardex_model->insertar_2015(6, $filter2);
        }
